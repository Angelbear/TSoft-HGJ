...
import cn.edu.tsinghua.hpc.syncbroker.VBuilder;
import cn.edu.tsinghua.hpc.syncbroker.VCardParser_V21;
import cn.edu.tsinghua.hpc.tcontacts.pim2.ContactStruct;
import cn.edu.tsinghua.hpc.tcontacts.pim2.VCardComposer;
import cn.edu.tsinghua.hpc.tcontacts.pim2.VCardConfig;
import cn.edu.tsinghua.hpc.tcontacts.pim2.VCardDataBuilder;
import cn.edu.tsinghua.hpc.tcontacts.pim2.VCardParser;

public class ExportContactsToSystemActivity extends Activity {
	private static final String TAG = "ExportContactsToSystemActivity";
	...
	protected void onCreate(Bundle bundle) {
		...
		startExportContactsToSystemActivity();
	}

	private void startExportContactsToSystemActivity() {
	...
	cn.edu.tsinghua.hpc.tcontacts.pim.VCardComposer.OneEntryHandler {

		@Override
		public boolean onEntryCreated(String vcard) {
			//android.pim.vcard.VCardDataBuilder builder;
			cn.edu.tsinghua.hpc.tcontacts.pim2.VCardDataBuilder builder;
			try {
				builder = new VCardDataBuilder("UTF-8", "UTF-8", false,
						VCardConfig.VCARD_TYPE_V21_GENERIC, null);
				cn.edu.tsinghua.hpc.tcontacts.pim2.VCardParser p = new cn.edu.tsinghua.hpc.tcontacts.pim2.VCardParser_V21();
				p.parse(new ByteArrayInputStream(vcard.getBytes()), "UTF-8",
						builder);
				cn.edu.tsinghua.hpc.tcontacts.pim2.ContactStruct contact = builder.mLastContactStruct;
	...
	private class ErrorReasonDisplayer implements Runnable {
		...
		public void run() {
			...
			if (!ExportContactsToSystemActivity.this.isFinishing()) {
				showDialog(mResId);
			}
		}
	}
	...
	public Dialog getExportConfirmationDialog() {
		return new AlertDialog.Builder(this).setTitle(
				...
				getString(R.string.confirm_export_to_system))
	...
	public Dialog getErrorDialogWithReason() {
		if (mErrorReason == null) {
			Log.e(TAG, "Error reason must have been set.");
	...
	protected void onPrepareDialog(int id, Dialog dialog) {
		if (id == R.id.dialog_fail_to_export_with_reason) {
			...
			((AlertDialog) dialog)
					.setMessage(getString(R.string.confirm_export_to_system));
		} else {
	...
	private Dialog getExportingVCardDialog() {
		if (mProgressDialog == null) {
			String title = getString(R.string.exporting_system_message);
			String message = getString(R.string.export_to_system);
			mProgressDialog = new ProgressDialog(
					ExportContactsToSystemActivity.this);
	...
	private class ActualExportThread extends Thread implements
		...
		public void run() {
			...
			mWakeLock.acquire();
			cn.edu.tsinghua.hpc.tcontacts.pim.VCardComposer composer = null;
			try {
				composer = new cn.edu.tsinghua.hpc.tcontacts.pim.VCardComposer(
						ExportContactsToSystemActivity.this,
