...
public class ContactsProvider2 extends SQLiteContentProvider implements
	...
	private boolean initialize() {
		...
		mNameSplitter = new NameSplitter(context.getString(InternalResource
				.getString("common_name_prefixes")),
				context.getString(InternalResource
						.getString("common_last_name_prefixes")),
				context.getString(InternalResource
						.getString("common_name_suffixes")),
				context.getString(InternalResource
						.getString("common_name_conjunctions")), locale);
	...
	private long insertGroup(Uri uri, ContentValues values, Account account,
		...
		if (packageName != null) {
			overriddenValues.put(GroupsColumns.PACKAGE_ID,
					mDbHelper.getPackageId(packageName));
		}
		...
		if (dataId != null) {
			...
			if (matchEmail) {
				...
				mSb.append(DataColumns.MIMETYPE_ID + " IN (")
						.append(mMimeTypeIdEmail).append(",")
	...
	protected int updateInTransaction(Uri uri, ContentValues values,
		...
		case CONTACTS_LOOKUP_ID: {
			...
			final String lookupKey = pathSegments.get(2);
			// final long contactId = lookupContactIdByLookupKey(mDb,
			// lookupKey);
			// add by chenqiang
		...
		try {
			while (c.moveToNext()) {
				if (c.isNull(0) && c.isNull(1)) {
					...
					if (GoogleSource.ACCOUNT_TYPE.equals(c.getString(1)))
						continue;
	...
	private static class RawContactsEntityIterator implements EntityIterator {
		...
		public Entity next() throws RemoteException {
			...
			contactValues.put(RawContacts.ACCOUNT_TYPE,
					c.getString(COLUMN_ACCOUNT_TYPE));
			...
			contactValues.put(RawContacts.SOURCE_ID,
					c.getString(COLUMN_SOURCE_ID));
			...
			contactValues.put(RawContacts.CONTACT_ID,
					c.getLong(COLUMN_CONTACT_ID));
			contactValues.put(RawContacts.STARRED, c.getLong(COLUMN_STARRED));
			contactValues.put(TRawContacts.IS_RESTRICTED,
					c.getInt(COLUMN_IS_RESTRICTED));
			...
			do {
				...
				dataValues.put(TData.RES_PACKAGE,
						c.getString(COLUMN_RES_PACKAGE));
				dataValues.put(Data.MIMETYPE, c.getString(COLUMN_MIMETYPE));
				dataValues.put(Data.IS_PRIMARY, c.getLong(COLUMN_IS_PRIMARY));
				dataValues.put(Data.IS_SUPER_PRIMARY,
						c.getLong(COLUMN_IS_SUPER_PRIMARY));
				dataValues.put(Data.DATA_VERSION,
						c.getLong(COLUMN_DATA_VERSION));
	...
	public void appendRawContactsByFilterAsNestedQuery(StringBuilder sb,
		...
		appendRawContactsByNormalizedNameFilter(sb,
				NameNormalizer.normalize(filterParam), limit, true);
	}
