...
import cn.edu.tsinghua.hpc.tcontacts.pim.VCardComposer.HandlerForOutputStream;
...
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.DisplayNameSources;
...
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.NicknameLookupColumns;
...
public class ContactsProvider2 extends SQLiteContentProvider implements
	...
	/**
	 * Shared preference key for the legacy contact import version. The need for
	 * a version as opposed to a boolean flag is that if we discover bugs in the
	...
	private static final int CONTACTS_FOR_QUERY = 1011;
	...
	private interface ContactsQuery {
		...
		public static final String[] PROJECTION = new String[] {
				RawContactsColumns.CONCRETE_ID, RawContacts.ACCOUNT_NAME,
	...
	private interface DisplayNameQuery {
		...
		public static final String[] COLUMNS = new String[] {
				MimetypesColumns.MIMETYPE, Data.IS_PRIMARY, Data.DATA1,
				Organization.TITLE, };
	...
	private interface RawContactsQuery {
		...
		String[] COLUMNS = new String[] { ContactsContract.RawContacts.DELETED };
	...
	static {
		...
		matcher
				.addURI(TContactsContract.AUTHORITY,
	...
	static {
		...
		sContactsProjectionMap
				.put(Contacts.DISPLAY_NAME, Contacts.DISPLAY_NAME);
		...
		sContactsVCardProjectionMap.put(OpenableColumns.SIZE, "0 AS "
		...
		sPhoneLookupProjectionMap.put(PhoneLookup.SEND_TO_VOICEMAIL,
				ContactsColumns.CONCRETE_SEND_TO_VOICEMAIL + " AS "
		...
		columns
				.put(
						...
						"(SELECT COUNT(*) FROM (SELECT 1 FROM "
								...
								+ Clauses.GROUP_BY_ACCOUNT_CONTACT_ID
								+ " HAVING " + Clauses.HAVING_NO_GROUPS
								+ ")) AS " + Settings.UNGROUPED_WITH_PHONES);
	...
	private abstract class DataRowHandler {
		...
		public void update(SQLiteDatabase db, ContentValues values, Cursor c,
			...
			if (values.size() > 0) {
				mDb
						.update(Tables.DATA, values, Data._ID + " = " + dataId,
								null);
			}
		...
		protected Cursor queryData(SQLiteDatabase db, long rawContactId) {
			return db.query(DataDeleteQuery.TABLE,
					DataDeleteQuery.CONCRETE_COLUMNS, Data.RAW_CONTACT_ID + "="
							+ rawContactId + " AND "
							+ MimetypesColumns.MIMETYPE + "='" + mMimetype
							+ "'", null, null, null, null);
		}
		...
		protected void fixRawContactDisplayName(SQLiteDatabase db,
			...
			Cursor c = db.query(DisplayNameQuery.TABLE,
					DisplayNameQuery.COLUMNS, Data.RAW_CONTACT_ID + "="
							+ rawContactId, null, null, null, null);
			try {
				while (c.moveToNext()) {
					String mimeType = c.getString(DisplayNameQuery.MIMETYPE);

					// Display name is at DATA1 in all type. This is ensured in
					// the constructor.
					...
					boolean primary = StructuredName.CONTENT_ITEM_TYPE
							.equals(mimeType)
					...
					if (name != null) {
						...
						if (source != null
								&& (source > bestDisplayNameSource || (source == bestDisplayNameSource && primary))) {
	...
	private HashMap<String, SoftReference<String[]>> mNicknameClusterCache = new HashMap<String, SoftReference<String[]>>();
	...
	private HashSet<Long> mInsertedRawContacts = new HashSet<Long>();
	private HashSet<Long> mUpdatedRawContacts = new HashSet<Long>();
	private HashMap<Long, Object> mUpdatedSyncStates = new HashMap<Long, Object>();
	...
	public boolean isNetworkNotified() {
		Log.d(TAG, "isNetworkNotified = " + mNetworkNotified);
	 ...
	 */
	protected/* package */ContactsProvider2(ContactAggregationScheduler scheduler) {
	...
	private boolean initialize() {
		...
		// XXX
		// do not support affregator first
		mContactAggregator.setEnabled(true);

		final SQLiteDatabase db = mDbHelper.getReadableDatabase();

		mSetPrimaryStatement = db.compileStatement("UPDATE " + Tables.DATA
				+ " SET " + Data.IS_PRIMARY + "=(_id=?)" + " WHERE "
				+ DataColumns.MIMETYPE_ID + "=?" + "   AND "
				+ Data.RAW_CONTACT_ID + "=?");

		mSetSuperPrimaryStatement = db.compileStatement("UPDATE " + Tables.DATA
				+ " SET " + Data.IS_SUPER_PRIMARY + "=(" + Data._ID + "=?)"
				+ " WHERE " + DataColumns.MIMETYPE_ID + "=?" + "   AND "
				+ Data.RAW_CONTACT_ID + " IN (" + "SELECT " + RawContacts._ID
				+ " FROM " + Tables.RAW_CONTACTS + " WHERE "
				+ RawContacts.CONTACT_ID + " =(" + "SELECT "
				+ RawContacts.CONTACT_ID + " FROM " + Tables.RAW_CONTACTS
				+ " WHERE " + RawContacts._ID + "=?))");

		mContactsLastTimeContactedUpdate = db.compileStatement("UPDATE "
				+ Tables.CONTACTS + " SET " + Contacts.LAST_TIME_CONTACTED
				+ "=? " + "WHERE " + Contacts._ID + "=?");

		mRawContactDisplayNameUpdate = db.compileStatement("UPDATE "
				+ Tables.RAW_CONTACTS + " SET "
				+ RawContactsColumns.DISPLAY_NAME + "=?,"
				+ RawContactsColumns.DISPLAY_NAME_SOURCE + "=?" + " WHERE "
				+ RawContacts._ID + "=?");

		mRawContactDirtyUpdate = db.compileStatement("UPDATE "
				+ Tables.RAW_CONTACTS + " SET " + RawContacts.DIRTY
				+ "=1 WHERE " + RawContacts._ID + "=?");

		mLastStatusUpdate = db.compileStatement("UPDATE " + Tables.CONTACTS
		...
		mNameSplitter = new NameSplitter(context
				.getString(InternalResource.getString("common_name_prefixes")), context
				.getString(InternalResource.getString("common_last_name_prefixes")), context
				.getString(InternalResource.getString("common_name_suffixes")), context
				.getString(InternalResource.getString("common_name_conjunctions")), locale);
		mNameLookupBuilder = new StructuredNameLookupBuilder(mNameSplitter);
		mPostalSplitter = new PostalSplitter(locale);

		mNameLookupInsert = db.compileStatement("INSERT OR IGNORE INTO "
		...
		mNameLookupDelete = db.compileStatement("DELETE FROM "
				+ Tables.NAME_LOOKUP + " WHERE " + NameLookupColumns.DATA_ID
				+ "=?");

		mStatusUpdateInsert = db.compileStatement("INSERT INTO "
		...
		mStatusUpdateReplace = db.compileStatement("INSERT OR REPLACE INTO "
				+ Tables.STATUS_UPDATES + "(" + StatusUpdatesColumns.DATA_ID
				+ ", " + StatusUpdates.STATUS_TIMESTAMP + ","
		...
		mStatusUpdateAutoTimestamp = db.compileStatement("UPDATE "
		...
		mStatusAttributionUpdate = db.compileStatement("UPDATE "
				+ Tables.STATUS_UPDATES + " SET "
				+ StatusUpdates.STATUS_RES_PACKAGE + "=?,"
				+ StatusUpdates.STATUS_ICON + "=?,"
		...
		mStatusUpdateDelete = db.compileStatement("DELETE FROM "
	...
	/* package */protected ContactAggregationScheduler getContactAggregationScheduler() {
		...
		Thread importThread = new Thread("LegacyContactImport") {
			...
			public void run() {
				if (importLegacyContacts()) {
					...
					/*
					 * When the import process is done, we can unlock the
					 * provider and start aggregating the imported contacts
					 * asynchronously.
					 */
	...
	/* Visible for testing */
	/* package */public boolean importLegacyContacts(
	...
	private void flushTransactionalChanges() {
		...
		if (!mUpdatedRawContacts.isEmpty()) {
			...
			mDb.execSQL(
					"UPDATE raw_contacts SET version = version + 1 WHERE _id in "
							+ ids, new Object[] {});
		}
	...
	public void scheduleContactAggregation() {
	 ...
	 * 
	 * @param values
	 *            the ContentValues to read from and update
	 * @param account
	 *            the explicitly provided Account
	...
	private long insertRawContact(ContentValues values, Account account) {
		...
		long rawContactId = mDb.insert(Tables.RAW_CONTACTS,
				RawContacts.CONTACT_ID, overriddenValues);
	...
	private long insertData(ContentValues values, boolean callerIsSyncAdapter) {
		...
		if (packageName != null) {
			mValues.put(DataColumns.PACKAGE_ID, mDbHelper
	...
	private long getOrMakeGroup(SQLiteDatabase db, long rawContactId,
		...
		Cursor c = db.query(ContactsQuery.TABLE, ContactsQuery.PROJECTION,
				RawContacts._ID + "=" + rawContactId, null, null, null, null);
		try {
			if (c.moveToNext()) {
				final String accountName = c
						.getString(ContactsQuery.ACCOUNT_NAME);
				final String accountType = c
						.getString(ContactsQuery.ACCOUNT_TYPE);
	...
	private long insertGroup(Uri uri, ContentValues values, Account account,
		...
		// Replace package with internal mapping
		final String packageName = overriddenValues
				.getAsString(TGroups.RES_PACKAGE);
		if (packageName != null) {
			overriddenValues.put(GroupsColumns.PACKAGE_ID, mDbHelper
					.getPackageId(packageName));
		}
		overriddenValues.remove(TGroups.RES_PACKAGE);
	...
	public long insertStatusUpdate(ContentValues values) {
		...
		if (dataId != null) {
			...
			if (matchEmail) {
				...
				// in fact the same
				// column - Data.DATA1
				mSb.append(DataColumns.MIMETYPE_ID + " IN (").append(
						mMimeTypeIdEmail).append(",").append(mMimeTypeIdIm)
						.append(")" + " AND " + Data.DATA1 + "=");
				DatabaseUtils.appendEscapedSQLString(mSb, handle);
				mSb.append(" AND ((" + DataColumns.MIMETYPE_ID + "=").append(
						mMimeTypeIdIm).append(" AND " + Im.PROTOCOL + "=")
						.append(protocol);
				if (customProtocol != null) {
					mSb.append(" AND " + Im.CUSTOM_PROTOCOL + "=");
					DatabaseUtils.appendEscapedSQLString(mSb, customProtocol);
				}
				mSb.append(") OR (" + DataColumns.MIMETYPE_ID + "=").append(
						mMimeTypeIdEmail).append("))");
			} else {
				mSb.append(DataColumns.MIMETYPE_ID + "=").append(mMimeTypeIdIm)
						.append(" AND " + Im.PROTOCOL + "=").append(protocol)
						.append(" AND " + Im.DATA + "=");
				DatabaseUtils.appendEscapedSQLString(mSb, handle);
				if (customProtocol != null) {
					mSb.append(" AND " + Im.CUSTOM_PROTOCOL + "=");
			...
			if (values.containsKey(StatusUpdates.DATA_ID)) {
				mSb.append(" AND " + DataColumns.CONCRETE_ID + "=").append(
						values.getAsLong(StatusUpdates.DATA_ID));
			}
		}
		...
		Cursor cursor = null;
		try {
			cursor = mDb.query(DataContactsQuery.TABLE,
					DataContactsQuery.PROJECTION, mSb.toString(), null, null,
		...
		if (values.containsKey(StatusUpdates.PRESENCE)) {
			...
			if (values.containsKey(StatusUpdates.IM_ACCOUNT)) {
				mValues.put(StatusUpdates.IM_ACCOUNT, values
						.getAsString(StatusUpdates.IM_ACCOUNT));
			}
			mValues.put(StatusUpdates.PRESENCE, values
					.getAsString(StatusUpdates.PRESENCE));
		...
		if (values.containsKey(StatusUpdates.STATUS)) {
			...
			if (TextUtils.isEmpty(status)) {
				...
				DatabaseUtils.bindObjectToProgram(mStatusUpdateReplace, 3,
						status);
				DatabaseUtils.bindObjectToProgram(mStatusUpdateReplace, 4,
						resPackage);
				DatabaseUtils.bindObjectToProgram(mStatusUpdateReplace, 5,
						iconResource);
				DatabaseUtils.bindObjectToProgram(mStatusUpdateReplace, 6,
						labelResource);
				...
				try {
					...
					DatabaseUtils.bindObjectToProgram(mStatusUpdateInsert, 2,
							status);
					DatabaseUtils.bindObjectToProgram(mStatusUpdateInsert, 3,
							resPackage);
					DatabaseUtils.bindObjectToProgram(mStatusUpdateInsert, 4,
							iconResource);
					DatabaseUtils.bindObjectToProgram(mStatusUpdateInsert, 5,
							labelResource);
					...
					DatabaseUtils.bindObjectToProgram(
							mStatusUpdateAutoTimestamp, 2, status);
					mStatusUpdateAutoTimestamp.bindLong(3, dataId);
					DatabaseUtils.bindObjectToProgram(
							mStatusUpdateAutoTimestamp, 4, status);
					mStatusUpdateAutoTimestamp.execute();

					DatabaseUtils.bindObjectToProgram(mStatusAttributionUpdate,
							1, resPackage);
					DatabaseUtils.bindObjectToProgram(mStatusAttributionUpdate,
							2, iconResource);
					DatabaseUtils.bindObjectToProgram(mStatusAttributionUpdate,
	...
	protected int deleteInTransaction(Uri uri, String selection,
		...
		case RAW_CONTACTS: {
			...
			Cursor c = mDb.query(Tables.RAW_CONTACTS,
					new String[] { RawContacts._ID }, appendAccountToSelection(
							uri, selection), selectionArgs, null, null, null);
			try {
				while (c.moveToNext()) {
					final long rawContactId = c.getLong(0);
					numDeletes += deleteRawContact(rawContactId,
							callerIsSyncAdapter);
				}
			} finally {
				c.close();
		...
		case POSTALS_ID: {
			...
			mSyncToNetwork |= !callerIsSyncAdapter;
			return deleteData(Data._ID + "=" + dataId, null,
					callerIsSyncAdapter);
		}
	...
	private static boolean readBooleanQueryParameter(Uri uri, String name,
	...
	public int deleteRawContact(long rawContactId, boolean callerIsSyncAdapter) {
		if (callerIsSyncAdapter) {
			mDb.delete(Tables.PRESENCE, PresenceColumns.RAW_CONTACT_ID + "="
					+ rawContactId, null);
			return mDb.delete(Tables.RAW_CONTACTS, RawContacts._ID + "="
					+ rawContactId, null);
		} else {
...
//			final long contactId = lookupContactIdByLookupKey(mDb, lookupKey);
		...
		case DATA: {
			count = updateData(uri, values, appendAccountToSelection(uri,
		...
		case GROUPS: {
			count = updateGroups(uri, values, appendAccountToSelection(uri,
					selection), selectionArgs, callerIsSyncAdapter);
			if (count > 0) {
				mSyncToNetwork |= !callerIsSyncAdapter;
			}
	...
	private int updateGroups(Uri uri, ContentValues values,
		...
		if (updatedValues.containsKey(Groups.SHOULD_SYNC)
			...
			Cursor c = mDb.query(Tables.GROUPS, new String[] {
					Groups.ACCOUNT_NAME, Groups.ACCOUNT_TYPE }, Groups._ID
	...
	private int updateRawContact(long rawContactId, ContentValues values) {
		...
		if (requestUndoDelete) {
			Cursor cursor = mDb
					.query(RawContactsQuery.TABLE, RawContactsQuery.COLUMNS,
	...
	private int updateContactOptions(long contactId, ContentValues values) {
		...
		return mDb.update(Tables.CONTACTS, mValues, Contacts._ID + "="
				+ contactId, null);
	}
	...
	private int updateAggregationException(SQLiteDatabase db,
		...
		if (exceptionType == AggregationExceptions.TYPE_AUTOMATIC) {
			db.delete(Tables.AGGREGATION_EXCEPTIONS,
					AggregationExceptions.RAW_CONTACT_ID1 + "=" + rawContactId1
	...
	public void onAccountsUpdated(Account[] accounts) {
		...
		try {
			...
			findValidAccounts(existingAccounts, hasUnassignedContacts,
					Tables.SETTINGS, Settings.ACCOUNT_NAME,
					Settings.ACCOUNT_TYPE);

			// Remove all valid accounts from the existing account set. What is
			// left
			// in the existingAccounts set will be extra accounts whose data
			// must be deleted.
	...
	private void findValidAccounts(Set<Account> validAccounts,
			boolean[] hasUnassignedContacts, String table,
			String accountNameColumn, String accountTypeColumn) {
		Cursor c = mDb.rawQuery("SELECT DISTINCT " + accountNameColumn + ","
				+ accountTypeColumn + " FROM " + table, null);
		try {
			while (c.moveToNext()) {
				if (c.isNull(0) && c.isNull(1)) {
					hasUnassignedContacts[0] = true;
				} else {
					if(GoogleSource.ACCOUNT_TYPE.equals(c.getString(1))) continue;
					Log.d("Accounts",c.getString(0)+"#"+c.getString(1));
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		case CONTACTS_LOOKUP_ID: {
			...
			if (segmentCount == 4) {
				...
				Cursor c = query(db, lookupQb, projection, selection,
						selectionArgs, sortOrder, groupBy, limit);
				if (c.getCount() != 0) {
					return c;
				}
			...
			qb.appendWhere(Contacts._ID + "="
					+ lookupContactIdByLookupKey(db, lookupKey));
			break;
		}
		...
		case CONTACTS_GROUP: {
			...
			if (uri.getPathSegments().size() > 2) {
				...
				selectionArgs = insertSelectionArg(selectionArgs, uri
						.getLastPathSegment());
			}
			break;
		}
	...
	private void setTablesAndProjectionMapForContacts(SQLiteQueryBuilder qb,
		...
		String requestingPackage = uri
				.getQueryParameter(REQUESTING_PACKAGE_PARAM_KEY);
	...
	private void setTablesAndProjectionMapForRawContactsEntities(
		...
		String requestingPackage = uri.getQueryParameter("requesting_package");
		if (requestingPackage != null) {
	...
	private void appendAccountFromParameter(SQLiteQueryBuilder qb, Uri uri) {
		final String accountName = uri
				.getQueryParameter(RawContacts.ACCOUNT_NAME);
		final String accountType = uri
				.getQueryParameter(RawContacts.ACCOUNT_TYPE);
		if (!TextUtils.isEmpty(accountName)) {
	...
	String getContactsRestrictions() {
		if (mDbHelper.hasAccessToRestrictedData()) {
			...
			return TRawContacts.IS_RESTRICTED + "=0";
		}
	}
	...
	public AssetFileDescriptor openAssetFile(Uri uri, String mode)
		...
		case CONTACTS_PHOTO: {
			if (!"r".equals(mode)) {
				throw new FileNotFoundException("Mode " + mode
						+ " not supported.");
			}
			...
			String sql = "SELECT " + Photo.PHOTO + " FROM "
					+ mDbHelper.getDataView() + " WHERE " + Data._ID + "="
		...
		case CONTACTS_AS_VCARD: {
			...
			// then pipe into MemoryFile once the exact size is known. final
	...
	private AssetFileDescriptor buildAssetFileDescriptor(
		...
		try {
			...
			try {
				// XXX
				// Hack!!!!
				Method methodDeactivate = Class
						.forName("android.os.MemoryFile").getDeclaredMethod(
								"deactivate", new Class[] {});
	...
	/**
	 * An implementation of EntityIterator that joins the contacts and data
	 * tables and consumes all the data rows for a contact in order to build the
	 * Entity for a contact.
	 */
	private static class RawContactsEntityIterator implements EntityIterator {
		...
		public static final String[] PROJECTION = new String[] {
				...
				RawContacts.SOURCE_ID, RawContacts.VERSION, RawContacts.DIRTY,
				RawContacts.Entity.DATA_ID, TData.RES_PACKAGE, Data.MIMETYPE,
				Data.DATA1, Data.DATA2, Data.DATA3, Data.DATA4, Data.DATA5,
				Data.DATA6, Data.DATA7, Data.DATA8, Data.DATA9, Data.DATA10,
				Data.DATA11, Data.DATA12, Data.DATA13, Data.DATA14,
				Data.DATA15, Data.SYNC1, Data.SYNC2, Data.SYNC3, Data.SYNC4,
				RawContacts._ID, Data.IS_PRIMARY, Data.IS_SUPER_PRIMARY,
				Data.DATA_VERSION, GroupMembership.GROUP_SOURCE_ID,
				RawContacts.SYNC1, RawContacts.SYNC2, RawContacts.SYNC3,
				RawContacts.SYNC4, RawContacts.DELETED, RawContacts.CONTACT_ID,
				RawContacts.STARRED, TRawContacts.IS_RESTRICTED };
		...
		public RawContactsEntityIterator(ContactsProvider2 provider,
				Uri entityUri, String contactsIdString, String selection,
				String[] selectionArgs, String sortOrder) {
			mIsClosed = false;
			Uri uri;
			if (contactsIdString != null) {
				uri = Uri.withAppendedPath(TRawContacts.CONTENT_URI,
						contactsIdString);
				uri = Uri.withAppendedPath(uri,
						RawContacts.Entity.CONTENT_DIRECTORY);
			} else {
				uri = TContactsContract.RawContactsEntity.CONTENT_URI;
			}
			final Uri.Builder builder = uri.buildUpon();
			String query = entityUri.getQuery();
			builder.encodedQuery(query);
			mEntityCursor = provider.query(builder.build(), PROJECTION,
		...
		public void reset() throws RemoteException {
			if (mIsClosed) {
				throw new IllegalStateException(
						"calling reset() when the iterator is closed");
			}
			mEntityCursor.moveToFirst();
		}
		...
		public boolean hasNext() throws RemoteException {
			if (mIsClosed) {
				throw new IllegalStateException(
						"calling hasNext() when the iterator is closed");
			}
		...
		public Entity next() throws RemoteException {
			if (mIsClosed) {
				throw new IllegalStateException(
						"calling next() when the iterator is closed");
			}
			if (!hasNext()) {
				throw new IllegalStateException(
						"you may only call next() if hasNext() is true");
			}
			...
			contactValues.put(RawContacts.ACCOUNT_NAME, c
					.getString(COLUMN_ACCOUNT_NAME));
			contactValues.put(RawContacts.ACCOUNT_TYPE, c
					.getString(COLUMN_ACCOUNT_TYPE));
			...
			contactValues.put(RawContacts.SOURCE_ID, c
					.getString(COLUMN_SOURCE_ID));
			...
			contactValues.put(RawContacts.CONTACT_ID, c
					.getLong(COLUMN_CONTACT_ID));
			contactValues.put(RawContacts.STARRED, c.getLong(COLUMN_STARRED));
			contactValues.put(TRawContacts.IS_RESTRICTED, c
					.getInt(COLUMN_IS_RESTRICTED));
			...
			do {
				...
				dataValues.put(TData.RES_PACKAGE, c
						.getString(COLUMN_RES_PACKAGE));
				dataValues.put(Data.MIMETYPE, c.getString(COLUMN_MIMETYPE));
				dataValues.put(Data.IS_PRIMARY, c.getLong(COLUMN_IS_PRIMARY));
				dataValues.put(Data.IS_SUPER_PRIMARY, c
						.getLong(COLUMN_IS_SUPER_PRIMARY));
				dataValues.put(Data.DATA_VERSION, c
						.getLong(COLUMN_DATA_VERSION));
				if (!c.isNull(COLUMN_GROUP_SOURCE_ID)) {
					dataValues.put(GroupMembership.GROUP_SOURCE_ID, c
							.getString(COLUMN_GROUP_SOURCE_ID));
				}
				dataValues.put(Data.DATA_VERSION, c
						.getLong(COLUMN_DATA_VERSION));
				...
				contact.addSubValue(TData.CONTENT_URI, dataValues);
	...
	private static class GroupsEntityIterator implements EntityIterator {
		...
		private static final String[] PROJECTION = new String[] { Groups._ID,
				Groups.ACCOUNT_NAME, Groups.ACCOUNT_TYPE, Groups.SOURCE_ID,
				Groups.DIRTY, Groups.VERSION, TGroups.RES_PACKAGE,
				Groups.TITLE, TGroups.TITLE_RES, Groups.GROUP_VISIBLE,
				Groups.SYNC1, Groups.SYNC2, Groups.SYNC3, Groups.SYNC4,
				Groups.SYSTEM_ID, Groups.NOTES, Groups.DELETED,
				Groups.SHOULD_SYNC };
		...
		public GroupsEntityIterator(ContactsProvider2 provider,
				String groupIdString, Uri uri, String selection,
				String[] selectionArgs, String sortOrder) {
			mIsClosed = false;

			final String updatedSortOrder = (sortOrder == null) ? Groups._ID
			...
			final String accountName = uri
					.getQueryParameter(Groups.ACCOUNT_NAME);
			final String accountType = uri
					.getQueryParameter(Groups.ACCOUNT_TYPE);
			if (!TextUtils.isEmpty(accountName)) {
		...
		public Entity next() throws RemoteException {
			...
			// we expect the cursor is already at the row we need to read from
			ContentValues groupValues = new ContentValues();
			groupValues.put(Groups.ACCOUNT_NAME, c
					.getString(COLUMN_ACCOUNT_NAME));
			groupValues.put(Groups.ACCOUNT_TYPE, c
					.getString(COLUMN_ACCOUNT_TYPE));
			...
			groupValues.put(TGroups.RES_PACKAGE, c
					.getString(COLUMN_RES_PACKAGE));
			groupValues.put(Groups.TITLE, c.getString(COLUMN_TITLE));
			groupValues.put(TGroups.TITLE_RES, c.getString(COLUMN_TITLE_RES));
			groupValues.put(Groups.GROUP_VISIBLE, c
					.getLong(COLUMN_GROUP_VISIBLE));
			...
			groupValues
					.put(Groups.SHOULD_SYNC, c.getString(COLUMN_SHOULD_SYNC));
	...
	public EntityIterator queryEntities(Uri uri, String selection,
			String[] selectionArgs, String sortOrder) {
		waitForAccess();
		...
		case GROUPS_ID:
			...
			return new GroupsEntityIterator(this, idString, uri, selection,
	...
	public void insertNameLookupForEmail(long rawContactId, long dataId,
			String email) {
		...
		insertNameLookup(rawContactId, dataId,
				NameLookupType.EMAIL_BASED_NICKNAME, NameNormalizer
						.normalize(address));
	}
	...
	protected String[] getCommonNicknameClusters(String normalizedName) {
		...
		if (clusters == null) {
			...
			ref = clusters == null ? null : new SoftReference<String[]>(
					clusters);
			synchronized (mNicknameClusterCache) {
	...
	protected String[] loadNicknameClusters(String normalizedName) {
		SQLiteDatabase db = mDbHelper.getReadableDatabase();
		String[] clusters = null;
		Cursor cursor = db.query(NicknameLookupQuery.TABLE,
				NicknameLookupQuery.COLUMNS, NicknameLookupColumns.NAME + "=?",
				new String[] { normalizedName }, null, null, null);
		try {
	...
	public void appendContactFilterAsNestedQuery(StringBuilder sb,
		...
		sb.append("*' AND " + NameLookupColumns.NAME_TYPE + " IN("
				...
				+ NameLookupType.NICKNAME + "," + NameLookupType.ORGANIZATION
				+ "))");
	}
	...
	public void appendRawContactsByFilterAsNestedQuery(StringBuilder sb,
			String filterParam, String limit) {
		appendRawContactsByNormalizedNameFilter(sb, NameNormalizer
				.normalize(filterParam), limit, true);
	}

	private void appendRawContactsByNormalizedNameFilter(StringBuilder sb,
			String normalizedName, String limit, boolean allowEmailMatch) {
		sb.append("(" + "SELECT DISTINCT " + NameLookupColumns.RAW_CONTACT_ID
				+ " FROM " + Tables.NAME_LOOKUP + " WHERE "
