...
public class GlobalSearchSupport {
    ...
    private interface SearchSuggestionQuery {
        ...
        public static final String PRESENCE_SQL =
                "(SELECT " + TStatusUpdates.PRESENCE_STATUS +
        ...
        public static final String[] COLUMNS = {
            ...
            Phone.NUMBER,
            Contacts.PHOTO_ID,
            "raw_contacts.sync_state" + " AS " +"raw_sync_state",
        };
    ...
    public Cursor handleSearchSuggestionsQuery(SQLiteDatabase db, Uri uri, String limit) {
    	Log.d(TAG,"handleSearchSuggestionsQuery");
    ...
    private Cursor buildCursorForSearchSuggestionsBasedOnPhoneNumber(String searchClause) {
        ...
        s = r.getString(InternalResource.getString("dial_number_using"), searchClause);
        i = s.indexOf('\n');
        if (i < 0) {
            dialNumber.add(s);
            dialNumber.add("");
        ...
        dialNumber.add(String.valueOf(InternalResource.getDrawable("call_contact")));
        ...
        s = r.getString(InternalResource.getString("create_contact_using"), searchClause);
        i = s.indexOf('\n');
        if (i < 0) {
            createContact.add(s);
            createContact.add("");
        ...
        createContact.add(String.valueOf(InternalResource.getDrawable("create_contact")));
    ...
    private Cursor buildCursorForSearchSuggestions(SQLiteDatabase db, String selection,
		...
		String selection2 = "(raw_sync_state = '" + SyncState.SYNC_STATE_PRESENT
		+ "'" + " OR raw_sync_state = '" + SyncState.SYNC_STATE_RECOVER
		+ "'" + " OR raw_sync_state = '" + SyncState.SYNC_STATE_UPDATED
		+ "')";

        Cursor c = db.query(true, SearchSuggestionQuery.TABLE,
                SearchSuggestionQuery.COLUMNS, selection2 + " AND " + selection, null, null, null, null, null);
        try {
            while (c.moveToNext()) {
