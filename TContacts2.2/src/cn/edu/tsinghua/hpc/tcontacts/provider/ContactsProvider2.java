...
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactLookupKey.LookupKeySegment;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.AggregatedPresenceColumns;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.AggregationExceptionColumns;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.Clauses;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.ContactsColumns;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.ContactsStatusUpdatesColumns;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.DataColumns;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.GroupsColumns;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.MimetypesColumns;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.NameLookupColumns;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.NameLookupType;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.PhoneColumns;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.PhoneLookupColumns;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.PresenceColumns;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.RawContactsColumns;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.SettingsColumns;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.StatusUpdatesColumns;
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.Tables;
import cn.edu.tsinghua.hpc.tcontacts.util.TContactsContract;
import cn.edu.tsinghua.hpc.tcontacts.util.TContactsContract.CommonDataKinds.TStructuredName;
import cn.edu.tsinghua.hpc.tcontacts.util.TContactsContract.TContactCounts;
...
import cn.edu.tsinghua.hpc.tcontacts.util.TContactsContract.TDisplayNameSources;
import cn.edu.tsinghua.hpc.tcontacts.util.TContactsContract.TFullNameStyle;
...
import cn.edu.tsinghua.hpc.tcontacts.util.TContactsContract.TPhoneticNameStyle;
import cn.edu.tsinghua.hpc.tcontacts.util.TContactsContract.TProviderStatus;
import cn.edu.tsinghua.hpc.tcontacts.util.TContactsContract.TRawContacts;
import cn.edu.tsinghua.hpc.tcontacts.util.TContactsContract.TSearchSnippetColumns;
import cn.edu.tsinghua.hpc.tcontacts.util.TMethod;
import cn.edu.tsinghua.hpc.vcard.VCardComposer;
import cn.edu.tsinghua.hpc.vcard.VCardConfig;
 ...
 */
public class ContactsProvider2 extends SQLiteContentProvider implements
	...
	// TODO: carefully prevent all incoming nested queries; they can be gaping
	// security holes
	// TODO: check for restricted flag during insert(), update(), and delete()
	// calls
	...
	/**
	 * Property key for the legacy contact import version. The need for a
	 * version as opposed to a boolean flag is that if we discover bugs in the
	 * contact import process, we can trigger re-import by incrementing the
	 * import version.
	 */
	...
	private static final String STREQUENT_ORDER_BY = Contacts.STARRED
			+ " DESC, " + TIMES_CONTACED_SORT_COLUMN + " DESC, "
			+ Contacts.DISPLAY_NAME + " COLLATE LOCALIZED ASC";
	private static final String STREQUENT_LIMIT = "(SELECT COUNT(1) FROM "
			+ Tables.CONTACTS + " WHERE " + Contacts.STARRED + "=1) + 25";

	/* package */static final String UPDATE_TIMES_CONTACTED_CONTACTS_TABLE = "UPDATE "
			+ Tables.CONTACTS
			+ " SET "
			...
			+ " ("
			+ Contacts.TIMES_CONTACTED
			+ " + 1) END WHERE " + Contacts._ID + "=?";

	/* package */static final String UPDATE_TIMES_CONTACTED_RAWCONTACTS_TABLE = "UPDATE "
			+ Tables.RAW_CONTACTS
			+ " SET "
			...
			+ " ("
			+ RawContacts.TIMES_CONTACTED
			+ " + 1) END WHERE " + RawContacts.CONTACT_ID + "=?";
	...
	private static final int CONTACTS_FOR_QUERY = 1012;
	...
	private interface DataContactsQuery {
		...
		public static final String[] PROJECTION = new String[] {
				RawContactsColumns.CONCRETE_ID, DataColumns.CONCRETE_ID,
	...
	private interface DataDeleteQuery {
		...
		public static final String[] CONCRETE_COLUMNS = new String[] {
				DataColumns.CONCRETE_ID, MimetypesColumns.MIMETYPE,
				Data.RAW_CONTACT_ID, Data.IS_PRIMARY, Data.DATA1, };

		public static final String[] COLUMNS = new String[] { Data._ID,
				MimetypesColumns.MIMETYPE, Data.RAW_CONTACT_ID,
				Data.IS_PRIMARY, Data.DATA1, };
	...
	private interface RawContactsQuery {
		...
		String[] COLUMNS = new String[] { RawContacts.DELETED,
	...
	private static final String CONTACTS_IN_GROUP_SELECT = Contacts._ID
			+ " IN " + "(SELECT " + RawContacts.CONTACT_ID + " FROM "
			+ Tables.RAW_CONTACTS + " WHERE " + RawContactsColumns.CONCRETE_ID
			+ " IN " + "(SELECT " + DataColumns.CONCRETE_RAW_CONTACT_ID
			+ " FROM " + Tables.DATA_JOIN_MIMETYPES + " WHERE " + Data.MIMETYPE
			+ "='" + GroupMembership.CONTENT_ITEM_TYPE + "' AND "
			+ GroupMembership.GROUP_ROW_ID + "=" + "(SELECT " + Tables.GROUPS
			+ "." + Groups._ID + " FROM " + Tables.GROUPS + " WHERE "
			+ Groups.TITLE + "=?)))";

	/** Sql for updating DIRTY flag on multiple raw contacts */
	private static final String UPDATE_RAW_CONTACT_SET_DIRTY_SQL = "UPDATE "
			+ Tables.RAW_CONTACTS + " SET " + RawContacts.DIRTY + "=1"
			+ " WHERE " + RawContacts._ID + " IN (";

	/** Sql for updating VERSION on multiple raw contacts */
	private static final String UPDATE_RAW_CONTACT_SET_VERSION_SQL = "UPDATE "
			+ Tables.RAW_CONTACTS + " SET " + RawContacts.VERSION + " = "
			+ RawContacts.VERSION + " + 1" + " WHERE " + RawContacts._ID
			+ " IN (";

	/** Name lookup types used for contact filtering */
	private static final String CONTACT_LOOKUP_NAME_TYPES = NameLookupType.NAME_COLLATION_KEY
			+ ","
	...
	private static final String WHERE_CLAUSE_FOR_STATUS_UPDATES_TABLE = StatusUpdatesColumns.DATA_ID
			+ " IN (SELECT Distinct "
	...
	static {
		...
		matcher.addURI(TContactsContract.AUTHORITY, "contacts", CONTACTS);
		// For query guid sync_state
		matcher.addURI(TContactsContract.AUTHORITY, "contacts/query",
				CONTACTS_FOR_QUERY);
		matcher.addURI(TContactsContract.AUTHORITY, "contacts/#", CONTACTS_ID);
		matcher.addURI(TContactsContract.AUTHORITY, "contacts/#/data",
				CONTACTS_DATA);
		matcher.addURI(TContactsContract.AUTHORITY, "contacts/#/suggestions",
				AGGREGATION_SUGGESTIONS);
		matcher.addURI(TContactsContract.AUTHORITY, "contacts/#/suggestions/*",
				AGGREGATION_SUGGESTIONS);
		matcher.addURI(TContactsContract.AUTHORITY, "contacts/#/photo",
				CONTACTS_PHOTO);
		matcher.addURI(TContactsContract.AUTHORITY, "contacts/filter/*",
				CONTACTS_FILTER);
		matcher.addURI(TContactsContract.AUTHORITY, "contacts/lookup/*",
				CONTACTS_LOOKUP);
		matcher.addURI(TContactsContract.AUTHORITY, "contacts/lookup/*/#",
				CONTACTS_LOOKUP_ID);
		matcher.addURI(TContactsContract.AUTHORITY, "contacts/as_vcard/*",
				CONTACTS_AS_VCARD);
		matcher.addURI(TContactsContract.AUTHORITY,
				"contacts/as_multi_vcard/*", CONTACTS_AS_MULTI_VCARD);
		matcher.addURI(TContactsContract.AUTHORITY, "contacts/strequent/",
				CONTACTS_STREQUENT);
		matcher.addURI(TContactsContract.AUTHORITY,
				"contacts/strequent/filter/*", CONTACTS_STREQUENT_FILTER);
		matcher.addURI(TContactsContract.AUTHORITY, "contacts/group/*",
				CONTACTS_GROUP);

		matcher.addURI(TContactsContract.AUTHORITY, "raw_contacts",
				RAW_CONTACTS);
		matcher.addURI(TContactsContract.AUTHORITY, "raw_contacts/#",
				RAW_CONTACTS_ID);
		matcher.addURI(TContactsContract.AUTHORITY, "raw_contacts/#/data",
				RAW_CONTACTS_DATA);
		matcher.addURI(TContactsContract.AUTHORITY, "raw_contacts/#/entity",
				RAW_CONTACT_ENTITY_ID);

		matcher.addURI(TContactsContract.AUTHORITY, "raw_contact_entities",
				RAW_CONTACT_ENTITIES);

		matcher.addURI(TContactsContract.AUTHORITY, "data", DATA);
		matcher.addURI(TContactsContract.AUTHORITY, "data/#", DATA_ID);
		matcher.addURI(TContactsContract.AUTHORITY, "data/phones", PHONES);
		matcher.addURI(TContactsContract.AUTHORITY, "data/phones/#", PHONES_ID);
		matcher.addURI(TContactsContract.AUTHORITY, "data/phones/filter",
				PHONES_FILTER);
		matcher.addURI(TContactsContract.AUTHORITY, "data/phones/filter/*",
				PHONES_FILTER);
		matcher.addURI(TContactsContract.AUTHORITY, "data/emails", EMAILS);
		matcher.addURI(TContactsContract.AUTHORITY, "data/emails/#", EMAILS_ID);
		matcher.addURI(TContactsContract.AUTHORITY, "data/emails/lookup/*",
				EMAILS_LOOKUP);
		matcher.addURI(TContactsContract.AUTHORITY, "data/emails/filter",
				EMAILS_FILTER);
		matcher.addURI(TContactsContract.AUTHORITY, "data/emails/filter/*",
				EMAILS_FILTER);
		matcher.addURI(TContactsContract.AUTHORITY, "data/postals", POSTALS);
		matcher.addURI(TContactsContract.AUTHORITY, "data/postals/#",
				POSTALS_ID);

		matcher.addURI(TContactsContract.AUTHORITY, "groups", GROUPS);
		matcher.addURI(TContactsContract.AUTHORITY, "groups/#", GROUPS_ID);
		matcher.addURI(TContactsContract.AUTHORITY, "groups_summary",
				GROUPS_SUMMARY);

		matcher.addURI(TContactsContract.AUTHORITY,
				SyncStateContentProviderHelper.PATH, SYNCSTATE);
		matcher.addURI(TContactsContract.AUTHORITY,
				SyncStateContentProviderHelper.PATH + "/#", SYNCSTATE_ID);

		matcher.addURI(TContactsContract.AUTHORITY, "phone_lookup/*",
				PHONE_LOOKUP);
		matcher.addURI(TContactsContract.AUTHORITY, "aggregation_exceptions",
				AGGREGATION_EXCEPTIONS);
		matcher.addURI(TContactsContract.AUTHORITY, "aggregation_exceptions/*",
				AGGREGATION_EXCEPTION_ID);

		matcher.addURI(TContactsContract.AUTHORITY, "settings", SETTINGS);

		matcher.addURI(TContactsContract.AUTHORITY, "status_updates",
				STATUS_UPDATES);
		matcher.addURI(TContactsContract.AUTHORITY, "status_updates/#",
				STATUS_UPDATES_ID);
		...
		matcher.addURI(TContactsContract.AUTHORITY, "live_folders/contacts",
		...
		matcher.addURI(TContactsContract.AUTHORITY, "live_folders/favorites",
				LIVE_FOLDERS_CONTACTS_FAVORITES);

		matcher.addURI(TContactsContract.AUTHORITY, "provider_status",
				PROVIDER_STATUS);
	}
	...
	static {
		...
		sContactsProjectionMap.put("guid", "guid");
		sContactsProjectionMap.put("sync_state", "sync_state");

		sContactsProjectionMap.put(Contacts.DISPLAY_NAME,
				TContacts.DISPLAY_NAME_PRIMARY);
		sContactsProjectionMap.put(TContacts.DISPLAY_NAME_ALTERNATIVE,
				TContacts.DISPLAY_NAME_ALTERNATIVE);
		sContactsProjectionMap.put(TContacts.DISPLAY_NAME_SOURCE,
				TContacts.DISPLAY_NAME_SOURCE);
		sContactsProjectionMap.put(TContacts.PHONETIC_NAME,
				TContacts.PHONETIC_NAME);
		sContactsProjectionMap.put(TContacts.PHONETIC_NAME_STYLE,
				TContacts.PHONETIC_NAME_STYLE);
		sContactsProjectionMap.put(TContacts.SORT_KEY_PRIMARY,
				TContacts.SORT_KEY_PRIMARY);
		sContactsProjectionMap.put(TContacts.SORT_KEY_ALTERNATIVE,
				TContacts.SORT_KEY_ALTERNATIVE);
		sContactsProjectionMap.put(Contacts.LAST_TIME_CONTACTED,
				Contacts.LAST_TIME_CONTACTED);
		sContactsProjectionMap.put(Contacts.TIMES_CONTACTED,
				Contacts.TIMES_CONTACTED);
		sContactsProjectionMap.put(Contacts.STARRED, Contacts.STARRED);
		sContactsProjectionMap.put(Contacts.IN_VISIBLE_GROUP,
				Contacts.IN_VISIBLE_GROUP);
		sContactsProjectionMap.put(Contacts.PHOTO_ID, Contacts.PHOTO_ID);
		sContactsProjectionMap.put(Contacts.CUSTOM_RINGTONE,
				Contacts.CUSTOM_RINGTONE);
		sContactsProjectionMap.put(Contacts.HAS_PHONE_NUMBER,
				Contacts.HAS_PHONE_NUMBER);
		sContactsProjectionMap.put(Contacts.SEND_TO_VOICEMAIL,
		...
		addProjection(sContactsProjectionMap,
		...
		sContactsProjectionWithSnippetMap.put(
				TSearchSnippetColumns.SNIPPET_MIMETYPE,
				TSearchSnippetColumns.SNIPPET_MIMETYPE);
		sContactsProjectionWithSnippetMap.put(
				TSearchSnippetColumns.SNIPPET_DATA_ID,
				TSearchSnippetColumns.SNIPPET_DATA_ID);
		sContactsProjectionWithSnippetMap.put(
				TSearchSnippetColumns.SNIPPET_DATA1,
				TSearchSnippetColumns.SNIPPET_DATA1);
		sContactsProjectionWithSnippetMap.put(
				TSearchSnippetColumns.SNIPPET_DATA2,
				TSearchSnippetColumns.SNIPPET_DATA2);
		sContactsProjectionWithSnippetMap.put(
				TSearchSnippetColumns.SNIPPET_DATA3,
				TSearchSnippetColumns.SNIPPET_DATA3);
		sContactsProjectionWithSnippetMap.put(
				TSearchSnippetColumns.SNIPPET_DATA4,
				TSearchSnippetColumns.SNIPPET_DATA4);

		sStrequentStarredProjectionMap = new HashMap<String, String>(
		...
		sStrequentFrequentProjectionMap = new HashMap<String, String>(
				sContactsProjectionMap);
		...
		sContactsVCardProjectionMap.put(OpenableColumns.DISPLAY_NAME,
				Contacts.DISPLAY_NAME + " || '.vcf' AS "
						+ OpenableColumns.DISPLAY_NAME);
		sContactsVCardProjectionMap.put(OpenableColumns.SIZE, "NULL AS "
				+ OpenableColumns.SIZE);

		sRawContactsProjectionMap = new HashMap<String, String>();
		sRawContactsProjectionMap.put(RawContacts._ID, RawContacts._ID);
		sRawContactsProjectionMap.put("guid", "guid");
		sRawContactsProjectionMap.put("sync_state", "sync_state");
		sRawContactsProjectionMap.put(RawContacts.CONTACT_ID,
				RawContacts.CONTACT_ID);
		sRawContactsProjectionMap.put(RawContacts.ACCOUNT_NAME,
				RawContacts.ACCOUNT_NAME);
		sRawContactsProjectionMap.put(RawContacts.ACCOUNT_TYPE,
				RawContacts.ACCOUNT_TYPE);
		sRawContactsProjectionMap.put(RawContacts.SOURCE_ID,
		...
		sRawContactsProjectionMap.put(TRawContacts.DISPLAY_NAME_PRIMARY,
				TRawContacts.DISPLAY_NAME_PRIMARY);
		sRawContactsProjectionMap.put(TRawContacts.DISPLAY_NAME_ALTERNATIVE,
				TRawContacts.DISPLAY_NAME_ALTERNATIVE);
		sRawContactsProjectionMap.put(TRawContacts.DISPLAY_NAME_SOURCE,
				TRawContacts.DISPLAY_NAME_SOURCE);
		sRawContactsProjectionMap.put(TRawContacts.PHONETIC_NAME,
				TRawContacts.PHONETIC_NAME);
		sRawContactsProjectionMap.put(TRawContacts.PHONETIC_NAME_STYLE,
				TRawContacts.PHONETIC_NAME_STYLE);
		sRawContactsProjectionMap.put(TRawContacts.NAME_VERIFIED,
				TRawContacts.NAME_VERIFIED);
		sRawContactsProjectionMap.put(TRawContacts.SORT_KEY_PRIMARY,
				TRawContacts.SORT_KEY_PRIMARY);
		sRawContactsProjectionMap.put(TRawContacts.SORT_KEY_ALTERNATIVE,
				TRawContacts.SORT_KEY_ALTERNATIVE);
		sRawContactsProjectionMap.put(RawContacts.TIMES_CONTACTED,
				RawContacts.TIMES_CONTACTED);
		sRawContactsProjectionMap.put(RawContacts.LAST_TIME_CONTACTED,
				RawContacts.LAST_TIME_CONTACTED);
		sRawContactsProjectionMap.put(RawContacts.CUSTOM_RINGTONE,
				RawContacts.CUSTOM_RINGTONE);
		sRawContactsProjectionMap.put(RawContacts.SEND_TO_VOICEMAIL,
		...
		sDataProjectionMap.put(TData.RES_PACKAGE, TData.RES_PACKAGE);
		...
		sDataProjectionMap.put(TRawContacts.NAME_VERIFIED,
				TRawContacts.NAME_VERIFIED);
		...
		sDataProjectionMap.put(TContacts.DISPLAY_NAME_ALTERNATIVE,
				TContacts.DISPLAY_NAME_ALTERNATIVE);
		sDataProjectionMap.put(TContacts.DISPLAY_NAME_SOURCE,
				TContacts.DISPLAY_NAME_SOURCE);
		sDataProjectionMap
				.put(TContacts.PHONETIC_NAME, TContacts.PHONETIC_NAME);
		sDataProjectionMap.put(TContacts.PHONETIC_NAME_STYLE,
				TContacts.PHONETIC_NAME_STYLE);
		sDataProjectionMap.put(TContacts.SORT_KEY_PRIMARY,
				TContacts.SORT_KEY_PRIMARY);
		sDataProjectionMap.put(TContacts.SORT_KEY_ALTERNATIVE,
				TContacts.SORT_KEY_ALTERNATIVE);
		sDataProjectionMap.put(Contacts.CUSTOM_RINGTONE,
				Contacts.CUSTOM_RINGTONE);
		sDataProjectionMap.put(Contacts.SEND_TO_VOICEMAIL,
				Contacts.SEND_TO_VOICEMAIL);
		sDataProjectionMap.put(Contacts.LAST_TIME_CONTACTED,
				Contacts.LAST_TIME_CONTACTED);
		sDataProjectionMap.put(Contacts.TIMES_CONTACTED,
				Contacts.TIMES_CONTACTED);
		sDataProjectionMap.put(Contacts.STARRED, Contacts.STARRED);
		sDataProjectionMap.put(Contacts.PHOTO_ID, Contacts.PHOTO_ID);
		sDataProjectionMap.put(Contacts.IN_VISIBLE_GROUP,
				Contacts.IN_VISIBLE_GROUP);
		sDataProjectionMap.put(TContacts.NAME_RAW_CONTACT_ID,
				TContacts.NAME_RAW_CONTACT_ID);
		sDataProjectionMap.put(GroupMembership.GROUP_SOURCE_ID,
		...
		columns.put("sync_state", "sync_state");
		...
		columns.put(TRawContacts.IS_RESTRICTED, TRawContacts.IS_RESTRICTED);
		...
		columns.put(TRawContacts.NAME_VERIFIED, TRawContacts.NAME_VERIFIED);
		columns.put(TData.RES_PACKAGE, TData.RES_PACKAGE);
		...
		columns.put(GroupMembership.GROUP_SOURCE_ID,
				GroupMembership.GROUP_SOURCE_ID);
		...
		addProjection(sDataProjectionMap, Data.PRESENCE, Tables.PRESENCE + "."
		...
		// Projection map for data grouped by contact (not raw contact) and some
		// data field(s)
		sDistinctDataProjectionMap = new HashMap<String, String>();
		sDistinctDataProjectionMap.put(Data._ID, "MIN(" + Data._ID + ") AS "
				+ Data._ID);
		...
		sDistinctDataProjectionMap.put(Data.IS_SUPER_PRIMARY,
				Data.IS_SUPER_PRIMARY);
		sDistinctDataProjectionMap.put(TData.RES_PACKAGE, TData.RES_PACKAGE);
		...
		sDistinctDataProjectionMap.put(RawContacts.CONTACT_ID,
				RawContacts.CONTACT_ID);
		sDistinctDataProjectionMap
				.put(Contacts.LOOKUP_KEY, Contacts.LOOKUP_KEY);
		sDistinctDataProjectionMap.put(Contacts.DISPLAY_NAME,
				Contacts.DISPLAY_NAME);
		sDistinctDataProjectionMap.put(TContacts.DISPLAY_NAME_ALTERNATIVE,
				TContacts.DISPLAY_NAME_ALTERNATIVE);
		sDistinctDataProjectionMap.put(TContacts.DISPLAY_NAME_SOURCE,
				TContacts.DISPLAY_NAME_SOURCE);
		sDistinctDataProjectionMap.put(TContacts.PHONETIC_NAME,
				TContacts.PHONETIC_NAME);
		sDistinctDataProjectionMap.put(TContacts.PHONETIC_NAME_STYLE,
				TContacts.PHONETIC_NAME_STYLE);
		sDistinctDataProjectionMap.put(TContacts.SORT_KEY_PRIMARY,
				TContacts.SORT_KEY_PRIMARY);
		sDistinctDataProjectionMap.put(TContacts.SORT_KEY_ALTERNATIVE,
				TContacts.SORT_KEY_ALTERNATIVE);
		sDistinctDataProjectionMap.put(Contacts.CUSTOM_RINGTONE,
				Contacts.CUSTOM_RINGTONE);
		sDistinctDataProjectionMap.put(Contacts.SEND_TO_VOICEMAIL,
				Contacts.SEND_TO_VOICEMAIL);
		sDistinctDataProjectionMap.put(Contacts.LAST_TIME_CONTACTED,
				Contacts.LAST_TIME_CONTACTED);
		sDistinctDataProjectionMap.put(Contacts.TIMES_CONTACTED,
				Contacts.TIMES_CONTACTED);
		...
		sDistinctDataProjectionMap.put(Contacts.IN_VISIBLE_GROUP,
				Contacts.IN_VISIBLE_GROUP);
		...
		addProjection(sDistinctDataProjectionMap, Contacts.CONTACT_STATUS,
				ContactsStatusUpdatesColumns.CONCRETE_STATUS);
		addProjection(sDistinctDataProjectionMap,
				Contacts.CONTACT_STATUS_TIMESTAMP,
				ContactsStatusUpdatesColumns.CONCRETE_STATUS_TIMESTAMP);
		addProjection(sDistinctDataProjectionMap,
				Contacts.CONTACT_STATUS_RES_PACKAGE,
		...
		sPhoneLookupProjectionMap.put(PhoneLookup._ID, "contacts_view."
				+ Contacts._ID + " AS " + PhoneLookup._ID);
		sPhoneLookupProjectionMap.put(PhoneLookup.LOOKUP_KEY, "contacts_view."
				+ Contacts.LOOKUP_KEY + " AS " + PhoneLookup.LOOKUP_KEY);
		sPhoneLookupProjectionMap.put(PhoneLookup.DISPLAY_NAME,
				"contacts_view." + Contacts.DISPLAY_NAME + " AS "
						+ PhoneLookup.DISPLAY_NAME);
		sPhoneLookupProjectionMap.put(PhoneLookup.LAST_TIME_CONTACTED,
				"contacts_view." + Contacts.LAST_TIME_CONTACTED + " AS "
						+ PhoneLookup.LAST_TIME_CONTACTED);
		sPhoneLookupProjectionMap.put(PhoneLookup.TIMES_CONTACTED,
				"contacts_view." + Contacts.TIMES_CONTACTED + " AS "
						+ PhoneLookup.TIMES_CONTACTED);
		sPhoneLookupProjectionMap.put(PhoneLookup.STARRED, "contacts_view."
				+ Contacts.STARRED + " AS " + PhoneLookup.STARRED);
		sPhoneLookupProjectionMap.put(PhoneLookup.IN_VISIBLE_GROUP,
				"contacts_view." + Contacts.IN_VISIBLE_GROUP + " AS "
						+ PhoneLookup.IN_VISIBLE_GROUP);
		sPhoneLookupProjectionMap.put(PhoneLookup.PHOTO_ID, "contacts_view."
				+ Contacts.PHOTO_ID + " AS " + PhoneLookup.PHOTO_ID);
		sPhoneLookupProjectionMap.put(PhoneLookup.CUSTOM_RINGTONE,
				"contacts_view." + Contacts.CUSTOM_RINGTONE + " AS "
						+ PhoneLookup.CUSTOM_RINGTONE);
		sPhoneLookupProjectionMap.put(PhoneLookup.HAS_PHONE_NUMBER,
				"contacts_view." + Contacts.HAS_PHONE_NUMBER + " AS "
						+ PhoneLookup.HAS_PHONE_NUMBER);
		sPhoneLookupProjectionMap.put(PhoneLookup.SEND_TO_VOICEMAIL,
				"contacts_view." + Contacts.SEND_TO_VOICEMAIL + " AS "
						+ PhoneLookup.SEND_TO_VOICEMAIL);
		sPhoneLookupProjectionMap.put(PhoneLookup.NUMBER, Phone.NUMBER + " AS "
				+ PhoneLookup.NUMBER);
		sPhoneLookupProjectionMap.put(PhoneLookup.TYPE, Phone.TYPE + " AS "
				+ PhoneLookup.TYPE);
		sPhoneLookupProjectionMap.put(PhoneLookup.LABEL, Phone.LABEL + " AS "
				+ PhoneLookup.LABEL);
		...
		columns.put(TGroups.RES_PACKAGE, TGroups.RES_PACKAGE);
		columns.put(Groups.TITLE, Groups.TITLE);
		columns.put(TGroups.TITLE_RES, TGroups.TITLE_RES);
		...
		columns.put(Groups.SUMMARY_COUNT, "(SELECT COUNT(DISTINCT "
				+ ContactsColumns.CONCRETE_ID + ") FROM "
				+ Tables.DATA_JOIN_MIMETYPES_RAW_CONTACTS_CONTACTS + " WHERE "
				+ Clauses.MIMETYPE_IS_GROUP_MEMBERSHIP + " AND "
				+ Clauses.BELONGS_TO_GROUP + ") AS " + Groups.SUMMARY_COUNT);
		columns.put(Groups.SUMMARY_WITH_PHONES, "(SELECT COUNT(DISTINCT "
				+ ContactsColumns.CONCRETE_ID + ") FROM "
				+ Tables.DATA_JOIN_MIMETYPES_RAW_CONTACTS_CONTACTS + " WHERE "
				+ Clauses.MIMETYPE_IS_GROUP_MEMBERSHIP + " AND "
				+ Clauses.BELONGS_TO_GROUP + " AND "
				+ Contacts.HAS_PHONE_NUMBER + ") AS "
				+ Groups.SUMMARY_WITH_PHONES);
		sGroupsSummaryProjectionMap = columns;

		// Aggregate exception projection map
		columns = new HashMap<String, String>();
		columns.put(AggregationExceptionColumns._ID,
				Tables.AGGREGATION_EXCEPTIONS + "._id AS _id");
		columns.put(AggregationExceptions.TYPE, AggregationExceptions.TYPE);
		columns.put(AggregationExceptions.RAW_CONTACT_ID1,
				AggregationExceptions.RAW_CONTACT_ID1);
		columns.put(AggregationExceptions.RAW_CONTACT_ID2,
		...
		columns.put(Settings.ANY_UNSYNCED, "(CASE WHEN MIN("
				+ Settings.SHOULD_SYNC + ",(SELECT (CASE WHEN MIN("
				+ Groups.SHOULD_SYNC + ") IS NULL THEN 1 ELSE MIN("
				...
				+ "))=0 THEN 1 ELSE 0 END) AS " + Settings.ANY_UNSYNCED);
		columns.put(
				Settings.UNGROUPED_COUNT,
				"(SELECT COUNT(*) FROM (SELECT 1 FROM "
						+ Tables.SETTINGS_JOIN_RAW_CONTACTS_DATA_MIMETYPES_CONTACTS
		...
		columns.put(
				...
				"(SELECT COUNT(*) FROM (SELECT 1 FROM "
						+ Tables.SETTINGS_JOIN_RAW_CONTACTS_DATA_MIMETYPES_CONTACTS
						+ " WHERE " + Contacts.HAS_PHONE_NUMBER + " GROUP BY "
						+ Clauses.GROUP_BY_ACCOUNT_CONTACT_ID + " HAVING "
						+ Clauses.HAVING_NO_GROUPS + ")) AS "
						+ Settings.UNGROUPED_WITH_PHONES);
		sSettingsProjectionMap = columns;

		columns = new HashMap<String, String>();
		columns.put(PresenceColumns.RAW_CONTACT_ID,
				PresenceColumns.RAW_CONTACT_ID);
		columns.put(StatusUpdates.DATA_ID, DataColumns.CONCRETE_ID + " AS "
				+ StatusUpdates.DATA_ID);
		...
		// We cannot allow a null in the custom protocol field, because SQLite3
		// does not
		// properly enforce uniqueness of null values
		columns.put(StatusUpdates.CUSTOM_PROTOCOL, "(CASE WHEN "
				+ StatusUpdates.CUSTOM_PROTOCOL + "='' THEN NULL ELSE "
				+ StatusUpdates.CUSTOM_PROTOCOL + " END) AS "
		...
		columns.put(StatusUpdates.STATUS_RES_PACKAGE,
		...
		sLiveFoldersProjectionMap.put(LiveFolders._ID, Contacts._ID + " AS "
				+ LiveFolders._ID);
		sLiveFoldersProjectionMap.put(LiveFolders.NAME, Contacts.DISPLAY_NAME
				+ " AS " + LiveFolders.NAME);
		// TODO: Put contact photo back when we have a way to display a default
		// icon
	...
	private static void addProjection(HashMap<String, String> map,
	...
	private abstract class DataRowHandler {
		...
		public DataRowHandler(String mimetype) {
			...
			// To ensure the data column position. This is dead code if properly
			// configured.
			if (StructuredName.DISPLAY_NAME != Data.DATA1
					|| Nickname.NAME != Data.DATA1
					|| Organization.COMPANY != Data.DATA1
					|| Phone.NUMBER != Data.DATA1 || Email.DATA != Data.DATA1) {
				throw new AssertionError(
						"Some of ContactsContract.CommonDataKinds class primary"
		 ...
		 */
		public long insert(SQLiteDatabase db, long rawContactId,
		 ...
		 */
		public boolean update(SQLiteDatabase db, ContentValues values,
			...
			if (values.containsKey(Data.IS_SUPER_PRIMARY)) {
				...
				// Now that we've taken care of setting these, remove them from
				// "values".
				...
				// Now that we've taken care of setting this, remove it from
				// "values".
				values.remove(Data.IS_PRIMARY);
			}
			...
			if (values.size() > 0) {
				...
				mDb.update(Tables.DATA, values, Data._ID + " =?",
						mSelectionArgs1);
			}
		...
		public int delete(SQLiteDatabase db, Cursor c) {
			...
			mSelectionArgs1[0] = String.valueOf(dataId);
			int count = db
					.delete(Tables.DATA, Data._ID + "=?", mSelectionArgs1);
		...
		private void fixPrimary(SQLiteDatabase db, long rawContactId) {
			...
			Cursor c = db.query(DataDeleteQuery.TABLE,
					DataDeleteQuery.CONCRETE_COLUMNS, Data.RAW_CONTACT_ID
							+ "=?" + " AND " + DataColumns.MIMETYPE_ID + "="
							+ mimeTypeId, mSelectionArgs1, null, null, null);
			try {
				while (c.moveToNext()) {
					long dataId = c.getLong(DataDeleteQuery._ID);
					int type = c.getInt(DataDeleteQuery.DATA1);
					if (primaryType == -1
							|| getTypeRank(type) < getTypeRank(primaryType)) {
						primaryId = dataId;
						primaryType = type;
					}
				}
		...
		/**
		 * Returns the rank of a specific record type to be used in determining
		 * the primary row. Lower number represents higher priority.
		 */
		protected int getTypeRank(int type) {
			return 0;
		}

		protected void fixRawContactDisplayName(SQLiteDatabase db,
				long rawContactId) {
			if (!isNewRawContact(rawContactId)) {
				updateRawContactDisplayName(db, rawContactId);
				mContactAggregator.updateDisplayNameForRawContact(db,
						rawContactId);
			}
		}
				...
				if (cursor.moveToFirst()) {
					for (int i = 0; i < cursor.getColumnCount(); i++) {
						...
						if (!changing && update.containsKey(key)) {
							...
							String newString = newValue == null ? null
									: newValue.toString();
	...
	public class StructuredNameRowHandler extends DataRowHandler {
		...
		public long insert(SQLiteDatabase db, long rawContactId,
			...
			Integer fullNameStyle = values
					.getAsInteger(TStructuredName.FULL_NAME_STYLE);
			insertNameLookupForStructuredName(
					rawContactId,
					dataId,
					name,
					fullNameStyle != null ? mNameSplitter
							.getAdjustedFullNameStyle(fullNameStyle)
							: TFullNameStyle.UNDEFINED);
		...
		public boolean update(SQLiteDatabase db, ContentValues values,
			...
			final ContentValues augmented = getAugmentedValues(db, dataId,
					values);
		...
		private final String[] STRUCTURED_FIELDS = new String[] {
				StructuredName.PREFIX, StructuredName.GIVEN_NAME,
		 ...
		 * name.
		 */
		private void fixStructuredNameComponents(ContentValues augmented,
				ContentValues update) {
			final String unstruct = update
					.getAsString(StructuredName.DISPLAY_NAME);

			final boolean touchedUnstruct = !TextUtils.isEmpty(unstruct);
			final boolean touchedStruct = !areAllEmpty(update,
					STRUCTURED_FIELDS);
			...
			if (touchedUnstruct && !touchedStruct) {
				...
				name.toValues(update);
					...
					&& (touchedStruct || areAnySpecified(update,
							STRUCTURED_FIELDS))) {
				// We need to update the display name when any structured
				// components
				// are specified, even when they are null, which is why we are
				// checking
				// areAnySpecified. The touchedStruct in the condition is an
				// optimization:
				// if there are non-null values, we know for a fact that some
				// values are present.
				NameSplitter.Name name = new NameSplitter.Name();
				name.fromValues(augmented);
				// As the name could be changed, let's guess the name style
				// again.
				name.fullNameStyle = TFullNameStyle.UNDEFINED;
				mSplitter.guessNameStyle(name);
				int unadjustedFullNameStyle = name.fullNameStyle;
				name.fullNameStyle = mSplitter
						.getAdjustedFullNameStyle(name.fullNameStyle);
				final String joined = mSplitter.join(name, true);
				update.put(StructuredName.DISPLAY_NAME, joined);

				update.put(TStructuredName.FULL_NAME_STYLE,
						unadjustedFullNameStyle);
				update.put(TStructuredName.PHONETIC_NAME_STYLE,
						name.phoneticNameStyle);
			} else if (touchedUnstruct && touchedStruct) {
				if (!update.containsKey(TStructuredName.FULL_NAME_STYLE)) {
					update.put(TStructuredName.FULL_NAME_STYLE,
							mSplitter.guessFullNameStyle(unstruct));
				}
				if (!update.containsKey(TStructuredName.PHONETIC_NAME_STYLE)) {
					update.put(TStructuredName.PHONETIC_NAME_STYLE,
	...
	public class StructuredPostalRowHandler extends DataRowHandler {
		 ...
		 */
		private final String[] STRUCTURED_FIELDS = new String[] {
				StructuredPostal.STREET, StructuredPostal.POBOX,
				StructuredPostal.NEIGHBORHOOD, StructuredPostal.CITY,
		 ...
		 */
		private void fixStructuredPostalComponents(ContentValues augmented,
				ContentValues update) {
			final String unstruct = update
	...
	public class CommonDataRowHandler extends DataRowHandler {
		...
		public CommonDataRowHandler(String mimetype, String typeColumn,
				String labelColumn) {
			super(mimetype);
		...
		private void enforceTypeAndLabel(ContentValues augmented,
				ContentValues update) {
			final boolean hasType = !TextUtils.isEmpty(augmented
					.getAsString(mTypeColumn));
			final boolean hasLabel = !TextUtils.isEmpty(augmented
					.getAsString(mLabelColumn));

			if (hasLabel && !hasType) {
				// When label exists, assert that some type is defined
				throw new IllegalArgumentException(mTypeColumn
						+ " must be specified when " + mLabelColumn
						+ " is defined.");
			}
		}
	}
	...
	public class OrganizationDataRowHandler extends CommonDataRowHandler {
		...
		public OrganizationDataRowHandler() {
			super(Organization.CONTENT_ITEM_TYPE, Organization.TYPE,
					Organization.LABEL);
		}
		...
		public long insert(SQLiteDatabase db, long rawContactId,
			...
			fixRawContactDisplayName(db, rawContactId);
			insertNameLookupForOrganization(rawContactId, dataId, company,
					title);
			return dataId;
		}
		...
		public boolean update(SQLiteDatabase db, ContentValues values,
			...
			if (containsCompany || containsTitle) {
				...
				if (containsCompany) {
					...
					mSelectionArgs1[0] = String.valueOf(dataId);
					company = DatabaseUtils.stringForQuery(db, "SELECT "
							+ Organization.COMPANY + " FROM " + Tables.DATA
				...
				if (containsTitle) {
					...
					mSelectionArgs1[0] = String.valueOf(dataId);
					title = DatabaseUtils.stringForQuery(db, "SELECT "
							+ Organization.TITLE + " FROM " + Tables.DATA
							+ " WHERE " + Data._ID + "=?", mSelectionArgs1);
				}
	...
	public class EmailDataRowHandler extends CommonDataRowHandler {
		...
		public long insert(SQLiteDatabase db, long rawContactId,
			...
			fixRawContactDisplayName(db, rawContactId);
			String address = insertNameLookupForEmail(rawContactId, dataId,
					email);
			if (address != null) {
				triggerAggregation(rawContactId);
			}
	...
	public class PhoneDataRowHandler extends CommonDataRowHandler {
		...
		public long insert(SQLiteDatabase db, long rawContactId,
			...
			if (values.containsKey(Phone.NUMBER)) {
				...
				updatePhoneLookup(db, rawContactId, dataId, number,
						normalizedNumber);
		...
		private void updatePhoneLookup(SQLiteDatabase db, long rawContactId,
				long dataId, String number, String normalizedNumber) {
			if (number != null) {
				ContentValues phoneValues = new ContentValues();
				phoneValues
						.put(PhoneLookupColumns.RAW_CONTACT_ID, rawContactId);
				...
				mSelectionArgs1[0] = String.valueOf(dataId);
				db.delete(Tables.PHONE_LOOKUP, PhoneLookupColumns.DATA_ID
						+ "=?", mSelectionArgs1);
			}
		}
	...
	public class GroupMembershipRowHandler extends DataRowHandler {
		...
		private void resolveGroupSourceIdInValues(long rawContactId,
				SQLiteDatabase db, ContentValues values, boolean isInsert) {
			boolean containsGroupSourceId = values
					.containsKey(GroupMembership.GROUP_SOURCE_ID);
			boolean containsGroupId = values
					.containsKey(GroupMembership.GROUP_ROW_ID);
			...
			if (containsGroupSourceId) {
				final String sourceId = values
						.getAsString(GroupMembership.GROUP_SOURCE_ID);
	...
	/**
	 * An entry in group id cache. It maps the combination of (account type,
	 * account name and source id) to group row id.
	 */
	...
	// We don't need a soft cache for groups - the assumption is that there will
	// only
	// be a small number of contact groups. The cache is keyed off source id.
	// The value
	// is a list of groups with this group id.
	private HashMap<String, ArrayList<GroupIdCacheEntry>> mGroupIdCache = Maps
			.newHashMap();
	...
	private int mProviderStatus = TProviderStatus.STATUS_NORMAL;
	...
	private boolean initialize() {
		...
		mLegacyApiSupport = new LegacyApiSupport(context, mDbHelper, this,
				mGlobalSearchSupport);
		mContactAggregator = new ContactAggregator(this, mDbHelper,
				createPhotoPriorityResolver(context));
		mContactAggregator.setEnabled(SystemProperties.getBoolean(
		...
		mSetPrimaryStatement = mDb.compileStatement("UPDATE " + Tables.DATA
				+ " SET " + Data.IS_PRIMARY + "=(_id=?)" + " WHERE "
				+ DataColumns.MIMETYPE_ID + "=?" + "   AND "
				+ Data.RAW_CONTACT_ID + "=?");

		mSetSuperPrimaryStatement = mDb.compileStatement("UPDATE "
				+ Tables.DATA + " SET " + Data.IS_SUPER_PRIMARY + "=("
				+ Data._ID + "=?)" + " WHERE " + DataColumns.MIMETYPE_ID + "=?"
				+ "   AND " + Data.RAW_CONTACT_ID + " IN (" + "SELECT "
				+ RawContacts._ID + " FROM " + Tables.RAW_CONTACTS + " WHERE "
				+ RawContacts.CONTACT_ID + " =(" + "SELECT "
				+ RawContacts.CONTACT_ID + " FROM " + Tables.RAW_CONTACTS
				+ " WHERE " + RawContacts._ID + "=?))");

		mRawContactDisplayNameUpdate = mDb.compileStatement("UPDATE "
				+ Tables.RAW_CONTACTS + " SET "
				+ TRawContacts.DISPLAY_NAME_SOURCE + "=?,"
				+ TRawContacts.DISPLAY_NAME_PRIMARY + "=?,"
				+ TRawContacts.DISPLAY_NAME_ALTERNATIVE + "=?,"
				+ TRawContacts.PHONETIC_NAME + "=?,"
				+ TRawContacts.PHONETIC_NAME_STYLE + "=?,"
				+ TRawContacts.SORT_KEY_PRIMARY + "=?,"
				+ TRawContacts.SORT_KEY_ALTERNATIVE + "=?" + " WHERE "
				+ RawContacts._ID + "=?");

		mLastStatusUpdate = mDb.compileStatement("UPDATE " + Tables.CONTACTS
				+ " SET " + ContactsColumns.LAST_STATUS_UPDATE_ID + "="
				+ "(SELECT " + DataColumns.CONCRETE_ID + " FROM "
				+ Tables.STATUS_UPDATES + " JOIN " + Tables.DATA + "   ON ("
				+ StatusUpdatesColumns.DATA_ID + "=" + DataColumns.CONCRETE_ID
				+ ")" + " JOIN " + Tables.RAW_CONTACTS + "   ON ("
				+ DataColumns.CONCRETE_RAW_CONTACT_ID + "="
				+ RawContactsColumns.CONCRETE_ID + ")" + " WHERE "
				+ RawContacts.CONTACT_ID + "=?" + " ORDER BY "
				+ StatusUpdates.STATUS_TIMESTAMP + " DESC,"
				+ StatusUpdates.STATUS + " LIMIT 1)" + " WHERE "
				+ ContactsColumns.CONCRETE_ID + "=?");

		mNameLookupInsert = mDb.compileStatement("INSERT OR IGNORE INTO "
		...
		mNameLookupDelete = mDb.compileStatement("DELETE FROM "
				+ Tables.NAME_LOOKUP + " WHERE " + NameLookupColumns.DATA_ID
				+ "=?");

		mStatusUpdateInsert = mDb.compileStatement("INSERT INTO "
				+ Tables.STATUS_UPDATES + "(" + StatusUpdatesColumns.DATA_ID
				+ ", " + StatusUpdates.STATUS + ","
				+ StatusUpdates.STATUS_RES_PACKAGE + ","
				+ StatusUpdates.STATUS_ICON + "," + StatusUpdates.STATUS_LABEL
				+ ")" + " VALUES (?,?,?,?,?)");

		mStatusUpdateReplace = mDb.compileStatement("INSERT OR REPLACE INTO "
				+ Tables.STATUS_UPDATES + "(" + StatusUpdatesColumns.DATA_ID
				+ ", " + StatusUpdates.STATUS_TIMESTAMP + ","
				+ StatusUpdates.STATUS + "," + StatusUpdates.STATUS_RES_PACKAGE
				+ "," + StatusUpdates.STATUS_ICON + ","
				+ StatusUpdates.STATUS_LABEL + ")" + " VALUES (?,?,?,?,?,?)");

		mStatusUpdateAutoTimestamp = mDb.compileStatement("UPDATE "
				+ Tables.STATUS_UPDATES + " SET "
				+ StatusUpdates.STATUS_TIMESTAMP + "=?," + StatusUpdates.STATUS
				+ "=?" + " WHERE " + StatusUpdatesColumns.DATA_ID + "=?"
				+ " AND " + StatusUpdates.STATUS + "!=?");

		mStatusAttributionUpdate = mDb.compileStatement("UPDATE "
				+ Tables.STATUS_UPDATES + " SET "
				+ StatusUpdates.STATUS_RES_PACKAGE + "=?,"
				+ StatusUpdates.STATUS_ICON + "=?,"
				+ StatusUpdates.STATUS_LABEL + "=?" + " WHERE "
				+ StatusUpdatesColumns.DATA_ID + "=?");

		mStatusUpdateDelete = mDb.compileStatement("DELETE FROM "
				+ Tables.STATUS_UPDATES + " WHERE "
				+ StatusUpdatesColumns.DATA_ID + "=?");

		// When setting NAME_VERIFIED to 1 on a raw contact, reset it to 0
		// on all other raw contacts in the same aggregate
		mResetNameVerifiedForOtherRawContacts = mDb.compileStatement("UPDATE "
				+ Tables.RAW_CONTACTS + " SET " + TRawContacts.NAME_VERIFIED
				+ "=0" + " WHERE " + RawContacts.CONTACT_ID + "=(" + "SELECT "
				+ RawContacts.CONTACT_ID + " FROM " + Tables.RAW_CONTACTS
				+ " WHERE " + RawContacts._ID + "=?)" + " AND "
				+ RawContacts._ID + "!=?");
		...
		mMimeTypeIdStructuredName = mDbHelper
				.getMimeTypeId(StructuredName.CONTENT_ITEM_TYPE);
		mMimeTypeIdOrganization = mDbHelper
				.getMimeTypeId(Organization.CONTENT_ITEM_TYPE);
		mMimeTypeIdNickname = mDbHelper
				.getMimeTypeId(Nickname.CONTENT_ITEM_TYPE);
	...
	private void initDataRowHandlers() {
		...
		mDataRowHandlers.put(Im.CONTENT_ITEM_TYPE, new CommonDataRowHandler(
				Im.CONTENT_ITEM_TYPE, Im.TYPE, Im.LABEL));
		mDataRowHandlers.put(Nickname.CONTENT_ITEM_TYPE,
				new CommonDataRowHandler(StructuredPostal.CONTENT_ITEM_TYPE,
						StructuredPostal.TYPE, StructuredPostal.LABEL));
		mDataRowHandlers.put(Organization.CONTENT_ITEM_TYPE,
				new OrganizationDataRowHandler());
		mDataRowHandlers
				.put(Phone.CONTENT_ITEM_TYPE, new PhoneDataRowHandler());
		mDataRowHandlers.put(Nickname.CONTENT_ITEM_TYPE,
				new NicknameDataRowHandler());
		...
		mDataRowHandlers
				.put(Photo.CONTENT_ITEM_TYPE, new PhotoDataRowHandler());
	}

	/**
	 * Visible for testing.
	 */
	/* package */PhotoPriorityResolver createPhotoPriorityResolver(
			Context context) {
	...
	private void initForDefaultLocale() {
		...
		mCommonNicknameCache = new CommonNicknameCache(
	...
	public void onConfigurationChanged(Configuration newConfig) {
		if (mProviderStatus != TProviderStatus.STATUS_NORMAL) {
			return;
		}
	...
	/**
	 * Verifies that the contacts database is properly configured for the
	 * current locale. If not, changes the database locale to the current locale
	 * using an asynchronous task. This needs to be done asynchronously because
	 * the process involves rebuilding large data structures (name lookup, sort
	 * keys), which can take minutes on a large set of contacts.
	 */
	protected void verifyLocale() {

		// The process is already running - postpone the change
		if (mProviderStatus == TProviderStatus.STATUS_CHANGING_LOCALE) {
			return;
		}
		...
		setProviderStatus(TProviderStatus.STATUS_CHANGING_LOCALE);
		...
		AsyncTask<Integer, Void, Void> task = new AsyncTask<Integer, Void, Void>() {
			...
			protected void onPostExecute(Void result) {
				prefs.edit().putString(PREF_LOCALE, currentLocale.toString())
						.commit();
				setProviderStatus(savedProviderStatus);

				// Recursive invocation, needed to cover the case where locale
				// changes once and then changes again before the db upgrade is
				// completed.
				verifyLocale();
			}
		};
	...
	protected boolean isLegacyContactImportNeeded() {
		int version = Integer.parseInt(mDbHelper.getProperty(
				PROPERTY_CONTACTS_IMPORTED, "0"));
	...
	/**
	 * Imports legacy contacts in a separate thread. As long as the import
	 * process is running all other access to the contacts is blocked.
	 */
	private void importLegacyContactsAsync() {
		Log.v(TAG, "Importing legacy contacts");
		setProviderStatus(TProviderStatus.STATUS_UPGRADING);
		...
		Thread importThread = new Thread("LegacyContactImport") {
			...
			public void run() {
				...
				prefs.edit().putString(PREF_LOCALE, mCurrentLocale.toString())
						.commit();
	...
	private void onLegacyContactImportSuccess() {
		...
		// Store a property in the database indicating that the conversion
		// process succeeded
		mDbHelper.setProperty(PROPERTY_CONTACTS_IMPORTED,
				String.valueOf(PROPERTY_CONTACTS_IMPORT_VERSION));
		setProviderStatus(TProviderStatus.STATUS_NORMAL);
	...
	private void onLegacyContactImportFailure() {
		...
		n.setLatestEventInfo(
				...
				PendingIntent.getActivity(context, 0, new Intent(
						TContactsContract.Intents.UI.LIST_DEFAULT), 0));
		n.flags |= Notification.FLAG_NO_CLEAR | Notification.FLAG_ONGOING_EVENT;

		nm.notify(LEGACY_IMPORT_FAILED_NOTIFICATION, n);

		setProviderStatus(TProviderStatus.STATUS_UPGRADE_OUT_OF_MEMORY);
	...
	/* package */boolean importLegacyContacts(LegacyContactImporter importer) {
		...
		mEstimatedStorageRequirement = importer
				.getEstimatedStorageRequirement();
		return false;
	}
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		if (mAccessLatch != null) {
			...
			if (match == PROVIDER_STATUS && isLegacyContactImportNeeded()) {
				Integer newStatus = values.getAsInteger(TProviderStatus.STATUS);
				if (newStatus != null
						&& newStatus == TProviderStatus.STATUS_UPGRADING) {
					importLegacyContactsAsync();
					return 1;
				} else {
					return 0;
	...
	//add by chenqiang
	public boolean mNetworkNotified = false;
	@Override
	protected void onBeginTransaction() {
		...
		Log.d(TAG, "onBeginTransaction " + mNetworkNotified);
	}
	...
	protected void notifyChange() {
		...
		in.setAction("cn.tsinghua.hpc.TSYNC_REQUEST.CONTACTS");
		this.getContext().sendBroadcast(in);
		mSyncToNetwork = false;
	}

	protected void notifyChange(boolean syncToNetwork) {
		getContext().getContentResolver().notifyChange(
				TContactsContract.AUTHORITY_URI, null, syncToNetwork);
		mNetworkNotified = syncToNetwork;
		Log.d(TAG, "notifyChange " + mNetworkNotified);
	}

	protected void setProviderStatus(int status) {
		mProviderStatus = status;
		getContext().getContentResolver().notifyChange(
				TContactsContract.TProviderStatus.CONTENT_URI, null, false);
	}
	...
	protected Uri insertInTransaction(Uri uri, ContentValues values) {
		...
		final boolean callerIsSyncAdapter = readBooleanQueryParameter(uri,
				ContactsContract.CALLER_IS_SYNCADAPTER, false);
	 ...
	 * 
	 * @param uri
	 *            Current {@link Uri} being operated on.
	 * @param values
	 *            {@link ContentValues} to read and possibly update.
	 * @throws IllegalArgumentException
	 *             when only one of {@link RawContacts#ACCOUNT_NAME} or
	 *             {@link RawContacts#ACCOUNT_TYPE} is specified, leaving the
	 *             other undefined.
	 * @throws IllegalArgumentException
	 *             when {@link RawContacts#ACCOUNT_NAME} and
	 *             {@link RawContacts#ACCOUNT_TYPE} are inconsistent between the
	 *             given {@link Uri} and {@link ContentValues}.
	 */
	private Account resolveAccount(Uri uri, ContentValues values)
		...
		final boolean partialUri = TextUtils.isEmpty(accountName)
		...
		if (validValues && validUri) {
			...
			final boolean accountMatch = TextUtils.equals(accountName,
					valueAccountName)
	...
	private long insertContact(ContentValues values) {
		throw new UnsupportedOperationException(
				"Aggregate contacts are created automatically");
	}
	 ...
	 *            the values for the new row
	 * @param values
	 *            the account this contact should be associated with. may be
	 *            null.
	 * @return the row ID of the newly created row
	 */
	private long insertRawContact(Uri uri, ContentValues values) {
		...
		long rawContactId = mDb.insert(Tables.RAW_CONTACTS,
				RawContacts.CONTACT_ID, mValues);
		int aggregationMode = RawContacts.AGGREGATION_MODE_DEFAULT;
		if (mValues.containsKey(RawContacts.AGGREGATION_MODE)) {
			aggregationMode = mValues
					.getAsInteger(RawContacts.AGGREGATION_MODE);
		}
		mContactAggregator.markNewForAggregation(rawContactId, aggregationMode);

		// Trigger creation of a Contact based on this RawContact at the end of
		// transaction
	...
	private long insertData(ContentValues values, boolean callerIsSyncAdapter) {
		...
		final String packageName = mValues.getAsString(TData.RES_PACKAGE);
		if (packageName != null) {
			mValues.put(DataColumns.PACKAGE_ID,
					mDbHelper.getPackageId(packageName));
		}
		mValues.remove(TData.RES_PACKAGE);
	 ...
	 * rawContactId. If the group doesn't already exist then it is first
	 * created,
	 * 
	 * @param db
	 *            SQLiteDatabase to use for this operation
	 * @param rawContactId
	 *            the contact this group is associated with
	 * @param sourceId
	 *            the sourceIf of the group to query or create
	 * @return the group id of the existing or created group
	 * @throws IllegalArgumentException
	 *             if the contact is not associated with an account
	 * @throws IllegalStateException
	 *             if a group needs to be created but the creation failed
	 */
	private long getOrMakeGroup(SQLiteDatabase db, long rawContactId,
			String sourceId, Account account) {

		if (account == null) {
			mSelectionArgs1[0] = String.valueOf(rawContactId);
			Cursor c = db.query(RawContactsQuery.TABLE,
					RawContactsQuery.COLUMNS, RawContacts._ID + "=?",
					mSelectionArgs1, null, null, null);
			try {
				if (c.moveToFirst()) {
					String accountName = c
							.getString(RawContactsQuery.ACCOUNT_NAME);
					String accountType = c
							.getString(RawContactsQuery.ACCOUNT_TYPE);
		...
		for (int i = 0; i < count; i++) {
			...
			if (entry.accountName.equals(account.name)
					&& entry.accountType.equals(account.type)) {
		...
		// look up the group that contains this sourceId and has the same
		// account name and type
		// as the contact refered to by rawContactId
		Cursor c = db.query(Tables.GROUPS, new String[] { RawContacts._ID },
				Clauses.GROUP_HAS_ACCOUNT_AND_SOURCE_ID, new String[] {
						sourceId, account.name, account.type }, null, null,
				null);
		try {
			if (c.moveToFirst()) {
				...
				long groupId = db.insert(Tables.GROUPS, Groups.ACCOUNT_NAME,
						groupValues);
				if (groupId < 0) {
					throw new IllegalStateException(
							"unable to create a new group with "
	...
	/**
	 * Updates a raw contact display name based on data rows, e.g. structured
	 * name, organization, email etc.
	 */
	public void updateRawContactDisplayName(SQLiteDatabase db, long rawContactId) {
		int bestDisplayNameSource = TDisplayNameSources.UNDEFINED;
		NameSplitter.Name bestName = null;
		String bestDisplayName = null;
		String bestPhoneticName = null;
		int bestPhoneticNameStyle = TPhoneticNameStyle.UNDEFINED;
		...
		try {
			while (c.moveToNext()) {
				...
				if (source < bestDisplayNameSource
						|| source == TDisplayNameSources.UNDEFINED) {
					continue;
				}
				...
				if (mimeType == mMimeTypeIdStructuredName) {
					...
					name.fullNameStyle = c
							.isNull(DisplayNameQuery.FULL_NAME_STYLE) ? TFullNameStyle.UNDEFINED
							: c.getInt(DisplayNameQuery.FULL_NAME_STYLE);
					name.phoneticFamilyName = c
							.getString(DisplayNameQuery.PHONETIC_FAMILY_NAME);
					name.phoneticMiddleName = c
							.getString(DisplayNameQuery.PHONETIC_MIDDLE_NAME);
					name.phoneticGivenName = c
							.getString(DisplayNameQuery.PHONETIC_GIVEN_NAME);
					name.phoneticNameStyle = c
							.isNull(DisplayNameQuery.PHONETIC_NAME_STYLE) ? TPhoneticNameStyle.UNDEFINED
					...
					c.copyStringToBuffer(DisplayNameQuery.DATA1,
							mCharArrayBuffer);
					if (mCharArrayBuffer.sizeCopied != 0) {
						...
						bestPhoneticName = c
								.getString(DisplayNameQuery.ORGANIZATION_PHONETIC_NAME);
						bestPhoneticNameStyle = c
								.isNull(DisplayNameQuery.ORGANIZATION_PHONETIC_NAME_STYLE) ? TPhoneticNameStyle.UNDEFINED
								: c.getInt(DisplayNameQuery.ORGANIZATION_PHONETIC_NAME_STYLE);
					} else {
						c.copyStringToBuffer(DisplayNameQuery.TITLE,
								mCharArrayBuffer);
						if (mCharArrayBuffer.sizeCopied != 0) {
							bestDisplayNameSource = source;
							bestDisplayName = new String(mCharArrayBuffer.data,
									0, mCharArrayBuffer.sizeCopied);
							bestPhoneticName = null;
							bestPhoneticNameStyle = TPhoneticNameStyle.UNDEFINED;
						}
					}
				} else {
		...
		int displayNameStyle = TFullNameStyle.UNDEFINED;

		if (bestDisplayNameSource == TDisplayNameSources.STRUCTURED_NAME) {
			displayNameStyle = bestName.fullNameStyle;
			if (displayNameStyle == TFullNameStyle.CJK
					|| displayNameStyle == TFullNameStyle.UNDEFINED) {
				displayNameStyle = mNameSplitter
						.getAdjustedFullNameStyle(displayNameStyle);
		...
		if (bestPhoneticName != null) {
			...
			if (bestPhoneticNameStyle == TPhoneticNameStyle.UNDEFINED) {
				bestPhoneticNameStyle = mNameSplitter
						.guessPhoneticNameStyle(bestPhoneticName);
			}
		} else {
			if (displayNameStyle == TFullNameStyle.UNDEFINED) {
				displayNameStyle = mNameSplitter
						.guessFullNameStyle(bestDisplayName);
				if (displayNameStyle == TFullNameStyle.UNDEFINED
						|| displayNameStyle == TFullNameStyle.CJK) {
					displayNameStyle = mNameSplitter
							.getAdjustedNameStyleBasedOnPhoneticNameStyle(
									displayNameStyle, bestPhoneticNameStyle);
				}
				displayNameStyle = mNameSplitter
						.getAdjustedFullNameStyle(displayNameStyle);
			}
			if (displayNameStyle == TFullNameStyle.CHINESE
					|| displayNameStyle == TFullNameStyle.CJK) {
		...
		setDisplayName(rawContactId, bestDisplayNameSource, displayNamePrimary,
				...
				bestPhoneticNameStyle, sortKeyPrimary, sortKeyAlternative);
	}

	private int getDisplayNameSource(int mimeTypeId) {
		if (mimeTypeId == mMimeTypeIdStructuredName) {
			return TDisplayNameSources.STRUCTURED_NAME;
		} else if (mimeTypeId == mMimeTypeIdEmail) {
			return TDisplayNameSources.EMAIL;
		} else if (mimeTypeId == mMimeTypeIdPhone) {
			return TDisplayNameSources.PHONE;
		} else if (mimeTypeId == mMimeTypeIdOrganization) {
			return TDisplayNameSources.ORGANIZATION;
		} else if (mimeTypeId == mMimeTypeIdNickname) {
			return TDisplayNameSources.NICKNAME;
		} else {
			return TDisplayNameSources.UNDEFINED;
		}
	}
	...
	private int deleteData(String selection, String[] selectionArgs,
		...
		// Note that the query will return data according to the access
		// restrictions,
		// so we don't need to worry about deleting data we don't have
		// permission to read.
		Cursor c = query(TData.CONTENT_URI, DataDeleteQuery.COLUMNS, selection,
				selectionArgs, null);
		try {
			while (c.moveToNext()) {
				long rawContactId = c.getLong(DataDeleteQuery.RAW_CONTACT_ID);
	...
	public int deleteData(long dataId, String[] allowedMimeTypes) {
		...
		// permission to read.
		mSelectionArgs1[0] = String.valueOf(dataId);
		Cursor c = query(TData.CONTENT_URI, DataDeleteQuery.COLUMNS, Data._ID
	 ...
	 */
	private long insertGroup(Uri uri, ContentValues values,
			boolean callerIsSyncAdapter) {
		mValues.clear();
		mValues.putAll(values);
		...
		// Replace package with internal mapping
		final String packageName = mValues.getAsString(TGroups.RES_PACKAGE);
		if (packageName != null) {
			mValues.put(GroupsColumns.PACKAGE_ID,
					mDbHelper.getPackageId(packageName));
		}
		mValues.remove(TGroups.RES_PACKAGE);
	...
	public long insertStatusUpdate(ContentValues values) {
		...
		if (dataId != null) {
			...
			if (TextUtils.isEmpty(handle) || protocol == null) {
				throw new IllegalArgumentException(
						"PROTOCOL and IM_HANDLE are required");
			}
			...
			if (matchEmail) {
				...
				// The following hack forces SQLite to use the
				// (mimetype_id,data1) index, otherwise
				// the "OR" conjunction confuses it and it switches to a full
				// scan of
				// the raw_contacts table.

				// This code relies on the fact that Im.DATA and Email.DATA are
				// in fact the same
				// column - Data.DATA1
				mSb.append(DataColumns.MIMETYPE_ID + " IN (?,?)" + " AND "
						+ Data.DATA1 + "=?" + " AND (("
						+ DataColumns.MIMETYPE_ID + "=? AND " + Im.PROTOCOL
						+ "=?");
				...
				mSb.append(DataColumns.MIMETYPE_ID + "=?" + " AND "
						+ Im.PROTOCOL + "=?" + " AND " + Im.DATA + "=?");
				mSelectionArgs.add(mimeTypeIdIm);
				mSelectionArgs.add(String.valueOf(protocol));
				mSelectionArgs.add(handle);
				if (customProtocol != null) {
		...
		try {
			cursor = mDb
					.query(DataContactsQuery.TABLE,
							DataContactsQuery.PROJECTION, mSb.toString(),
							mSelectionArgs.toArray(EMPTY_STRING_ARRAY), null,
							null, Contacts.IN_VISIBLE_GROUP + " DESC, "
									+ Data.RAW_CONTACT_ID);
			if (cursor.moveToFirst()) {
		...
		if (values.containsKey(StatusUpdates.PRESENCE)) {
			if (customProtocol == null) {
				// We cannot allow a null in the custom protocol field, because
				// SQLite3 does not
				// properly enforce uniqueness of null values
				customProtocol = "";
			}
			...
			if (values.containsKey(StatusUpdates.IM_ACCOUNT)) {
				mValues.put(StatusUpdates.IM_ACCOUNT,
						values.getAsString(StatusUpdates.IM_ACCOUNT));
			}
		...
		if (values.containsKey(StatusUpdates.STATUS)) {
			...
			String resPackage = values
					.getAsString(StatusUpdates.STATUS_RES_PACKAGE);
			Integer labelResource = values
					.getAsInteger(StatusUpdates.STATUS_LABEL);
			...
			if (TextUtils.isEmpty(status)) {
				...
				long timestamp = values
						.getAsLong(StatusUpdates.STATUS_TIMESTAMP);
	...
	protected int deleteInTransaction(Uri uri, String selection,
		...
		switch (match) {
		case SYNCSTATE:
			return mDbHelper.getSyncState().delete(mDb, selection,
					selectionArgs);

		case SYNCSTATE_ID:
			String selectionWithId = (SyncStateContract.Columns._ID + "="
		...
		case CONTACTS_LOOKUP_ID: {
			// lookup contact by id and lookup key to see if they still match
			// the actual record
			long contactId = ContentUris.parseId(uri);
			final List<String> pathSegments = uri.getPathSegments();
			final String lookupKey = pathSegments.get(2);
			...
			if (selectionArgs == null) {
				...
				System.arraycopy(selectionArgs, 0, args, 2,
			...
			try {
				if (c.getCount() == 1) {
					...
					// row was changed (e.g. the merging might have changed), we
					// got multiple
		...
		case RAW_CONTACTS: {
			int numDeletes = 0;
			Cursor c = mDb.query(Tables.RAW_CONTACTS, new String[] {
		...
		case RAW_CONTACTS_ID: {
			...
			return deleteRawContact(rawContactId,
					mDbHelper.getContactId(rawContactId), callerIsSyncAdapter);
		}

		case DATA: {
			mSyncToNetwork |= !callerIsSyncAdapter;
			return deleteData(appendAccountToSelection(uri, selection),
		...
		case POSTALS_ID: {
			...
			mSyncToNetwork |= !callerIsSyncAdapter;
			mSelectionArgs1[0] = String.valueOf(dataId);
			return deleteData(Data._ID + "=?", mSelectionArgs1,
					callerIsSyncAdapter);
		}

		case GROUPS_ID: {
			mSyncToNetwork |= !callerIsSyncAdapter;
			return deleteGroup(uri, ContentUris.parseId(uri),
					callerIsSyncAdapter);
		}
		...
		case GROUPS: {
			...
			try {
				while (c.moveToNext()) {
					numDeletes += deleteGroup(uri, c.getLong(0),
							callerIsSyncAdapter);
				}
			} finally {
				c.close();
	...
	private int deleteContact(long contactId) {
		...
		Cursor c = mDb.query(Tables.RAW_CONTACTS,
				...
				RawContacts.CONTACT_ID + "=?", mSelectionArgs1, null, null,
				null);
		try {
			while (c.moveToNext()) {
		...
		return mDb
				.delete(Tables.CONTACTS, Contacts._ID + "=" + contactId, null);
	}
	...
	public int deleteRawContact(long rawContactId, long contactId,
		...
		if (callerIsSyncAdapter) {
			mDb.delete(Tables.PRESENCE, PresenceColumns.RAW_CONTACT_ID + "="
					+ rawContactId, null);
			int count = mDb.delete(Tables.RAW_CONTACTS, RawContacts._ID + "="
					+ rawContactId, null);
	...
	private int deleteStatusUpdates(String selection, String[] selectionArgs) {
		...
		mDb.delete(Tables.STATUS_UPDATES,
				getWhereClauseForStatusUpdatesTable(selection), selectionArgs);
...
//			count = updateContactOptions(contactId, values);
			// add by chenqiang
			final long contactId = Long.parseLong(pathSegments.get(3));
			count = updateContactOptions(contactId, values);
			break;
		}
		...
		case RAW_CONTACTS_DATA: {
			...
			count = updateData(uri, values, selectionWithId, selectionArgs,
					callerIsSyncAdapter);
		...
		case SETTINGS: {
			count = updateSettings(uri, values,
					appendAccountToSelection(uri, selection), selectionArgs);
			mSyncToNetwork |= !callerIsSyncAdapter;
			break;
		}
	...
	private int updateStatusUpdate(Uri uri, ContentValues values,
			String selection, String[] selectionArgs) {
		...
		if (settableValues.size() > 0) {
			updateCount = mDb.update(Tables.STATUS_UPDATES, settableValues,
		...
		// TODO updateCount is not entirely a valid count of updated rows
		// because 2 tables could
		// potentially get updated in this method.
		return updateCount;
	}

	/**
	 * Build a where clause to select the rows to be updated in status_updates
	 * table.
	 */
	...
	private ContentValues getSettableColumnsForStatusUpdatesTable(
			ContentValues values) {
		mValues.clear();
		ContactsDatabaseHelper.copyStringValue(mValues, StatusUpdates.STATUS,
				values, StatusUpdates.STATUS);
		ContactsDatabaseHelper.copyStringValue(mValues,
				StatusUpdates.STATUS_TIMESTAMP, values,
				StatusUpdates.STATUS_TIMESTAMP);
		ContactsDatabaseHelper.copyStringValue(mValues,
				StatusUpdates.STATUS_RES_PACKAGE, values,
				StatusUpdates.STATUS_RES_PACKAGE);
		ContactsDatabaseHelper.copyStringValue(mValues,
				StatusUpdates.STATUS_LABEL, values, StatusUpdates.STATUS_LABEL);
		ContactsDatabaseHelper.copyStringValue(mValues,
				StatusUpdates.STATUS_ICON, values, StatusUpdates.STATUS_ICON);
		return mValues;
	}

	private ContentValues getSettableColumnsForPresenceTable(
			ContentValues values) {
		mValues.clear();
		ContactsDatabaseHelper.copyStringValue(mValues, StatusUpdates.PRESENCE,
				values, StatusUpdates.PRESENCE);
		return mValues;
	}

	private int updateGroups(Uri uri, ContentValues values,
			String selectionWithId, String[] selectionArgs,
			boolean callerIsSyncAdapter) {
		...
		int count = mDb.update(Tables.GROUPS, updatedValues, selectionWithId,
				selectionArgs);
		...
		if (updatedValues.containsKey(Groups.SHOULD_SYNC)
			...
			Cursor c = mDb.query(Tables.GROUPS, new String[] {
				...
				while (c.moveToNext()) {
					...
					if (!TextUtils.isEmpty(accountName)
						...
						ContentResolver.requestSync(account,
								TContactsContract.AUTHORITY, new Bundle());
						break;
					}
				}
			} finally {
	...
	private int updateSettings(Uri uri, ContentValues values, String selection,
			String[] selectionArgs) {
		final int count = mDb.update(Tables.SETTINGS, values, selection,
				selectionArgs);
		if (values.containsKey(Settings.UNGROUPED_VISIBLE)) {
			mVisibleTouched = true;
		}
	...
	private int updateRawContacts(ContentValues values, String selection,
			String[] selectionArgs) {
		...
		Cursor cursor = mDb.query(mDbHelper.getRawContactView(),
				new String[] { RawContacts._ID }, selection, selectionArgs,
				null, null, null);
		try {
	...
	private int updateRawContact(long rawContactId, ContentValues values) {
		...
		final boolean requestUndoDelete = (values
				.containsKey(RawContacts.DELETED) && values
				.getAsInteger(RawContacts.DELETED) == 0);
		int previousDeleted = 0;
		String accountType = null;
		String accountName = null;
		if (requestUndoDelete) {
			Cursor cursor = mDb.query(RawContactsQuery.TABLE,
					RawContactsQuery.COLUMNS, selection, mSelectionArgs1, null,
					null, null);
			try {
				if (cursor.moveToFirst()) {
		...
		int count = mDb.update(Tables.RAW_CONTACTS, values, selection,
				mSelectionArgs1);
		if (count != 0) {
			if (values.containsKey(RawContacts.AGGREGATION_MODE)) {
				int aggregationMode = values
						.getAsInteger(RawContacts.AGGREGATION_MODE);

				// As per ContactsContract documentation, changing aggregation
				// mode
			...
			if (values.containsKey(RawContacts.SOURCE_ID)) {
				mContactAggregator.updateLookupKeyForRawContact(mDb,
						rawContactId);
			}
			if (values.containsKey(TRawContacts.NAME_VERIFIED)) {

				// If setting NAME_VERIFIED for this raw contact, reset it for
				// all
				// other raw contacts in the same aggregate
				if (values.getAsInteger(TRawContacts.NAME_VERIFIED) != 0) {
					mResetNameVerifiedForOtherRawContacts.bindLong(1,
							rawContactId);
					mResetNameVerifiedForOtherRawContacts.bindLong(2,
							rawContactId);
					mResetNameVerifiedForOtherRawContacts.execute();
				}
				mContactAggregator.updateDisplayNameForRawContact(mDb,
						rawContactId);
			}
			if (requestUndoDelete && previousDeleted == 1) {
				// undo delete, needs aggregation again.
				mInsertedRawContacts.put(rawContactId, new Account(accountName,
						accountType));
			}
		}
		return count;
	...
	private int updateData(Uri uri, ContentValues values, String selection,
		...
		String packageName = values.getAsString(TData.RES_PACKAGE);
		if (packageName != null) {
			mValues.remove(TData.RES_PACKAGE);
			mValues.put(DataColumns.PACKAGE_ID,
					mDbHelper.getPackageId(packageName));
		...
		boolean containsIsSuperPrimary = mValues
				.containsKey(Data.IS_SUPER_PRIMARY);
		boolean containsIsPrimary = mValues.containsKey(Data.IS_PRIMARY);

		// Remove primary or super primary values being set to 0. This is
		// disallowed by the
		// content provider.
		if (containsIsSuperPrimary
				&& mValues.getAsInteger(Data.IS_SUPER_PRIMARY) == 0) {
		...
		// Note that the query will return data according to the access
		// restrictions,
		// so we don't need to worry about updating data we don't have
		// permission to read.
		Cursor c = query(uri, DataUpdateQuery.COLUMNS, selection,
				selectionArgs, null);
		try {
			while (c.moveToNext()) {
	...
	private int updateData(ContentValues values, Cursor c,
			boolean callerIsSyncAdapter) {
		if (values.size() == 0) {
			return 0;
		}
	...
	private int updateContactOptions(ContentValues values, String selection,
		...
		Cursor cursor = mDb.query(mDbHelper.getContactView(),
				new String[] { Contacts._ID }, selection, selectionArgs, null,
				null, null);
		try {
			while (cursor.moveToNext()) {
	...
	private int updateContactOptions(long contactId, ContentValues values) {
		...
		mValues.clear();
		ContactsDatabaseHelper.copyStringValue(mValues, "guid", values, "guid");
		ContactsDatabaseHelper.copyStringValue(mValues, "sync_state", values,
				"sync_state");
		ContactsDatabaseHelper.copyStringValue(mValues,
				RawContacts.CUSTOM_RINGTONE, values, Contacts.CUSTOM_RINGTONE);
		ContactsDatabaseHelper.copyLongValue(mValues,
				RawContacts.SEND_TO_VOICEMAIL, values,
				Contacts.SEND_TO_VOICEMAIL);
		ContactsDatabaseHelper.copyLongValue(mValues,
				RawContacts.LAST_TIME_CONTACTED, values,
				Contacts.LAST_TIME_CONTACTED);
		ContactsDatabaseHelper.copyLongValue(mValues,
				RawContacts.TIMES_CONTACTED, values, Contacts.TIMES_CONTACTED);
		...
		int rslt = mDb.update(Tables.CONTACTS, mValues, Contacts._ID + "=?",
				mSelectionArgs1);
		...
		if (values.containsKey(Contacts.LAST_TIME_CONTACTED)
			...
			mDb.execSQL(UPDATE_TIMES_CONTACTED_RAWCONTACTS_TABLE,
					mSelectionArgs1);
		}
		return rslt;
	}

	private int updateAggregationException(SQLiteDatabase db,
			ContentValues values) {
		...
		if (exceptionType == AggregationExceptions.TYPE_AUTOMATIC) {
			...
			exceptionValues.put(AggregationExceptions.RAW_CONTACT_ID1,
					rawContactId1);
			exceptionValues.put(AggregationExceptions.RAW_CONTACT_ID2,
					rawContactId2);
			db.replace(Tables.AGGREGATION_EXCEPTIONS,
					AggregationExceptions._ID, exceptionValues);
		}
		...
		// The return value is fake - we just confirm that we made a change, not
		// count actual
		// rows changed.
		return 1;
	}
	 ...
	 * @return the group id
	 */
	private long getOrCreateMyContactsGroupInTransaction(String accountName,
			String accountType) {
		Cursor cursor = mDb.query(Tables.GROUPS, new String[] { "_id" },
				Groups.ACCOUNT_NAME + " =? AND " + Groups.ACCOUNT_TYPE
						+ " =? AND " + Groups.TITLE + " =?", new String[] {
						accountName, accountType,
						GOOGLE_MY_CONTACTS_GROUP_TITLE }, null, null, null);
		try {
		...
		try {
			...
			for (Account account : accounts) {
				if (!existingAccounts.contains(account)) {
					mDb.execSQL("INSERT INTO " + Tables.ACCOUNTS + " ("
							+ RawContacts.ACCOUNT_NAME + ", "
			...
			// Remove all valid accounts from the existing account set. What is
			// left
			// in the accountsToDelete set will be extra accounts whose data
			// must be deleted.
			...
			for (Account account : accountsToDelete) {
				...
				mDb.execSQL("DELETE FROM " + Tables.GROUPS + " WHERE "
						+ Groups.ACCOUNT_NAME + " = ?" + " AND "
						+ Groups.ACCOUNT_TYPE + " = ?", params);
				mDb.execSQL("DELETE FROM " + Tables.PRESENCE + " WHERE "
						+ PresenceColumns.RAW_CONTACT_ID + " IN (" + "SELECT "
						+ RawContacts._ID + " FROM " + Tables.RAW_CONTACTS
						+ " WHERE " + RawContacts.ACCOUNT_NAME + " = ?"
						+ " AND " + RawContacts.ACCOUNT_TYPE + " = ?)", params);
				mDb.execSQL("DELETE FROM " + Tables.RAW_CONTACTS + " WHERE "
						+ RawContacts.ACCOUNT_NAME + " = ?" + " AND "
						+ RawContacts.ACCOUNT_TYPE + " = ?", params);
				mDb.execSQL("DELETE FROM " + Tables.SETTINGS + " WHERE "
						+ Settings.ACCOUNT_NAME + " = ?" + " AND "
						+ Settings.ACCOUNT_TYPE + " = ?", params);
				mDb.execSQL("DELETE FROM " + Tables.ACCOUNTS + " WHERE "
						+ RawContacts.ACCOUNT_NAME + "=?" + " AND "
						+ RawContacts.ACCOUNT_TYPE + "=?", params);
			}

			if (!accountsToDelete.isEmpty()) {
				// Find all aggregated contacts that used to contain the raw
				// contacts
				// we have just deleted and see if they are still referencing
				// the deleted
				...
				Cursor cursor = mDb.rawQuery("SELECT " + Contacts._ID
						+ " FROM " + Tables.CONTACTS + " WHERE ("
						+ TContacts.NAME_RAW_CONTACT_ID + " NOT NULL AND "
						+ TContacts.NAME_RAW_CONTACT_ID + " NOT IN "
						+ "(SELECT " + RawContacts._ID + " FROM "
						+ Tables.RAW_CONTACTS + "))" + " OR ("
						+ Contacts.PHOTO_ID + " NOT NULL AND "
						+ Contacts.PHOTO_ID + " NOT IN " + "(SELECT "
						+ Data._ID + " FROM " + Tables.DATA + "))", null);
				try {
					while (cursor.moveToNext()) {
			...
			if (hasUnassignedContacts[0]) {
				...
				if (primaryAccount != null) {
					String[] params = new String[] { primaryAccount.name,
							primaryAccount.type };
					if (primaryAccount.type.equals(DEFAULT_ACCOUNT_TYPE)) {
						...
						if (groupId != -1) {
							long mimeTypeId = mDbHelper
									.getMimeTypeId(GroupMembership.CONTENT_ITEM_TYPE);
							mDb.execSQL("INSERT INTO " + Tables.DATA + "("
					...
					mDb.execSQL("UPDATE " + Tables.RAW_CONTACTS + " SET "
							+ RawContacts.ACCOUNT_NAME + "=?,"
							+ RawContacts.ACCOUNT_TYPE + "=?" + " WHERE "
							+ RawContacts.ACCOUNT_NAME + " IS NULL" + " AND "
							+ RawContacts.ACCOUNT_TYPE + " IS NULL", params);

					// We don't currently support groups for unsynced accounts,
					// so this is for
					// the future
					mDb.execSQL("UPDATE " + Tables.GROUPS + " SET "
							+ Groups.ACCOUNT_NAME + "=?," + Groups.ACCOUNT_TYPE
							+ "=?" + " WHERE " + Groups.ACCOUNT_NAME
							+ " IS NULL" + " AND " + Groups.ACCOUNT_TYPE
							+ " IS NULL", params);
		...
		try {
			while (c.moveToNext()) {
				if (c.isNull(0) && c.isNull(1)) {
					...
					validAccounts.add(new Account(c.getString(0), c
							.getString(1)));
				}
			}
		} finally {
	...
	/**
	 * Returns true if a value (possibly null) is specified for at least one of
	 * the supplied keys.
	 */
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		switch (match) {
		case SYNCSTATE:
			return mDbHelper.getSyncState().query(db, projection, selection,
		...
		case CONTACTS_LOOKUP_ID: {
			...
			if (segmentCount == 4) {
				// TODO: pull this out into a method and generalize to not
				// require contactId
				...
				args[0] = String.valueOf(contactId);
				args[1] = Uri.encode(lookupKey);
				lookupQb.appendWhere(Contacts._ID + "=? AND "
						+ Contacts.LOOKUP_KEY + "=?");
				Cursor c = query(db, lookupQb, projection, selection, args,
		...
		case CONTACTS_AS_VCARD: {
			...
			qb.setTables(mDbHelper
					.getContactView(true /* require restricted */));
		...
		case CONTACTS_AS_MULTI_VCARD: {
			...
			return db.rawQuery("SELECT" + " 'vcards_' || ? || '.vcf' AS "
					+ OpenableColumns.DISPLAY_NAME + "," + " NULL AS "
					+ OpenableColumns.SIZE, new String[] { currentDateString });
		}
		...
		case CONTACTS_FILTER: {
			...
			setTablesAndProjectionMapForContactsWithSnippet(qb, uri,
					projection, filterParam);
			break;
		}
		...
		case CONTACTS_STREQUENT: {
			...
			if (projection != null) {
				starredProjection = appendProjectionArg(projection,
			...
			final String starredQuery = qb.buildQuery(starredProjection,
					Contacts.STARRED + "=1", null, Contacts._ID, null, null,
					null);
			...
			// Put them together
			final String query = qb.buildUnionQuery(new String[] {
					starredQuery, frequentQuery }, STREQUENT_ORDER_BY,
					STREQUENT_LIMIT);
			Cursor c = db.rawQuery(query, null);
			if (c != null) {
				c.setNotificationUri(getContext().getContentResolver(),
						TContactsContract.AUTHORITY_URI);
			}
			return c;
		}
		...
		case PHONES_FILTER: {
			...
			if (uri.getPathSegments().size() > 2) {
				...
				if (normalizedName.length() > 0) {
					...
					appendRawContactsByNormalizedNameFilter(sb, normalizedName,
							false);
					orNeeded = true;
					hasCondition = true;
				}
				...
				if (isPhoneNumber(filterParam)) {
					...
					String number = PhoneNumberUtils
							.convertKeypadLettersToDigits(filterParam);
					String reversed = PhoneNumberUtils
							.getStrippedReversed(number);
					sb.append(Data._ID + " IN (SELECT "
							+ PhoneLookupColumns.DATA_ID + " FROM "
							+ Tables.PHONE_LOOKUP + " WHERE "
			...
			groupBy = PhoneColumns.NORMALIZED_NUMBER + ","
					+ RawContacts.CONTACT_ID;
			if (sortOrder == null) {
				sortOrder = Contacts.IN_VISIBLE_GROUP + " DESC, "
						+ RawContacts.CONTACT_ID;
			}
			break;
		}
		...
		case EMAILS_ID: {
			...
			selectionArgs = insertSelectionArg(selectionArgs,
					uri.getLastPathSegment());
			qb.appendWhere(" AND " + Data.MIMETYPE + " = '"
					+ Email.CONTENT_ITEM_TYPE + "'" + " AND " + Data._ID + "=?");
			break;
		}
		...
		case EMAILS_LOOKUP: {
			...
			if (uri.getPathSegments().size() > 2) {
				...
				String address = mDbHelper
						.extractAddressFromEmailAddress(email);
		...
		case EMAILS_FILTER: {
			...
			if (filterParam == null) {
				...
				sb.append("SELECT " + Data._ID + " FROM " + Tables.DATA
						+ " WHERE " + DataColumns.MIMETYPE_ID + "="
						+ mMimeTypeIdEmail + " AND " + Data.DATA1 + " LIKE ");
				DatabaseUtils.appendEscapedSQLString(sb, filterParam + '%');
				if (!filterParam.contains("@")) {
					String normalizedName = NameNormalizer
							.normalize(filterParam);
					if (normalizedName.length() > 0) {

						/*
						 * Using a UNION instead of an "OR" to make SQLite use
						 * the right indexes. We need it to use the
						 * (mimetype,data1) index for the email lookup (see
						 * above), but not for the name lookup. SQLite is not
						 * smart enough to use the index on one side of an OR
						 * but not on the other. Using two separate nested
						 * queries and a UNION between them does the job.
						 */
						sb.append(" UNION SELECT " + Data._ID + " FROM "
								+ Tables.DATA + " WHERE +"
								+ DataColumns.MIMETYPE_ID + "="
								+ mMimeTypeIdEmail + " AND "
								+ Data.RAW_CONTACT_ID + " IN ");
						appendRawContactsByNormalizedNameFilter(sb,
		...
		case PHONE_LOOKUP: {
			...
			if (TextUtils.isEmpty(sortOrder)) {
				// Default the sort order to something reasonable so we get
				// consistent
				// results when callers don't request an ordering
				sortOrder = RawContactsColumns.CONCRETE_ID;
			}

			String number = uri.getPathSegments().size() > 1 ? uri
					.getLastPathSegment() : "";
		...
		case AGGREGATION_SUGGESTIONS: {
			...
			return mContactAggregator.queryAggregationSuggestions(qb,
					projection, contactId, maxSuggestions, filter);
		}
		...
		case SETTINGS: {
			...
			if (projection != null
					&& projection.length != 0
					&& mDbHelper.isInProjection(projection,
							Settings.UNGROUPED_COUNT)) {
				selectionArgs = insertSelectionArg(selectionArgs,
		...
		case SEARCH_SUGGESTIONS: {
			return mGlobalSearchSupport.handleSearchSuggestionsQuery(db, uri,
					limit);
		}

		case SEARCH_SHORTCUT: {
			String lookupKey = uri.getLastPathSegment();
			return mGlobalSearchSupport.handleSearchShortcutRefresh(db,
					lookupKey, projection);
		}
		...
		default:
			return mLegacyApiSupport.query(uri, projection, selection,
					selectionArgs, sortOrder, limit);
		}

		// qb.setStrictProjectionMap(true);
		TMethod.setStrictProjectionMap(qb, true);

		Cursor cursor = query(db, qb, projection, selection, selectionArgs,
				sortOrder, groupBy, limit);
		if (readBooleanQueryParameter(uri,
				TContactCounts.ADDRESS_BOOK_INDEX_EXTRAS, false)) {
			cursor = bundleLetterCountExtras(cursor, db, qb, selection,
					selectionArgs, sortOrder);
		}
		return cursor;
	}
	...
	/**
	 * Creates a single-row cursor containing the current status of the
	 * provider.
	 */
	private Cursor queryProviderStatus(Uri uri, String[] projection) {
		...
		for (int i = 0; i < projection.length; i++) {
			if (TProviderStatus.STATUS.equals(projection[i])) {
				row.add(mProviderStatus);
			} else if (TProviderStatus.DATA1.equals(projection[i])) {
	...
	/**
	 * Computes counts by the address book index titles and adds the resulting
	 * tally to the returned cursor as a bundle of extras.
	 */
	private Cursor bundleLetterCountExtras(Cursor cursor,
		...
		if (sortOrder != null) {
			...
			sortKey = TContacts.SORT_KEY_PRIMARY;
		}

		String locale = getLocale().toString();
		HashMap<String, String> projectionMap = Maps.newHashMap();
		projectionMap.put(AddressBookIndexQuery.LETTER, "SUBSTR(" + sortKey
				+ ",1,1) AS " + AddressBookIndexQuery.LETTER);

		/**
		 * Use the GET_PHONEBOOK_INDEX function, which is an android extension
		 * for SQLite3, to map the first letter of the sort key to a character
		 * that is traditionally used in phonebooks to represent that letter.
		 * For example, in Korean it will be the first consonant in the letter;
		 * for Japanese it will be Hiragana rather than Katakana.
		 */
		projectionMap.put(AddressBookIndexQuery.TITLE,
				"GET_PHONEBOOK_INDEX(SUBSTR(" + sortKey + ",1,1),'" + locale
						+ "')" + " AS " + AddressBookIndexQuery.TITLE);
		projectionMap.put(AddressBookIndexQuery.COUNT, "COUNT(" + Contacts._ID
				+ ") AS " + AddressBookIndexQuery.COUNT);
		qb.setProjectionMap(projectionMap);

		Cursor indexCursor = qb.query(db, AddressBookIndexQuery.COLUMNS,
		...
		try {
			...
			bundle.putStringArray(
					TContactCounts.EXTRA_ADDRESS_BOOK_INDEX_TITLES, titles);
			bundle.putIntArray(TContactCounts.EXTRA_ADDRESS_BOOK_INDEX_COUNTS,
					counts);
	...
	public long lookupContactIdByLookupKey(SQLiteDatabase db, String lookupKey) {
		...
		long contactId = -1;
		if (lookupKeyContainsType(segments,
		...
		boolean hasRawContactIds = lookupKeyContainsType(segments,
		...
		if (hasRawContactIds
				|| lookupKeyContainsType(segments,
	...
	private interface LookupBySourceIdQuery {
		...
		String COLUMNS[] = { RawContacts.CONTACT_ID, RawContacts.ACCOUNT_TYPE,
				RawContacts.ACCOUNT_NAME, RawContacts.SOURCE_ID };
	...
	private long lookupContactIdBySourceIds(SQLiteDatabase db,
		...
		Cursor c = db.query(LookupBySourceIdQuery.TABLE,
				LookupBySourceIdQuery.COLUMNS, sb.toString(), null, null, null,
				null);
		try {
			while (c.moveToNext()) {
				String accountType = c
						.getString(LookupBySourceIdQuery.ACCOUNT_TYPE);
				String accountName = c
						.getString(LookupBySourceIdQuery.ACCOUNT_NAME);
				int accountHashCode = ContactLookupKey.getAccountHashCode(
				...
				for (int i = 0; i < segments.size(); i++) {
					...
					if (segment.lookupType == ContactLookupKey.LOOKUP_TYPE_SOURCE_ID
						...
						segment.contactId = c
								.getLong(LookupBySourceIdQuery.CONTACT_ID);
						break;
					}
				}
			}
	...
	private interface LookupByRawContactIdQuery {
		...
		String COLUMNS[] = { RawContacts.CONTACT_ID, RawContacts.ACCOUNT_TYPE,
				RawContacts.ACCOUNT_NAME, RawContacts._ID, };
	...
	private long lookupContactIdByRawContactIds(SQLiteDatabase db,
		...
		Cursor c = db.query(LookupByRawContactIdQuery.TABLE,
				LookupByRawContactIdQuery.COLUMNS, sb.toString(), null, null,
				null, null);
		try {
			while (c.moveToNext()) {
				String accountType = c
						.getString(LookupByRawContactIdQuery.ACCOUNT_TYPE);
				String accountName = c
						.getString(LookupByRawContactIdQuery.ACCOUNT_NAME);
				int accountHashCode = ContactLookupKey.getAccountHashCode(
						accountType, accountName);
				...
				for (int i = 0; i < segments.size(); i++) {
					...
					if (segment.lookupType == ContactLookupKey.LOOKUP_TYPE_RAW_CONTACT_ID
						...
						segment.contactId = c
								.getLong(LookupByRawContactIdQuery.CONTACT_ID);
						break;
					}
				}
			}
	...
	private interface LookupByDisplayNameQuery {
		...
		String COLUMNS[] = { RawContacts.CONTACT_ID, RawContacts.ACCOUNT_TYPE,
				RawContacts.ACCOUNT_NAME, NameLookupColumns.NORMALIZED_NAME };
	...
	private long lookupContactIdByDisplayNames(SQLiteDatabase db,
		...
		sb.append(") AND " + NameLookupColumns.NAME_TYPE + "="
				+ NameLookupType.NAME_COLLATION_KEY + " AND "
				+ RawContacts.CONTACT_ID + " NOT NULL");

		Cursor c = db.query(LookupByDisplayNameQuery.TABLE,
				LookupByDisplayNameQuery.COLUMNS, sb.toString(), null, null,
				null, null);
		try {
			while (c.moveToNext()) {
				String accountType = c
						.getString(LookupByDisplayNameQuery.ACCOUNT_TYPE);
				String accountName = c
						.getString(LookupByDisplayNameQuery.ACCOUNT_NAME);
				int accountHashCode = ContactLookupKey.getAccountHashCode(
						accountType, accountName);
				String name = c
						.getString(LookupByDisplayNameQuery.NORMALIZED_NAME);
				for (int i = 0; i < segments.size(); i++) {
					LookupKeySegment segment = segments.get(i);
					if ((segment.lookupType == ContactLookupKey.LOOKUP_TYPE_DISPLAY_NAME || segment.lookupType == ContactLookupKey.LOOKUP_TYPE_RAW_CONTACT_ID)
							&& accountHashCode == segment.accountHashCode
							&& segment.key.equals(name)) {
						segment.contactId = c
								.getLong(LookupByDisplayNameQuery.CONTACT_ID);
						break;
					}
				}
			}
	...
	private boolean lookupKeyContainsType(ArrayList<LookupKeySegment> segments,
			int lookupType) {
		for (int i = 0; i < segments.size(); i++) {
			LookupKeySegment segment = segments.get(i);
	...
	public void updateLookupKeyForRawContact(SQLiteDatabase db,
			long rawContactId) {
	...
	private void setTablesAndProjectionMapForContacts(SQLiteQueryBuilder qb,
	...
	/**
	 * Finds name lookup records matching the supplied filter, picks one
	 * arbitrary match per contact and joins that with other contacts tables.
	 */
	private void setTablesAndProjectionMapForContactsWithSnippet(
			SQLiteQueryBuilder qb, Uri uri, String[] projection, String filter) {

		StringBuilder sb = new StringBuilder();
		appendContactsTables(sb, uri, projection);

		sb.append(" JOIN (SELECT " + RawContacts.CONTACT_ID
				+ " AS snippet_contact_id");
		...
		if (mDbHelper.isInProjection(projection,
				TSearchSnippetColumns.SNIPPET_DATA1)) {
			sb.append(", " + Data.DATA1 + " AS "
					+ TSearchSnippetColumns.SNIPPET_DATA1);
		}

		if (mDbHelper.isInProjection(projection,
				TSearchSnippetColumns.SNIPPET_DATA2)) {
			sb.append(", " + Data.DATA2 + " AS "
					+ TSearchSnippetColumns.SNIPPET_DATA2);
		}

		if (mDbHelper.isInProjection(projection,
				TSearchSnippetColumns.SNIPPET_DATA3)) {
			sb.append(", " + Data.DATA3 + " AS "
					+ TSearchSnippetColumns.SNIPPET_DATA3);
		}

		if (mDbHelper.isInProjection(projection,
				TSearchSnippetColumns.SNIPPET_DATA4)) {
			sb.append(", " + Data.DATA4 + " AS "
					+ TSearchSnippetColumns.SNIPPET_DATA4);
		}

		if (mDbHelper.isInProjection(projection,
				TSearchSnippetColumns.SNIPPET_MIMETYPE)) {
			sb.append(", (" + "SELECT " + MimetypesColumns.MIMETYPE + " FROM "
					+ Tables.MIMETYPES + " WHERE " + MimetypesColumns._ID + "="
					+ DataColumns.MIMETYPE_ID + ") AS "
					+ TSearchSnippetColumns.SNIPPET_MIMETYPE);
		}

		sb.append(" FROM " + Tables.DATA_JOIN_RAW_CONTACTS + " WHERE "
				+ DataColumns.CONCRETE_ID + " IN (");

		// Construct a query that gives us exactly one data _id per matching
		// contact.
		// MIN stands in for ANY in this context.
		sb.append("SELECT MIN(" + Tables.NAME_LOOKUP + "."
				+ NameLookupColumns.DATA_ID + ")" + " FROM "
				+ Tables.NAME_LOOKUP + " JOIN " + Tables.RAW_CONTACTS + " ON ("
				+ RawContactsColumns.CONCRETE_ID + "=" + Tables.NAME_LOOKUP
				+ "." + NameLookupColumns.RAW_CONTACT_ID + ")" + " WHERE "
				+ NameLookupColumns.NORMALIZED_NAME + " GLOB '");
		sb.append(NameNormalizer.normalize(filter));
		sb.append("*' AND " + NameLookupColumns.NAME_TYPE + " IN("
				+ CONTACT_LOOKUP_NAME_TYPES + ")" + " GROUP BY "
	...
	private void appendContactsTables(StringBuilder sb, Uri uri,
			String[] projection) {
		...
		if (requestingPackage != null) {
			excludeRestrictedData = !mDbHelper
					.hasAccessToRestrictedData(requestingPackage);
		}
		sb.append(mDbHelper.getContactView(excludeRestrictedData));
		if (mDbHelper.isInProjection(projection, Contacts.CONTACT_PRESENCE)) {
			sb.append(" LEFT OUTER JOIN " + Tables.AGGREGATED_PRESENCE
					+ " ON (" + Contacts._ID + " = "
					+ AggregatedPresenceColumns.CONTACT_ID + ")");
		}
		if (mDbHelper.isInProjection(projection, Contacts.CONTACT_STATUS,
				Contacts.CONTACT_STATUS_RES_PACKAGE,
	...
	private void setTablesAndProjectionMapForRawContactsEntities(
			SQLiteQueryBuilder qb, Uri uri) {
		// Note: currently, "export only" equals to "restricted", but may not in
		// the future.
		boolean excludeRestrictedData = readBooleanQueryParameter(uri,
				TData.FOR_EXPORT_ONLY, false);
	...
	private void setTablesAndProjectionMapForData(SQLiteQueryBuilder qb,
			Uri uri, String[] projection, boolean distinct) {
		StringBuilder sb = new StringBuilder();
		// Note: currently, "export only" equals to "restricted", but may not in
		// the future.
		boolean excludeRestrictedData = readBooleanQueryParameter(uri,
		...
		if (mDbHelper.isInProjection(projection, Data.CONTACT_STATUS,
				Data.CONTACT_STATUS_RES_PACKAGE, Data.CONTACT_STATUS_ICON,
				Data.CONTACT_STATUS_LABEL, Data.CONTACT_STATUS_TIMESTAMP)) {
			sb.append(" LEFT OUTER JOIN " + Tables.STATUS_UPDATES + " "
					+ ContactsStatusUpdatesColumns.ALIAS + " ON ("
					+ ContactsColumns.LAST_STATUS_UPDATE_ID + "="
					+ ContactsStatusUpdatesColumns.CONCRETE_DATA_ID + ")");
		...
		if (mDbHelper.isInProjection(projection, Data.PRESENCE)) {
			sb.append(" LEFT OUTER JOIN " + Tables.PRESENCE + " ON ("
					+ StatusUpdates.DATA_ID + "=" + DataColumns.CONCRETE_ID
					+ ")");
		}

		// Include individual status updates when requested
		if (mDbHelper.isInProjection(projection, Data.STATUS,
				Data.STATUS_RES_PACKAGE, Data.STATUS_ICON, Data.STATUS_LABEL,
		...
		qb.setTables(sb.toString());
		qb.setProjectionMap(distinct ? sDistinctDataProjectionMap
				: sDataProjectionMap);
		appendAccountFromParameter(qb, uri);
	}
	...
	private void setTableAndProjectionMapForStatusUpdates(
		...
		if (mDbHelper.isInProjection(projection, StatusUpdates.PRESENCE)) {
			sb.append(" LEFT OUTER JOIN " + Tables.PRESENCE + " ON("
					+ Tables.PRESENCE + "." + StatusUpdates.DATA_ID + "="
					+ DataColumns.CONCRETE_ID + ")");
		}

		if (mDbHelper.isInProjection(projection, StatusUpdates.STATUS,
	...
	private void appendAccountFromParameter(SQLiteQueryBuilder qb, Uri uri) {
		final String accountName = getQueryParameter(uri,
				RawContacts.ACCOUNT_NAME);
		final String accountType = getQueryParameter(uri,
				RawContacts.ACCOUNT_TYPE);
	...
	public String getContactsRestrictionExceptionAsNestedQuery(
		...
		if (mDbHelper.hasAccessToRestrictedData()) {
			...
			return "(SELECT " + TRawContacts.IS_RESTRICTED + " FROM "
					+ Tables.RAW_CONTACTS + " WHERE "
					+ RawContactsColumns.CONCRETE_ID + "=" + contactIdColumn
					+ ")=0";
		}
	}
	...
	public AssetFileDescriptor openAssetFile(Uri uri, String mode)
		...
		switch (match) {
		case CONTACTS_PHOTO: {
			return openPhotoAssetFile(uri, mode, Data._ID + "="
					+ Contacts.PHOTO_ID + " AND " + RawContacts.CONTACT_ID
					+ "=?", new String[] { uri.getPathSegments().get(1) });
		}

		case DATA_ID: {
			return openPhotoAssetFile(uri, mode, Data._ID + "=? AND "
		...
		case CONTACTS_AS_VCARD: {
			final String lookupKey = Uri.encode(uri.getPathSegments().get(2));
			mSelectionArgs1[0] = String.valueOf(lookupContactIdByLookupKey(mDb,
					lookupKey));
		...
		case CONTACTS_AS_MULTI_VCARD: {
			...
			inBuilder.append(')');
			final String selection = Contacts._ID + " IN "
					+ inBuilder.toString();
		...
		default:
			throw new FileNotFoundException(mDbHelper.exceptionMessage(
					"File does not exist", uri));
		}
	}

	private AssetFileDescriptor openPhotoAssetFile(Uri uri, String mode,
			String selection, String[] selectionArgs)
			throws FileNotFoundException {
		if (!"r".equals(mode)) {
			throw new FileNotFoundException(mDbHelper.exceptionMessage("Mode "
					+ mode + " not supported.", uri));
		}

		String sql = "SELECT " + Photo.PHOTO + " FROM "
				+ mDbHelper.getDataView() + " WHERE " + selection;
	 ...
	 */
	private AssetFileDescriptor buildAssetFileDescriptor(
		...
		try {
			...
			final MemoryFile memoryFile = new MemoryFile(
					CONTACT_MEMORY_FILE_NAME, size);
			memoryFile.writeBytes(byteData, 0, 0, size);
			/*
			 * memoryFile.deactivate(); fd =
			 * AssetFileDescriptor.fromMemoryFile(memoryFile);
			 */
			fd = TMethod.memoryFile(memoryFile);
		} catch (IOException e) {
			Log.w(TAG, "Problem writing stream into an AssetFileDescriptor: "
					+ e.toString());
		}
		return fd;
	}
	 ...
	 */
	private void outputRawContactsAsVCard(OutputStream stream,
			String selection, String[] selectionArgs) {
		final Context context = this.getContext();
		final VCardComposer composer = new VCardComposer(context,
	...
	public String getType(Uri uri) {
		...
		case PHONE_LOOKUP:
			return TPhoneLookup.CONTENT_TYPE;
		case EMAILS:
	...
	private void setDisplayName(long rawContactId, int displayNameSource,
			...
			String phoneticName, int phoneticNameStyle, String sortKeyPrimary,
	...
	/*
	 * Sets the given dataId record in the "data" table to primary, and resets
	 * all data records of the same mimetype and under the same contact to not
	 * be primary.
	 * 
	...
	/*
	 * Sets the given dataId record in the "data" table to "super primary", and
	 * resets all data records of the same mimetype and under the same aggregate
	 * to not be "super primary".
	 * 
	 * @param dataId the id of the data record to be set to primary.
	 */
	private void setIsSuperPrimary(long rawContactId, long dataId,
			long mimeTypeId) {
	...
	public String insertNameLookupForEmail(long rawContactId, long dataId,
			String email) {
		...
		insertNameLookup(rawContactId, dataId,
				NameLookupType.EMAIL_BASED_NICKNAME,
				NameNormalizer.normalize(address));
		return address;
	}
	 ...
	 */
	public void insertNameLookupForNickname(long rawContactId, long dataId,
			String nickname) {
		if (TextUtils.isEmpty(nickname)) {
			return;
		}

		insertNameLookup(rawContactId, dataId, NameLookupType.NICKNAME,
				NameNormalizer.normalize(nickname));
	}

	public void insertNameLookupForOrganization(long rawContactId, long dataId,
			String company, String title) {
		if (!TextUtils.isEmpty(company)) {
			insertNameLookup(rawContactId, dataId, NameLookupType.ORGANIZATION,
	...
	public void insertNameLookupForStructuredName(long rawContactId,
			long dataId, String name, int fullNameStyle) {
	...
	private class StructuredNameLookupBuilder extends NameLookupBuilder {
		...
		@Override
		protected void insertNameLookup(long rawContactId, long dataId,
				int lookupType, String name) {
			ContactsProvider2.this.insertNameLookup(rawContactId, dataId,
					lookupType, name);
		}

		@Override
		protected String[] getCommonNicknameClusters(String normalizedName) {
			return mCommonNicknameCache
					.getCommonNicknameClusters(normalizedName);
		}
	}
	...
	public void insertNameLookupForPhoneticName(long rawContactId, long dataId,
		...
		if (givenName != null) {
			// We want the phonetic given name to be used for search, but not
			// for aggregation,
			// which is why we are using NAME_SHORTHAND rather than
			// NAME_COLLATION_KEY
			insertNameLookup(rawContactId, dataId,
					NameLookupType.NAME_SHORTHAND,
	 ...
	 */
	public void insertNameLookup(long rawContactId, long dataId,
			int lookupType, String name) {
	...
	/**
	 * Deletes all {@link Tables#NAME_LOOKUP} table rows associated with the
	...
	public void appendContactFilterAsNestedQuery(StringBuilder sb,
			String filterParam) {
		sb.append("(" + "SELECT DISTINCT " + RawContacts.CONTACT_ID + " FROM "
				+ Tables.RAW_CONTACTS + " JOIN " + Tables.NAME_LOOKUP + " ON("
	...
	public void appendRawContactsByFilterAsNestedQuery(StringBuilder sb,
			String filterParam) {
		appendRawContactsByNormalizedNameFilter(sb,
				NameNormalizer.normalize(filterParam), true);
	}

	private void appendRawContactsByNormalizedNameFilter(StringBuilder sb,
			String normalizedName, boolean allowEmailMatch) {
		sb.append("(" + "SELECT " + NameLookupColumns.RAW_CONTACT_ID + " FROM "
				+ Tables.NAME_LOOKUP + " WHERE "
				+ NameLookupColumns.NORMALIZED_NAME + " GLOB '");
		sb.append(normalizedName);
		sb.append("*' AND " + NameLookupColumns.NAME_TYPE + " IN ("
				+ NameLookupType.NAME_COLLATION_KEY + ","
				+ NameLookupType.NICKNAME + "," + NameLookupType.NAME_SHORTHAND
				+ "," + NameLookupType.ORGANIZATION + ","
				+ NameLookupType.NAME_CONSONANTS);
		if (allowEmailMatch) {
	...
	protected Account getDefaultAccount() {
		...
		try {
			Account[] accounts = accountManager.getAccountsByTypeAndFeatures(
					...
					new String[] { FEATURE_LEGACY_HOSTED_OR_GOOGLE }, null,
					null).getResult();
	...
	protected boolean isWritableAccount(String accountType) {
		...
		IContentService contentService = IContentService.Stub
				.asInterface(ServiceManager.getService("content"));
		try {
	...
	/* package */static boolean readBooleanQueryParameter(Uri uri,
			String parameter, boolean defaultValue) {

		// Manually parse the query, which is much faster than calling
