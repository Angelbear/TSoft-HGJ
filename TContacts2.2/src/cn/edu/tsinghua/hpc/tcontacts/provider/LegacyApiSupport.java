...
import cn.edu.tsinghua.hpc.tcontacts.provider.ContactsDatabaseHelper.ExtensionsColumns;
...
import cn.edu.tsinghua.hpc.tcontacts.util.TContactsContract.TSettings;
...
public class LegacyApiSupport {
    ...
    private static final Uri LIVE_FOLDERS_CONTACTS_URI = Uri.withAppendedPath(
            TContactsContract.AUTHORITY_URI, "live_folders/contacts");

    private static final Uri LIVE_FOLDERS_CONTACTS_WITH_PHONES_URI = Uri.withAppendedPath(
            TContactsContract.AUTHORITY_URI, "live_folders/contacts_with_phones");

    private static final Uri LIVE_FOLDERS_CONTACTS_FAVORITES_URI = Uri.withAppendedPath(
            TContactsContract.AUTHORITY_URI, "live_folders/favorites");
    ...
    static {
        ...
        String authority = TContacts.AUTHORITY;
...
//        matcher.addURI(authority, "voice_dialer_timestamp", VOICE_DIALER_TIMESTAMP);
    ...
    public static void createViews(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE VIEW " + LegacyTables.PEOPLE + " AS SELECT " +
                ...
                " AND " + TRawContacts.IS_RESTRICTED + "=0" + ";");
    ...
    private long insertPeople(ContentValues values) {
        ...
        Uri contactUri = mContactsProvider.insertInTransaction(TRawContacts.CONTENT_URI, mValues);
        long rawContactId = ContentUris.parseId(contactUri);

        if (mValues2.size() != 0) {
            mValues2.put(Data.RAW_CONTACT_ID, rawContactId);
            mContactsProvider.insertInTransaction(TData.CONTENT_URI, mValues2);
        }
        if (mValues3.size() != 0) {
            mValues3.put(Data.RAW_CONTACT_ID, rawContactId);
            mContactsProvider.insertInTransaction(TData.CONTENT_URI, mValues3);
        }
    ...
    private long insertOrganization(ContentValues values) {
        ...
        Uri uri = mContactsProvider.insertInTransaction(TData.CONTENT_URI, mValues);
    ...
    private long insertGroup(ContentValues values) {
        ...
        Uri uri = mContactsProvider.insertInTransaction(TGroups.CONTENT_URI, mValues);
        return ContentUris.parseId(uri);
    }
    ...
    private int updatePeople(long rawContactId, ContentValues values) {
        ...
        int count = mContactsProvider.update(TRawContacts.CONTENT_URI,
    ...
    private int updateOrganizations(long dataId, ContentValues values) {
        ...
        return mContactsProvider.updateInTransaction(TData.CONTENT_URI, mValues,
    ...
    private int updateGroups(long groupId, ContentValues values) {
        ...
        return mContactsProvider.updateInTransaction(TGroups.CONTENT_URI, mValues,
    ...
    private int updatePhoto(long rawContactId, ContentValues values) {
        ...
        if (dataId == -1) {
            ...
            mValues.put(Data.RAW_CONTACT_ID, rawContactId);
            Uri dataUri = mContactsProvider.insertInTransaction(TData.CONTENT_URI, mValues);
            dataId = ContentUris.parseId(dataUri);
            count = 1;
        } else {
            Uri dataUri = ContentUris.withAppendedId(TData.CONTENT_URI, dataId);
    ...
    private int updatePhotoByDataId(long dataId, ContentValues values) {
        ...
        if (values.containsKey(android.provider.Contacts.Photos.DATA)) {
            ...
            mValues.clear();
            mValues.put(Photo.PHOTO, bytes);
            mContactsProvider.updateInTransaction(TData.CONTENT_URI, mValues,
                    Data._ID + "=" + dataId, null);
        }
    ...
    private void updateLegacyPhotoData(long rawContactId, long dataId, ContentValues values) {
        ...
        int updated = mContactsProvider.updateInTransaction(TData.CONTENT_URI, mValues,
        ...
        if (updated == 0) {
            ...
            mContactsProvider.insertInTransaction(TData.CONTENT_URI, mValues);
        }
    }
    ...
    private int updateSettings(ContentValues values) {
        ...
        if (key.equals(android.provider.Contacts.Settings.SYNC_EVERYTHING)) {
            ...
            int count = mContactsProvider.updateInTransaction(TSettings.CONTENT_URI, mValues,
                    selection, selectionArgs);
            if (count == 0) {
                mValues.put(Settings.ACCOUNT_NAME, accountName);
                mValues.put(Settings.ACCOUNT_TYPE, accountType);
                mContactsProvider.insertInTransaction(TSettings.CONTENT_URI, mValues);
            }
        }
        return 1;
    }
    ...
    private Uri findFirstDataRow(long rawContactId, String contentItemType) {
        ...
        return ContentUris.withAppendedId(TData.CONTENT_URI, dataId);
    }

    private long findFirstDataId(long rawContactId, String mimeType) {
        long dataId = -1;
        Cursor c = mContactsProvider.query(TData.CONTENT_URI, IdQuery.COLUMNS,
    ...
    private void onChange(Uri uri) {
        mContext.getContentResolver().notifyChange(TContacts.CONTENT_URI, null);
    }
