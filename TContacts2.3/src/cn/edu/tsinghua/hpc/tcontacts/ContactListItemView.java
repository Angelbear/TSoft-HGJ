...
import cn.edu.tsinghua.hpc.tcontacts.ui.widget.DontPressWithParentImageView;
import cn.edu.tsinghua.hpc.tcontacts.util.HiddenResource;
...
public class ContactListItemView extends ViewGroup {
	...
	private static final int QUICK_CONTACT_BADGE_STYLE = com.android.internal.R.attr.quickContactBadgeStyleWindowMedium;
	...
	private final int mSimViewPadding;
	...
	public ContactListItemView(Context context, AttributeSet attrs) {
		...
		mPreferredHeight = a
				.getDimensionPixelSize(HiddenResource
						.getStylable("Theme_listPreferredItemHeight")/*
																	 * android.R.
																	 * styleable
																	 * .
	 ...
	 */
	public void setOnCallButtonClickListener(
			OnClickListener callButtonClickListener) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (isVisible(mSimView)) {
			mSimView.measure(0, 0);
		}
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		if (photoView != null) {
			...
			int photoTop = topBound + (height - topBound - mPhotoViewHeight)
					/ 2;
			photoView.layout(leftBound, photoTop, leftBound + mPhotoViewWidth,
		...
		if (isVisible(mCallButton)) {
			...
			mCallButton.layout(rightBound, topBound, rightBound + buttonWidth,
					height);
			mVerticalDividerVisible = true;
			ensureVerticalDivider();
			rightBound -= mVerticalDividerWidth;
			mVerticalDividerDrawable.setBounds(rightBound, topBound
					+ mVerticalDividerMargin, rightBound
					+ mVerticalDividerWidth, height - mVerticalDividerMargin);
		}
		if (isVisible(mSimView)) {
			int buttonWidth = mSimView.getMeasuredWidth();
			rightBound -= buttonWidth;
			mSimView.layout(rightBound, topBound, rightBound + buttonWidth,
					height);
			mVerticalDividerVisible = true;
			ensureVerticalDivider();
			rightBound -= mVerticalDividerWidth;
		...
		if (isVisible(mPresenceIcon)) {
			...
			mPresenceIcon.layout(rightBound, topBound, rightBound + iconWidth,
					height);
		}

		if (mHorizontalDividerVisible) {
			ensureHorizontalDivider();
			mHorizontalDividerDrawable.setBounds(0, height
					- mHorizontalDividerHeight, width, height);
		}
		...
		mNameTextView.layout(leftBound, textTopBound, rightBound, textTopBound
				+ mLine1Height);

		int dataLeftBound = leftBound;
		if (isVisible(mLabelView)) {
			dataLeftBound = leftBound + mLabelView.getMeasuredWidth();
			mLabelView.layout(leftBound, textTopBound + mLine1Height,
					dataLeftBound, textTopBound + mLine1Height + mLine2Height);
			dataLeftBound += mGapBetweenLabelAndData;
		}

		if (isVisible(mDataView)) {
			mDataView.layout(dataLeftBound, textTopBound + mLine1Height,
					rightBound, textTopBound + mLine1Height + mLine2Height);
		}

		if (isVisible(mSnippetView)) {
			mSnippetView.layout(leftBound, textTopBound + mLine1Height
					+ mLine2Height, rightBound, textTopBound + mLine1Height
					+ mLine2Height + mLine3Height);
		}
	}
	...
	/**
	 * Loads the drawable for the vertical divider if it has not yet been
	 * loaded.
	 */
	private void ensureVerticalDivider() {
		if (mVerticalDividerDrawable == null) {
			...
			mVerticalDividerWidth = mVerticalDividerDrawable
					.getIntrinsicWidth();
		}
	}

	/**
	 * Loads the drawable for the horizontal divider if it has not yet been
	 * loaded.
	 */
	private void ensureHorizontalDivider() {
		if (mHorizontalDividerDrawable == null) {
			mHorizontalDividerDrawable = mContext
					.getResources()
					.getDrawable(
							com.android.internal.R.drawable.divider_horizontal_dark_opaque);
			mHorizontalDividerHeight = mHorizontalDividerDrawable
					.getIntrinsicHeight();
		}
	}
	...
	private void ensurePhotoViewSize() {
		if (mPhotoViewWidth == 0 && mPhotoViewHeight == 0) {
			...
			mPhotoViewWidth = a.getLayoutDimension(
					HiddenResource
					.getStylable("ViewGroup_Layout_layout_width")/*android.R.styleable.ViewGroup_Layout_layout_width*/,
					ViewGroup.LayoutParams.WRAP_CONTENT);
			mPhotoViewHeight = a.getLayoutDimension(
					HiddenResource
					.getStylable("ViewGroup_Layout_layout_height")/*android.R.styleable.ViewGroup_Layout_layout_height*/,
					ViewGroup.LayoutParams.WRAP_CONTENT);
			a.recycle();
		}
	}
		...
		if (!TextUtils.isEmpty(title)) {
			if (mHeaderTextView == null) {
				...
				mHeaderTextView.setTypeface(mHeaderTextView.getTypeface(),
						Typeface.BOLD);
				mHeaderTextView.setTextColor(mContext.getResources().getColor(
						com.android.internal.R.color.dim_foreground_dark));
	...
	public TextView getNameTextView() {
		if (mNameTextView == null) {
			...
			mNameTextView.setTextAppearance(mContext,
	...
	public void showCallButton(int id, int tag) {
		if (mCallButton == null) {
			...
			mCallButton
					.setPadding(mCallButtonPadding, 0, mCallButtonPadding, 0);
	...
	public void showSimView(int tag) {
		if (mSimView == null) {
			mSimView = new ImageView(mContext, null);
			mSimView.setBackgroundResource(R.drawable.call_background);
			mSimView.setImageResource(R.drawable.contacts_icon_sim);
			mSimView.setPadding(mSimViewPadding, 0, mSimViewPadding, 0);
			mSimView.setScaleType(ScaleType.CENTER);
			addView(mSimView);
		}

		mSimView.setTag(tag);
		mSimView.setVisibility(View.VISIBLE);
	}

	public void hideSimView() {
		if (mSimView != null) {
			mSimView.setVisibility(View.GONE);
		}
	}
	...
	public TextView getLabelView() {
		if (mLabelView == null) {
			...
			mLabelView.setTextAppearance(mContext,
	...
	public TextView getDataView() {
		if (mDataView == null) {
			...
			mDataView.setTextAppearance(mContext,
					android.R.style.TextAppearance_Small);
			addView(mDataView);
		}
		return mDataView;
	...
	public TextView getSnippetView() {
		if (mSnippetView == null) {
			...
			mSnippetView.setTextAppearance(mContext,
					android.R.style.TextAppearance_Small);
