...
import cn.edu.tsinghua.hpc.tcontacts.util.Maps;
import cn.edu.tsinghua.hpc.tcontacts.util.Sets;
...
public class Sources extends BroadcastReceiver implements OnAccountsUpdateListener {
    ...
    private ContactsSource mSimSource = null;
    ...
    protected void addSource(ContactsSource source) {
        if (DBG) Log.d(TAG, "----> addSource() source = " + source);
    ...
    protected synchronized void queryAccounts() {
        ...
        final IContentService cs = TMethod.getContentService();
        ...
        try {
            ...
            for (SyncAdapterType sync : syncs) {
                if (DBG) Log.d(TAG, "----> syncs +1 " + sync.toString());
                ...
                if (GoogleSource.ACCOUNT_TYPE.equals(accountType)) {
                    ...
                    if (DBG) Log.d(TAG, "----> auth.packageName " + auth.packageName);
                } else {
                    // TODO: use syncadapter package instead, since it provides resources
                    if (DBG) Log.d(TAG, "Creating external source for type=" + accountType
    ...
    public ArrayList<Account> getAccounts(boolean writableOnly) {
        ...
        //default account -> 
        Account localAccount = new Account(AccountTypeInfo.ACCOUNT_TYPE_PHONE,AccountTypeInfo.ACCOUNT_TYPE_PHONE);        
        matching.add(localAccount);
        //check Sim condition
        /*
        if(PhoneBookManageSim.isSimEnabled())
        {
            localAccount = new Account(AccountTypeInfo.ACCOUNT_TYPE_SIM,AccountTypeInfo.ACCOUNT_TYPE_SIM);
            matching.add(localAccount);
        }
        */
    ...
    public ContactsSource getInflatedSource(String accountType, int inflateLevel) {
        ...
        if (source == null && accountType.equals("com.android.contact.sim")){
            source = mSimSource;
        }else{
