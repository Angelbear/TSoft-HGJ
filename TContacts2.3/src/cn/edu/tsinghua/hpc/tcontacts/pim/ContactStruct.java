...
public class ContactStruct {
    ...
    public Uri pushIntoContentResolver(ContentResolver resolver) {
        ...
        if (mAccount != null) {
            ...
            if (ACCOUNT_TYPE_GOOGLE.equals(mAccount.type)) {
                final Cursor cursor = resolver.query(TGroups.CONTENT_URI, new String[] {
                        Groups.SOURCE_ID },
        ...
        builder.withValue("guid", this.guid);
        ...
        try {
        	// FIXME
        	ContentProviderResult[] results = resolver.applyBatch(TContactsContract.AUTHORITY, operationList);
        	return getRawContactUri(results);
    ...
    private Uri getRawContactUri(final ContentProviderResult[] results) {
        for (int i = 0; i < results.length; i++) {
            if (results[i].uri.toString().contains(
