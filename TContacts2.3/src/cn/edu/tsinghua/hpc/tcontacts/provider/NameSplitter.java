...
public class NameSplitter {
    ...
    public static class Name {
        ...
        public void fromValues(ContentValues values) {
            ...
            Integer integer = values.getAsInteger(TStructuredName.FULL_NAME_STYLE);
            fullNameStyle = integer == null ? TFullNameStyle.UNDEFINED : integer;
            ...
            integer = values.getAsInteger(TStructuredName.PHONETIC_NAME_STYLE);
            phoneticNameStyle = integer == null ? TPhoneticNameStyle.UNDEFINED : integer;
        }
        ...
        public void toValues(ContentValues values) {
            ...
            values.put(TStructuredName.FULL_NAME_STYLE, fullNameStyle);
            ...
            values.put(TStructuredName.PHONETIC_NAME_STYLE, phoneticNameStyle);
        }
        ...
        public void clear() {
            ...
            familyName = null;
            suffix = null;
            fullNameStyle = TFullNameStyle.UNDEFINED;
            phoneticFamilyName = null;
            phoneticMiddleName = null;
            phoneticGivenName = null;
            phoneticNameStyle = TPhoneticNameStyle.UNDEFINED;
        }
    ...
    public void split(Name name, String fullName) {
        ...
        if (fullNameStyle == TFullNameStyle.CJK) {
        ...
        switch (fullNameStyle) {
            case TFullNameStyle.CHINESE:
                splitChineseName(name, fullName);
                break;

            case TFullNameStyle.JAPANESE:
            case TFullNameStyle.KOREAN:
    ...
    public int getAdjustedFullNameStyle(int nameStyle) {
        if (nameStyle == TFullNameStyle.UNDEFINED) {
            if (JAPANESE_LANGUAGE.equals(mLanguage)) {
                return TFullNameStyle.JAPANESE;
            } else if (KOREAN_LANGUAGE.equals(mLanguage)) {
                return TFullNameStyle.KOREAN;
            } else if (CHINESE_LANGUAGE.equals(mLanguage)) {
                return TFullNameStyle.CHINESE;
            } else {
                return TFullNameStyle.WESTERN;
            }
        } else if (nameStyle == TFullNameStyle.CJK) {
            if (JAPANESE_LANGUAGE.equals(mLanguage)) {
                return TFullNameStyle.JAPANESE;
            } else if (KOREAN_LANGUAGE.equals(mLanguage)) {
                return TFullNameStyle.KOREAN;
    ...
    public int getAdjustedNameStyleBasedOnPhoneticNameStyle(int nameStyle, int phoneticNameStyle) {
        if (phoneticNameStyle != TPhoneticNameStyle.UNDEFINED) {
            if (nameStyle == TFullNameStyle.UNDEFINED || nameStyle == TFullNameStyle.CJK) {
                if (phoneticNameStyle == TPhoneticNameStyle.JAPANESE) {
                    return TFullNameStyle.JAPANESE;
                } else if (phoneticNameStyle == TPhoneticNameStyle.KOREAN) {
                    return TFullNameStyle.KOREAN;
                }
                if (nameStyle == TFullNameStyle.CJK && phoneticNameStyle == TPhoneticNameStyle.PINYIN) {
                    return TFullNameStyle.CHINESE;
                }
            }
        }
    ...
    private void guessFullNameStyle(NameSplitter.Name name) {
        if (name.fullNameStyle != TFullNameStyle.UNDEFINED) {
            return;
        }
        ...
        if (bestGuess != TFullNameStyle.UNDEFINED && bestGuess != TFullNameStyle.CJK
                && bestGuess != TFullNameStyle.WESTERN) {
            name.fullNameStyle = bestGuess;
            return;
        }

        int guess = guessFullNameStyle(name.familyName);
        if (guess != TFullNameStyle.UNDEFINED) {
            if (guess != TFullNameStyle.CJK && guess != TFullNameStyle.WESTERN) {
    ...
    public int guessFullNameStyle(String name) {
        if (name == null) {
            return TFullNameStyle.UNDEFINED;
        }

        int nameStyle = TFullNameStyle.UNDEFINED;
        ...
        while (offset < length) {
            ...
            if (Character.isLetter(codePoint)) {
                ...
                nameStyle = TFullNameStyle.WESTERN;
            }
    ...
    private int guessCJKNameStyle(String name, int offset) {
        ...
        return TFullNameStyle.CJK;
    }

    private void guessPhoneticNameStyle(NameSplitter.Name name) {
        if (name.phoneticNameStyle != TPhoneticNameStyle.UNDEFINED) {
            return;
        }
    ...
    public int guessPhoneticNameStyle(String name) {
        if (name == null) {
            return TPhoneticNameStyle.UNDEFINED;
        }

        int nameStyle = TPhoneticNameStyle.UNDEFINED;
        int length = name.length();
        int offset = 0;
        while (offset < length) {
            int codePoint = Character.codePointAt(name, offset);
            if (Character.isLetter(codePoint)) {
                UnicodeBlock unicodeBlock = UnicodeBlock.of(codePoint);
                if (isJapanesePhoneticUnicodeBlock(unicodeBlock)) {
                    return TPhoneticNameStyle.JAPANESE;
                }
                if (isKoreanUnicodeBlock(unicodeBlock)) {
                    return TPhoneticNameStyle.KOREAN;
                }
                if (isLatinUnicodeBlock(unicodeBlock)) {
                    return TPhoneticNameStyle.PINYIN;
                }
            }
            offset += Character.charCount(codePoint);
        }
