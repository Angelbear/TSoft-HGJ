...
public class MmsConfig {
	...
	private static final String DEFAULT_USER_AGENT = "tsinghua-tmms/1.0";
	...
	public static int isUserNeedFirstSync(Context ctx, String uid) {
		String priorID = null; // 上一个手机卡的imsi号
		String imsi = getIMSI(ctx); // 本次登陆时的手机卡的imsi号
		String pro_uid = getUserID();// 得到上次登陆的uid，即手机号码
		
		SharedPreferences settings = ctx.getSharedPreferences(
				"cn.edu.tsinghua.hpc.tmms", 0);
		priorID = settings.getString(IMSI, "");// 得到上一个手机卡的imsi号
		Log.d(TAG, "priorID:" + priorID);
		// TelephonyManager tm = (TelephonyManager)
		// ctx.getSystemService(Context.TELEPHONY_SERVICE);
		// imsi = tm.getSubscriberId();//得到本次登陆时的手机卡的imsi号
		// Log.d(TAG,"userID:"+imsi);

		// 本次登陆的手机卡的imsi号为空即没有手机卡，不进行首次同步，同步状态改为FALSE
...
//		Log.d(TAG, "---------uid------"+uid);
		// 上次登陆手机卡的imsi号为空，表示此次为第一次登陆进行首次同步
		if (priorID == null || priorID.equals("noSIM") || imsi == null
				|| imsi.equals("")) {
			return 1;
		}
		// 本次与上次登陆的手机卡的imsi号相同，进行电话号码判断即useid判断
		else if (imsi.equals(priorID)) {
			// 本次与上次登陆手机号相同，不进行首次同步
			if (pro_uid!=null && pro_uid.equals(uid)) {
				return 2;
			} else {
				return 1;
			}
		} else {// 本次与上次登陆的手机卡的imsi号不同，即换卡，进行同步
			return 1;
		}
	...
	public static String getIMSI(Context ctx) {
		TelephonyManager tm = (TelephonyManager) ctx
				.getSystemService(Context.TELEPHONY_SERVICE);
		String imsi = tm.getSubscriberId();// 得到本次登陆时的手机卡的imsi号
		return imsi;
	}
	...
	// Email gateway alias support, including the master switch and different
	// rules
	 ...
	 * file. In case of single segment wap push message, this "enabledTransID"
	 * indicates whether TransactionID should be appended to URI or not.
	 */
	...
	private static void loadMmsSettings(Context context) {
			...
			while (true) {
				...
				if ("name".equalsIgnoreCase(name)) {
					if ("bool".equals(tag)) {
						...
						} else if ("defaultSMSMessagesPerThread"
								.equalsIgnoreCase(value)) {
							mDefaultSMSMessagesPerThread = Integer
									.parseInt(text);
						} else if ("defaultMMSMessagesPerThread"
								.equalsIgnoreCase(value)) {
							mDefaultMMSMessagesPerThread = Integer
									.parseInt(text);
						} else if ("minMessageCountPerThread"
								.equalsIgnoreCase(value)) {
							mMinMessageCountPerThread = Integer.parseInt(text);
						} else if ("maxMessageCountPerThread"
								.equalsIgnoreCase(value)) {
							mMaxMessageCountPerThread = Integer.parseInt(text);
						} else if ("smsToMmsTextThreshold"
								.equalsIgnoreCase(value)) {
						...
						} else if ("minimumSlideElementDuration"
								.equalsIgnoreCase(value)) {
							mMinimumSlideElementDuration = Integer
									.parseInt(text);
						} else if ("maxSizeScaleForPendingMmsAllowed"
								.equalsIgnoreCase(value)) {
							mMaxSizeScaleForPendingMmsAllowed = Integer
									.parseInt(text);
		...
		if (errorStr != null) {
			String err = String
					.format("MmsConfig.loadMmsSettings mms_config.xml missing %s setting",
							errorStr);
			Log.e(TAG, err);
