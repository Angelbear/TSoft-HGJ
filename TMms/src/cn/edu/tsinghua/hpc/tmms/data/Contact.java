...
import cn.edu.tsinghua.hpc.tmms.util.TContactsContract.TPresence;
import cn.edu.tsinghua.hpc.tmms.util.TaskStack;
...
//    };
    ...
    private static boolean handleLocalNumber(Contact c) {
        if (MessageUtils.isLocalNumber(c.mNumber)) {
           // c.mName = Cache.getContext().getString(com.android.internal.R.string.me);// 在ophone手机上不能正确显示
           //Modified by Boern 2010/11/09
        	c.mName = Cache.getContext().getString(R.string.messagelist_sender_self);
    ...
    public synchronized Uri getUri() {
        return ContentUris.withAppendedId(TContacts.getContentUri(), mPersonId);
    }
    ...
    public static void startPresenceObserver() {
        Cache.getContext().getContentResolver().registerContentObserver(
                TPresence.CONTENT_URI, true, sPresenceObserver);
    }
