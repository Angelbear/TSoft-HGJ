...
public class Conversation {
	 ...
	 * with an empty recipient list, equivalent to {@link createEmpty}.
	 */
	 ...
	 * The recipient list of this conversation can be empty if the results were
	 * not in cache.
	 */
			...
			public void run() {
				if (threadUri != null) {
					...
					mContext.getContentResolver().update(threadUri,
							mReadContentValues, "read=0", null);
	...
	private static long getOrCreateThreadId(Context context, ContactList list) {
		...
		Log.d(TAG,"getOrCreateThreadId: "+recipients);
		if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
			Log.d(TAG,"getOrCreateThreadId before");
			LogTag.debug("getOrCreateThreadId %s", recipients);
		}
		return TThreads.getOrCreateThreadId(context, recipients);
	}
	...
	public static void cleanup(Context context) {
		...
		context.getContentResolver().delete(TThreads.OBSOLETE_THREADS_URI, null,
				null);
	}
	...
	public static void startQueryForAll(AsyncQueryHandler handler, int token) {
		String selection = "(sync_state = '" + SyncState.SYNC_STATE_PRESENT
				+ "' OR sync_state = '" + SyncState.SYNC_STATE_NOT_SYNC + "') and message_count != 0";
//		String selection = "sync_state = '" + SyncState.SYNC_STATE_PRESENT
//		+ "' OR sync_state = '" + SyncState.SYNC_STATE_NOT_SYNC + "' OR sync_state = '" + SyncState.SYNC_STATE_RECOVER+ "'";
//		String selection = "message_count != 0";
	 ...
	 * @param finalDelete
	 *            If this delete action is final delete
	 */
	...
	public static void startDeleteAll(AsyncQueryHandler handler, int token,
		...
		String selection = deleteAll ? null : "locked=0";
		// handler.startDelete(token, null, Threads.CONTENT_URI, selection,
		// null);
		ContentValues values = new ContentValues();
		if (!finalDelete) {
			values.put("sync_state", SyncState.SYNC_STATE_DELETED);
	...
	private static class Cache {
		...
		static void put(Conversation c) {
			synchronized (sInstance) {
				...
				if (DEBUG) {
					LogTag.debug("Conversation c: " + c
							+ " put with threadid: " + c.getThreadId()
							+ " c.hash: " + c.hashCode());
					dumpCache();
				}
				...
				while (c.moveToNext()) {
					...
					if (conv == null) {
						...
						try {
							...
							LogTag
									.error("Tried to add duplicate Conversation to Cache");
						}
					} else {
