...
import cn.edu.tsinghua.hpc.tmms.model.AudioModel;
import cn.edu.tsinghua.hpc.tmms.model.ImageModel;
import cn.edu.tsinghua.hpc.tmms.model.MediaModel;
import cn.edu.tsinghua.hpc.tmms.model.SlideModel;
import cn.edu.tsinghua.hpc.tmms.model.SlideshowModel;
import cn.edu.tsinghua.hpc.tmms.model.TextModel;
import cn.edu.tsinghua.hpc.tmms.model.VideoModel;
import cn.edu.tsinghua.hpc.tmms.transaction.MessageSender;
import cn.edu.tsinghua.hpc.tmms.transaction.MmsMessageSender;
import cn.edu.tsinghua.hpc.tmms.transaction.SmsMessageSender;
import cn.edu.tsinghua.hpc.tmms.ui.ComposeMessageActivity;
import cn.edu.tsinghua.hpc.tmms.ui.MessageUtils;
import cn.edu.tsinghua.hpc.tmms.ui.SlideshowEditor;
...
public class WorkingMessage {
     ...
     */
    public static WorkingMessage load(ComposeMessageActivity activity, Uri uri) throws InvalidHeaderValueException {
        // If the message is not already in the draft box, move it there.
        if (!uri.toString().startsWith(TMms.TDraft.CONTENT_URI.toString())) {
            PduPersister persister = PduPersister.getPduPersister(activity);
            if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
                LogTag.debug("load: moving %s to drafts", uri);
            }
            try {
                uri = persister.move(uri, TMms.TDraft.CONTENT_URI);
    ...
    private boolean loadFromUri(Uri uri) throws InvalidHeaderValueException {
    ...
    public static WorkingMessage loadDraft(ComposeMessageActivity activity,
                                           Conversation conv) throws InvalidHeaderValueException {
        WorkingMessage msg = new WorkingMessage(activity);
    ...
    private boolean loadFromConversation(Conversation conv) throws InvalidHeaderValueException {
     ...
     * @throws InvalidHeaderValueException 
     */
    public void readStateFromBundle(Bundle bundle) throws InvalidHeaderValueException {
        if (bundle == null) {
            return;
        }
    ...
    private void sendMmsWorker(Conversation conv, Uri mmsUri, PduPersister persister,
        ...
        try {
            cursor = SqliteWrapper.query(mContext, mContentResolver,
                    TMms.TOutbox.CONTENT_URI, MMS_OUTBOX_PROJECTION, null, null, null);
            if (cursor != null) {
    ...
    private static Uri readDraftMmsMessage(Context context, long threadId, StringBuilder sb) {
        ...
        cursor = SqliteWrapper.query(context, cr,
                TMms.TDraft.CONTENT_URI, MMS_DRAFT_PROJECTION,
                selection, null, null);

        Uri uri;
        try {
            if (cursor.moveToFirst()) {
                uri = ContentUris.withAppendedId(TMms.TDraft.CONTENT_URI,
    ...
    private static Uri createDraftMmsMessage(PduPersister persister, SendReq sendReq,
        ...
        try {
            ...
            sendReq.setBody(pb);
            Uri res = persister.persist(sendReq, TMms.TDraft.CONTENT_URI);
            slideshow.sync(pb);
            return res;
        } catch (MmsException e) {
            return null;
    ...
    private static String readDraftSmsMessage(Context context, long thread_id, Conversation conv) {
        ...
        Uri thread_uri = ContentUris.withAppendedId(TSms.TConversations.CONTENT_URI, thread_id);
        String body = "";
...
//        if (conv.getMessageCount() == 0) {
        if (conv.getMessageCount() == 0 && thread_id<=0) {//modified by chenqiang
    ...
    private void updateDraftSmsMessage(long thread_id, String contents) {
        ...
        SqliteWrapper.insert(mContext, mContentResolver, TSms.CONTENT_URI, values);
    ...
    private void asyncDeleteDraftSmsMessage(Conversation conv) {
        long threadId = conv.getThreadId();
        if (threadId > 0) {
            asyncDelete(ContentUris.withAppendedId(TSms.TConversations.CONTENT_URI, threadId),
    ...
    private void deleteDraftSmsMessage(long threadId) {
        SqliteWrapper.delete(mContext, mContentResolver,
                ContentUris.withAppendedId(TSms.TConversations.CONTENT_URI, threadId),
                SMS_DRAFT_WHERE, null);
    }

    private void asyncDeleteDraftMmsMessage(long threadId) {
        final String where = Mms.THREAD_ID + " = " + threadId;
        asyncDelete(TMms.TDraft.CONTENT_URI, where, null);
    }
