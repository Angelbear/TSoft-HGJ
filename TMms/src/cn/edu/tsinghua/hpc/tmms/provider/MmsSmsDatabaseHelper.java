...
import static cn.edu.tsinghua.hpc.google.tmms.pdu.PduHeaders.MESSAGE_TYPE_DELIVERY_IND;
import static cn.edu.tsinghua.hpc.google.tmms.pdu.PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND;
import static cn.edu.tsinghua.hpc.google.tmms.pdu.PduHeaders.MESSAGE_TYPE_READ_ORIG_IND;
import static cn.edu.tsinghua.hpc.google.tmms.pdu.PduHeaders.MESSAGE_TYPE_READ_REC_IND;
import static cn.edu.tsinghua.hpc.google.tmms.pdu.PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF;
...
//                        "        AND sms." + Sms.TYPE + " != 3) + " +
//                        add by chenqiang
                        "        AND sms." + Sms.TYPE + " != 3 AND (sms.sync_state = 'present' or sms.sync_state='recover')) + " +
...
//                        "        AND " + Mms.MESSAGE_BOX + " != 3) " +
                        //add by chenqiang
                        "        AND " + Mms.MESSAGE_BOX + " != 3 AND (pdu.sync_state = 'present' or pdu.sync_state='recover'))  " +
...
//                      "        AND " + Mms.MESSAGE_BOX + " != 3) " +
     ...
     * database.
     */
    /* package */ public static synchronized MmsSmsDatabaseHelper getInstance(Context context) {
        if (mInstance == null) {
...
//   				   "guid INTEGER ," + 
                 //add by chenqiang
        		   ...
        		   "sync_enable INTERGER DEFAULT 1,"+
		 ...
		 * Add by Yangyang Zhao
		 */
		db.execSQL("CREATE TABLE " + "mms_data" + " (" + BaseColumns._ID
				+ " INTEGER PRIMARY KEY," + "mms_id INTEGER DEFAULT -1,"
				+ "data TEXT);");
    }
...
//                   "  UPDATE OF " + Mms.DATE + ", " + Mms.SUBJECT + ", " + Mms.MESSAGE_BOX +
        		//add by chenqiang
        		"  UPDATE OF " + Mms.DATE + ", " + Mms.SUBJECT + ", " + Mms.MESSAGE_BOX + ", " + "sync_state" +
...
//                   "  UPDATE OF " + Sms.DATE + ", " + Sms.BODY + ", " + Sms.TYPE +
        		//add by chenqiang
        		   "  UPDATE OF " + Sms.DATE + ", " + Sms.BODY + ", " + Sms.TYPE +", " + "sync_state" +
                   "  ON sms " +
                   SMS_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE);
...
//        //add by chenqiang 短信sync_state变化后相应thread的变化
//        db.execSQL("CREATE TRIGGER sms_update_thread_sync_state "+
//					"AFTER UPDATE OF sync_state "+
//					"ON sms "+
//					"BEGIN "+
//					"update threads set message_count = "+ 
//					"case when new.sync_state='present' then message_count+1 "+
//					"when new.sync_state='deleted' and message_count>0 then message_count-1 "+
//					"else 0 "+
//					"end "+
//					"where _id = new.thread_id; "+
//					"update threads set sync_state = "+ 
//			        "case when message_count<=0 then 'deleted'  "+ 
//			        "else 'present' "+ 
//			        "end "+ 
//			        "where _id = new.thread_id; "+ 
//					"END;");
//        //add by chenqiang 会话sync_state变化后相应短信的变化
//        db.execSQL("CREATE TRIGGER thread_update_sms_sync_state "+
//					"AFTER UPDATE OF sync_state "+
//					"ON threads "+
//					"FOR EACH ROW "+
//					"BEGIN "+
//					"update sms set sync_state =  "+
//					"case when new.sync_state='deleted' then 'deleted' " +
//					"else sync_state "+
//					"end "+
//					"where thread_id = new._id; "+
//					"END;");
    ...
    */
    public void wipeData() {
        ...
        db.execSQL("DELETE FROM " + MmsSmsProvider.TABLE_PENDING_MSG);
        db.execSQL("DELETE FROM sms");
        db.execSQL("DELETE FROM mms_data");
        db.execSQL("DELETE FROM raw");
        db.execSQL("DELETE FROM attachments");
//        db.execSQL("DELETE FROM thread_ids");
        db.execSQL("DELETE FROM sr_pending");
        db.execSQL("DELETE FROM " + MmsProvider.TABLE_PDU + ";");
        db.execSQL("DELETE FROM " + MmsProvider.TABLE_ADDR + ";");
        db.execSQL("DELETE FROM " + MmsProvider.TABLE_PART + ";");
        db.execSQL("DELETE FROM " + MmsProvider.TABLE_RATE + ";");
        db.execSQL("DELETE FROM " + MmsProvider.TABLE_DRM + ";");
    ...
    private void upgradeDatabaseToVersion46(SQLiteDatabase db) {
        ...
        try {
                ...
                while (textRows.moveToNext()) {
                    ...
                    if (path != null) {
						try {
							...
							Method updateStringMethod = Class.forName(
									"android.database.Cursor").getMethod(
									"updateString",
									new Class[] { int.class, String.class });
							updateStringMethod.setAccessible(true);
							updateStringMethod.invoke(textRows, new Object[] {
									partTextColumn, v.getString() });

							Method updateToNullMethod = Class.forName(
									"android.database.Cursor").getMethod(
									"updateToNull", new Class[] { int.class });
							updateToNullMethod.setAccessible(true);
							updateToNullMethod.invoke(textRows,
									new Object[] { partDataColumn });

							// textRows.updateString(partTextColumn,
							// v.getString());
			...
			Method commitUpdatesMethod;
			try {
				commitUpdatesMethod = Class.forName("android.database.Cursor")
						.getMethod("updateString", new Class[] {});
				commitUpdatesMethod.setAccessible(true);
				commitUpdatesMethod.invoke(textRows, new Object[] {});
			} catch (Exception e) {
				Log.d(TAG, e.getMessage());
			}
