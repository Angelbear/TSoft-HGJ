...
public class TelephonyProvider extends ContentProvider
{
    ...
    static {
    	//modified by chenqiang "telephony"->"ttelephony"
        s_urlMatcher.addURI("ttelephony", "carriers", URL_TELEPHONY);
        s_urlMatcher.addURI("ttelephony", "carriers/current", URL_CURRENT);
        s_urlMatcher.addURI("ttelephony", "carriers/#", URL_ID);
        s_urlMatcher.addURI("ttelephony", "carriers/restore", URL_RESTOREAPN);
        s_urlMatcher.addURI("ttelephony", "carriers/preferapn", URL_PREFERAPN);
    ...
    public Uri insert(Uri url, ContentValues initialValues)
    {
        ...
        switch (match)
        {
            case URL_TELEPHONY:
            {
                ...
                if (rowID > 0)
                {
                    result = ContentUris.withAppendedId(TTelephony.TCarriers.CONTENT_URI, rowID);
                    notify = true;
                }
        ...
        if (notify) {
            getContext().getContentResolver().notifyChange(TTelephony.TCarriers.CONTENT_URI, null);
        }
