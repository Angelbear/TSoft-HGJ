...
public abstract class Recycler {
	...
	abstract protected void deleteMessagesForThread(Context context,
			long threadId, int keep);
	...
	public static class SmsRecycler extends Recycler {
		...
		static private final String[] SMS_MESSAGE_PROJECTION = new String[] {
				BaseColumns._ID, Conversations.THREAD_ID, Sms.ADDRESS,
				Sms.BODY, Sms.DATE, Sms.READ, Sms.TYPE, Sms.STATUS, };
		...
		public int getMessageLimit(Context context) {
			SharedPreferences prefs = PreferenceManager
					.getDefaultSharedPreferences(context);
			return prefs.getInt(MAX_SMS_MESSAGES_PER_THREAD, MmsConfig
					.getDefaultSMSMessagesPerThread());
		}
		...
		protected Cursor getAllThreads(Context context) {
			...
			Cursor cursor = SqliteWrapper.query(context, resolver,
					TTelephony.TSms.TConversations.CONTENT_URI,
					ALL_SMS_THREADS_PROJECTION, null, null,
					Conversations.DEFAULT_SORT_ORDER);

			return cursor;
		}

		protected void deleteMessagesForThread(Context context, long threadId,
				int keep) {
			if (LOCAL_DEBUG) {
				Log.v(TAG, "SMS: deleteMessagesForThread");
			}
			ContentResolver resolver = context.getContentResolver();
			Cursor cursor = SqliteWrapper.query(context, resolver, ContentUris
					.withAppendedId(TSms.TConversations.CONTENT_URI, threadId),
					SMS_MESSAGE_PROJECTION, "locked=0", null, "date DESC"); // get
																			// in
																			// newest
																			// to
																			// oldest
			...
			try {
				// Move to the keep limit and then delete everything older than
				// that one.
				cursor.move(keep);
				...
				values.put("sync_state", SyncState.SYNC_STATE_ARCHIVED);
				
				long cntDeleted = SqliteWrapper.update(context, resolver,
						ContentUris.withAppendedId(
								TSms.TConversations.CONTENT_URI, threadId),
						values, "locked=0 AND date<" + latestDate, null);

				//long cntDeleted = SqliteWrapper.delete(context, resolver,
				//		ContentUris.withAppendedId(
				//				Sms.Conversations.CONTENT_URI, threadId),
				//		"locked=0 AND date<" + latestDate, null);
				if (LOCAL_DEBUG) {
		...
		protected void dumpMessage(Cursor cursor, Context context) {
			...
			String dateStr = MessageUtils.formatTimeStampString(context, date,
					true);
			if (LOCAL_DEBUG) {
				Log.v(TAG, "Recycler message " + "\n    address: "
						+ cursor.getString(COLUMN_SMS_ADDRESS) + "\n    body: "
			...
			try {
				while (cursor.moveToNext()) {
					...
					Cursor msgs = SqliteWrapper.query(context, resolver,
							ContentUris.withAppendedId(
									TSms.TConversations.CONTENT_URI, threadId),
							SMS_MESSAGE_PROJECTION, "locked=0", null,
	...
	public static class MmsRecycler extends Recycler {
		...
		static private final String[] MMS_MESSAGE_PROJECTION = new String[] {
				BaseColumns._ID, Conversations.THREAD_ID, Mms.DATE, };
		...
		public int getMessageLimit(Context context) {
			SharedPreferences prefs = PreferenceManager
					.getDefaultSharedPreferences(context);
			return prefs.getInt(MAX_MMS_MESSAGES_PER_THREAD, MmsConfig
					.getDefaultMMSMessagesPerThread());
		}
		...
		protected Cursor getAllThreads(Context context) {
			...
			Cursor cursor = SqliteWrapper.query(context, resolver, Uri
					.withAppendedPath(TTelephony.TMms.CONTENT_URI, "threads"),
					ALL_MMS_THREADS_PROJECTION, null, null,
					Conversations.DEFAULT_SORT_ORDER);

			return cursor;
		}

		public void deleteOldMessagesInSameThreadAsMessage(Context context,
				Uri uri) {
			if (LOCAL_DEBUG) {
			...
			try {
				...
				cursor = SqliteWrapper.query(context, resolver,
						TTelephony.TMms.CONTENT_URI, MMS_MESSAGE_PROJECTION,
						"thread_id in (select thread_id from pdu where _id="
								+ msgId + ") AND locked=0", null, "date DESC"); // get
																				// in
																				// newest
																				// to
																				// oldest
		...
		protected void deleteMessagesForThread(Context context, long threadId,
			...
			try {
				...
				cursor = SqliteWrapper.query(context, resolver,
						TTelephony.TMms.CONTENT_URI, MMS_MESSAGE_PROJECTION,
						"thread_id=" + threadId + " AND locked=0", null,
						"date DESC"); // get in newest to oldest order
		...
		private void deleteMessagesOlderThanDate(Context context,
				long threadId, long latestDate) {
			
			// Add by Yangyang Zhao
			ContentValues values = new ContentValues();
			values.put("sync_state", SyncState.SYNC_STATE_ARCHIVED);
			
			long cntDeleted = SqliteWrapper.update(context, context
					.getContentResolver(),
					ContentUris.withAppendedId(
							 TTelephony.TMms.CONTENT_URI, threadId),
					values, "locked=0 AND date<" + latestDate, null);

			
			//long cntDeleted = SqliteWrapper.delete(context, context
			//		.getContentResolver(), Telephony.Mms.CONTENT_URI,
			//		"thread_id=" + threadId + " AND locked=0 AND date<"
			//				+ latestDate, null);
			if (LOCAL_DEBUG) {
				Log.v(TAG, "MMS: deleteMessagesOlderThanDate cntDeleted: "
						+ cntDeleted);
			}
		}
