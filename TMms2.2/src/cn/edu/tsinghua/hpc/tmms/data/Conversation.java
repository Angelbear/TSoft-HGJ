package cn.edu.tsinghua.hpc.tmms.data;
...
public class Conversation {
	...
	private static final Uri sAllThreadsUri = TThreads.CONTENT_URI.buildUpon()
			.appendQueryParameter("simple", "true").build();

	private static final String[] ALL_THREADS_PROJECTION = { Threads._ID,
	...
	private static final String[] UNREAD_PROJECTION = { Threads._ID,
			Threads.READ };
	 ...
	 * be called as many times as you like; the conversation will not be created
	 * in the database until {@link #ensureThreadId} is called.
	 */
	...
	/**
	 * Find the conversation matching the provided recipient set. When called
	 * with an empty recipient list, equivalent to {@link #createNew}.
	 */
	public static Conversation get(Context context, ContactList recipients,
		...
		conv = new Conversation(context, threadId, allowQuery);
		Log.d(TAG,
				"Conversation.get: created new conversation " + conv.toString());

		if (!conv.getRecipients().equals(recipients)) {
			Log.e(TAG,
					"Conversation.get: new conv's recipients don't match input recpients "
							+ recipients);
		}
	...
	/**
	 * Find the conversation matching in the specified Uri. Example forms:
	 * {@value content://mms-sms/conversations/3} or {@value sms:+12124797990}.
	...
	public static Conversation get(Context context, Uri uri, boolean allowQuery) {
		...
		return get(context, ContactList.getByNumbers(recipient,
				allowQuery /* don't block */, true /* replace number */),
				allowQuery);
	}

	/**
	 * Returns true if the recipient in the uri matches the recipient list in
	 * this conversation.
	 */
	...
	public static Conversation from(Context context, Cursor cursor) {
		// First look in the cache for the Conversation and return that one.
		// That way, all the
		// people that are looking at the cached copy will get updated when
		// fillFromCursor() is
	...
	/**
	 * Marks all messages in this conversation as read and updates relevant
	 * notifications. This method returns immediately; work is dispatched to a
	 * background thread.
	 */
			...
			public void run() {
				synchronized (mMarkAsBlockedSyncer) {
					...
					if (threadUri != null) {
						...
						// Check the read flag first. It's much faster to do a
						// query than
						// to do an update. Timing this function show it's about
						// 10x faster to
						// do the query compared to the update, even when
						// there's nothing to
						// update.
						boolean needUpdate = true;

						Cursor c = mContext.getContentResolver().query(
								threadUri, UNREAD_PROJECTION, UNREAD_SELECTION,
								null, null);
						if (c != null) {
							try {
						...
						if (needUpdate) {
							...
							mContext.getContentResolver().update(threadUri,
									mReadContentValues, UNREAD_SELECTION, null);
						}
	...
	public synchronized Uri getUri() {
		...
		return ContentUris.withAppendedId(TThreads.CONTENT_URI, mThreadId);
	}
	...
	public static Uri getUri(long threadId) {
		...
		return ContentUris.withAppendedId(TThreads.CONTENT_URI, threadId);
	}
	...
	/**
	 * Guarantees that the conversation has been created in the database. This
	 * will make a blocking database call if it hasn't.
	 * 
	...
	public synchronized void clearThreadId() {
		...
		if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
			LogTag.debug("clearThreadId old threadId was: " + mThreadId
					+ " now zero");
		}
	...
	/**
	 * Sets the list of recipients associated with this conversation. If called,
	 * {@link #ensureThreadId} must be called before the next operation that
	 * depends on this conversation existing in the database (e.g. storing a
	 * draft message to it).
	 */
	...
	/**
	 * Returns a snippet of text from the most recent message in the
	 * conversation.
	 */
	...
	private static long getOrCreateThreadId(Context context, ContactList list) {
		...
		long retVal = TThreads.getOrCreateThreadId(context, recipients);
		if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
			LogTag.debug(
	...
	/*
	 * The primary key of a conversation is its recipient set; override equals()
	...
	public static void asyncDeleteObsoleteThreads(AsyncQueryHandler handler,
			int token) {
		handler.startDelete(token, null, TThreads.OBSOLETE_THREADS_URI, null,
				null);
	}
	...
	public static void startQueryForAll(AsyncQueryHandler handler, int token) {
		String selection = "(sync_state = '" + SyncState.SYNC_STATE_PRESENT
				+ "' OR sync_state = '" + SyncState.SYNC_STATE_NOT_SYNC
				+ "') and message_count != 0";
		// String selection = "sync_state = '" + SyncState.SYNC_STATE_PRESENT
		// + "' OR sync_state = '" + SyncState.SYNC_STATE_NOT_SYNC +
		// "' OR sync_state = '" + SyncState.SYNC_STATE_RECOVER+ "'";
		// String selection = "message_count != 0";
	...
	public static void startDelete(AsyncQueryHandler handler, int token,
			boolean deleteAll, long threadId, boolean finalDelete) {
	...
	public static void startDeleteAll(AsyncQueryHandler handler, int token,
			boolean deleteAll, boolean finalDelete) {
		String selection = deleteAll ? null : "locked=0";
		// handler.startDelete(token, null, TThreads.CONTENT_URI, selection,
		// null);
		ContentValues values = new ContentValues();
		if (!finalDelete) {
			values.put("sync_state", SyncState.SYNC_STATE_DELETED);
		...
		handler.startUpdate(token, null, TThreads.CONTENT_URI, values,
				selection, null);
	}
	...
	public static void startQueryHaveLockedMessages(AsyncQueryHandler handler,
		...
		handler.cancelOperation(token);
		Uri uri = TMmsSms.CONTENT_LOCKED_URI;
		if (threadId != -1) {
	 ...
	 * cursor, possibly setting recipients to empty if {@value allowQuery} is
	 * false and the recipient IDs are not in cache. The cursor should be one
	 * made via {@link #startQueryForAll}.
	 */
	private static void fillFromCursor(Context context, Conversation conv,
		...
		synchronized (conv) {
			...
			String snippet = MessageUtils.extractEncStrFromCursor(c, SNIPPET,
					SNIPPET_CS);
		...
		// Fill in as much of the conversation as we can before doing the slow
		// stuff of looking
		...
		if (Log.isLoggable(LogTag.THREAD_CACHE, Log.VERBOSE)) {
			LogTag.debug("fillFromCursor: conv=" + conv + ", recipientIds="
					+ recipientIds);
		}
	}
	...
	private static class Cache {
		...
		/**
		 * Return the conversation with the specified thread ID, or null if it's
		 * not in cache.
		 */
		...
		/**
		 * Return the conversation with the specified recipient list, or null if
		 * it's not in cache.
		 */
		...
		/**
		 * Put the specified conversation in the cache. The caller should not
		 * place an already-existing conversation in the cache, but rather
		 * update it in place.
		 */
		static void put(Conversation c) {
			synchronized (sInstance) {
				...
				if (Log.isLoggable(LogTag.THREAD_CACHE, Log.VERBOSE)) {
					LogTag.debug("Conversation.Cache.put: conv= " + c
							+ ", hash: " + c.hashCode());
				}
		...
		static void dumpCache() {
			synchronized (sInstance) {
				...
				for (Conversation c : sInstance.mCache) {
					LogTag.debug("   conv: " + c.toString() + " hash: "
							+ c.hashCode());
				}
			}
		}
		...
		/**
		 * Remove all conversations from the cache that are not in the provided
		 * set of thread IDs.
		 */
	...
	/**
	 * Set up the conversation cache. To be called once at application startup
	 * time.
	 */
	...
	private static void blockingMarkAllSmsMessagesAsSeen(final Context context) {
		...
		Cursor cursor = resolver.query(TSms.TInbox.CONTENT_URI,
				SEEN_PROJECTION, "seen=0", null, null);
		...
		resolver.update(TSms.TInbox.CONTENT_URI, values, "seen=0", null);
	}

	private static void blockingMarkAllMmsMessagesAsSeen(final Context context) {
		ContentResolver resolver = context.getContentResolver();
		Cursor cursor = resolver.query(TMms.TInbox.CONTENT_URI,
				SEEN_PROJECTION, "seen=0", null, null);
		...
		resolver.update(TMms.TInbox.CONTENT_URI, values, "seen=0", null);
	...
	private boolean loadFromThreadId(long threadId, boolean allowQuery) {
		Cursor c = mContext.getContentResolver().query(sAllThreadsUri,
				ALL_THREADS_PROJECTION, "_id=" + Long.toString(threadId), null,
				null);
		try {
			if (c.moveToFirst()) {
