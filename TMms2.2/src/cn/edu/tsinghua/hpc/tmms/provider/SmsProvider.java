...
import cn.edu.tsinghua.hpc.tmms.util.TMethod;
import cn.edu.tsinghua.hpc.tmms.util.TTelephony;
...
public class SmsProvider extends ContentProvider {
    private static final Uri NOTIFICATION_URI = TSms.CONTENT_URI;//Uri.parse("content://sms");
    private static final Uri ICC_URI = Uri.withAppendedPath(TSms.CONTENT_URI, "icc");//Uri.parse("content://sms/icc");
    ...
    private Cursor getSingleMessageFromIcc(String messageIndexString) {
        try {
            ...
            ArrayList<SmsMessage> messages = TMethod.getAllMessagesFromIcc(smsManager);
    ...
    public Uri insert(Uri url, ContentValues initialValues) {
        ...
        if (table.equals(TABLE_SMS)) {
            ...
            if (((threadId == null) || (threadId == 0)) && (address != null)) {
                values.put(Sms.THREAD_ID, TThreads.getOrCreateThreadId(
                                   getContext(), address));
            }
        ...
        if (rowID > 0) {
            Uri uri = Uri.parse("content://" + TTelephony.PADAUTHORITY + table + "/" + rowID);
    ...
    public int delete(Uri url, String where, String[] whereArgs) {
        ...
        switch (match) {
            ...
            case SMS_CONVERSATIONS_ID:
                ...
                where = TMethod.concatenateWhere("thread_id=" + threadID, where);
                count = db.delete(TABLE_SMS, where, whereArgs);
    ...
    private int deleteMessageFromIcc(String messageIndexString) {
        ...
        try {
            //return smsManager.deleteMessageFromIcc(Integer.parseInt(messageIndexString))
            //        ? 1 : 0;
            return TMethod.deleteMessageFromIcc(Integer.parseInt(messageIndexString))
            ? 1 : 0;
        } catch (NumberFormatException exception) {
            throw new IllegalArgumentException(
                    "Bad SMS ICC ID: " + messageIndexString);
    ...
    public int update(Uri url, ContentValues values, String where, String[] whereArgs) {
        ...
        where = TMethod.concatenateWhere(where, extraWhere);
    ...
    private void notifyChange(Uri uri) {
        ...
        cr.notifyChange(TMmsSms.CONTENT_URI, null);
        cr.notifyChange(Uri.parse("content://"+TMmsSms.AUTHORITY+"/conversations/"), null);
    }
    ...
    static {
    	String smsAuthority = TSms.AUTHORITY;
        sURLMatcher.addURI(smsAuthority, null, SMS_ALL);
        sURLMatcher.addURI(smsAuthority, "#", SMS_ALL_ID);
        sURLMatcher.addURI(smsAuthority, "inbox", SMS_INBOX);
        sURLMatcher.addURI(smsAuthority, "inbox/#", SMS_INBOX_ID);
        sURLMatcher.addURI(smsAuthority, "sent", SMS_SENT);
        sURLMatcher.addURI(smsAuthority, "sent/#", SMS_SENT_ID);
        sURLMatcher.addURI(smsAuthority, "draft", SMS_DRAFT);
        sURLMatcher.addURI(smsAuthority, "draft/#", SMS_DRAFT_ID);
        sURLMatcher.addURI(smsAuthority, "outbox", SMS_OUTBOX);
        sURLMatcher.addURI(smsAuthority, "outbox/#", SMS_OUTBOX_ID);
        sURLMatcher.addURI(smsAuthority, "undelivered", SMS_UNDELIVERED);
        sURLMatcher.addURI(smsAuthority, "failed", SMS_FAILED);
        sURLMatcher.addURI(smsAuthority, "failed/#", SMS_FAILED_ID);
        sURLMatcher.addURI(smsAuthority, "queued", SMS_QUEUED);
        sURLMatcher.addURI(smsAuthority, "conversations", SMS_CONVERSATIONS);
        sURLMatcher.addURI(smsAuthority, "conversations/*", SMS_CONVERSATIONS_ID);
        sURLMatcher.addURI(smsAuthority, "raw", SMS_RAW_MESSAGE);
        sURLMatcher.addURI(smsAuthority, "attachments", SMS_ATTACHMENT);
        sURLMatcher.addURI(smsAuthority, "attachments/#", SMS_ATTACHMENT_ID);
        sURLMatcher.addURI(smsAuthority, "threadID", SMS_NEW_THREAD_ID);
        sURLMatcher.addURI(smsAuthority, "threadID/*", SMS_QUERY_THREAD_ID);
        sURLMatcher.addURI(smsAuthority, "status/#", SMS_STATUS_ID);
        sURLMatcher.addURI(smsAuthority, "sr_pending", SMS_STATUS_PENDING);
        sURLMatcher.addURI(smsAuthority, "icc", SMS_ALL_ICC);
        sURLMatcher.addURI(smsAuthority, "icc/#", SMS_ICC);
        //we keep these for not breaking old applications
        sURLMatcher.addURI(smsAuthority, "sim", SMS_ALL_ICC);
        sURLMatcher.addURI(smsAuthority, "sim/#", SMS_ICC);
