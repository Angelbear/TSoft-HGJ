...
import cn.edu.tsinghua.hpc.tmms.ui.ClassZeroActivity;
...
import cn.edu.tsinghua.hpc.tmms.util.TTelephony.TSms.TInbox;
import cn.edu.tsinghua.hpc.tmms.util.TTelephony.TSms.TOutbox;
...
//        }
    ...
    private void handleServiceStateChanged(Intent intent) {
        ...
        ServiceState serviceState = TMethod.newFromBundle(intent.getExtras());
    ...
    public synchronized void sendFirstQueuedMessage() {
        ...
        final Uri uri = Uri.withAppendedPath(TSms.CONTENT_URI, "queued");//Uri.parse("content://sms/queued");
        ...
        if (c != null) {
            try {
                if (c.moveToFirst()) {
                    ...
                    Uri msgUri = ContentUris.withAppendedId(TSms.CONTENT_URI, msgId);
    ...
    private void moveOutboxMessagesToQueuedBox() {
        ...
        SqliteWrapper.update(
                getApplicationContext(), getContentResolver(), TOutbox.CONTENT_URI,
    ...
    private Uri insertMessage(Context context, SmsMessage[] msgs, int error) {
        ...
        if (sms.getMessageClass() == SmsMessage.MessageClass.CLASS_0) {
            ...
            result = replaceMessage(context, msgs, error);
            MmsUtils.markMessageSyncDirty(context, result, 1);
            return result;
        } else {
            result =  storeMessage(context, msgs, error);
            MmsUtils.markMessageOrThread(context, result,
    ...
    private Uri replaceMessage(Context context, SmsMessage[] msgs, int error) {
        ...
        Cursor cursor = SqliteWrapper.query(context, resolver, TInbox.CONTENT_URI,
    ...
    private Uri storeMessage(Context context, SmsMessage[] msgs, int error) {
        ...
        if (((threadId == null) || (threadId == 0)) && (address != null)) {
            threadId = TThreads.getOrCreateThreadId(context, address);
            values.put(Sms.THREAD_ID, threadId);
        }

        ContentResolver resolver = context.getContentResolver();

        Uri insertedUri = SqliteWrapper.insert(context, resolver, TInbox.CONTENT_URI, values);
